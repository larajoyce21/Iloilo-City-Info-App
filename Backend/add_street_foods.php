...
if (!file_exists('uploads/street_foods/')) {
    mkdir('uploads/street_foods/', 0777, true);
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("SELECT image, image_detail FROM street_foods WHERE id=?");
    $stmt->bind_param("i", $delete_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $food = $result->fetch_assoc();
    
    $stmt = $conn->prepare("DELETE FROM street_foods WHERE id=?");
    $stmt->bind_param("i", $delete_id);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['message'] = "Street food deleted successfully";
    } else {
        $_SESSION['error'] = "Error deleting street food: " . $conn->error;
    }
    
    header("Location: add_street_foods.php");
    exit();
}

// Image deletion
if (isset($_GET['delete_image']) && isset($_GET['food_id'])) {
    ...
    $stmt = $conn->prepare("SELECT image_detail FROM street_foods WHERE id=?");
    ...
    if ($food) {
        ...
        $stmt = $conn->prepare("UPDATE street_foods SET image_detail=? WHERE id=?");
        $stmt->bind_param("si", $updated_images_str, $food_id);
        $stmt->execute();
    }
    
    $_SESSION['message'] = "Image deleted successfully";
    header("Location: add_street_foods.php?edit=" . $food_id);
    exit();
}

// Fetch all street foods
$query = "SELECT id, name, image, description, location FROM street_foods ORDER BY name";
$stmt = $conn->prepare($query);
$stmt->execute();
$street_foods = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $target_dir = "uploads/street_foods/";
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $new_filename)) {
            ...
            header("Location: add_street_foods.php" . ($id ? "?edit=$id" : ""));
            exit();
        }
    }
    ...
    if (!empty($_FILES['image_detail']['name'][0])) {
        ...
        if ($totalCombined > 5) {
            $_SESSION['error'] = "You can upload a maximum of 5 detail images total";
            header("Location: add_street_foods.php" . ($id ? "?edit=$id" : ""));
            exit();
        }
    ...
    if ($id) {
        $query = "UPDATE street_foods SET 
                  name=?, image=?, image_detail=?, description=?, location=?, 
                  maps_embed=?, maps_link=?, directions=?, hours=?, nearby_places=?, 
                  stores=?, stalls=?, corners=?, markets=?, details_link=?, transportation_routes=?
                  WHERE id=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssssssssssi", 
            $name, $target_file, $target_file_detail, $description, $location, 
            $mapEmbedURL, $maps_link, $directions, $hours, $nearby_places_json, 
            $stores, $stalls, $corners, $markets, $details_link, $transportation_routes_json, $id);
    } else {
        $query = "INSERT INTO street_foods 
                  (name, image, image_detail, description, location, maps_embed, 
                   maps_link, directions, hours, nearby_places, stores, stalls, 
                   corners, markets, details_link, transportation_routes) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssssssssss", 
            $name, $target_file, $target_file_detail, $description, $location, 
            $mapEmbedURL, $maps_link, $directions, $hours, $nearby_places_json, 
            $stores, $stalls, $corners, $markets, $details_link, $transportation_routes_json);
    }
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Street food " . ($id ? "updated" : "added") . " successfully";
    } else {
        $_SESSION['error'] = "Error saving street food: " . $conn->error;
    }
...
// Prepare nearby places string for edit form
$nearby_places_str = '';
$transportation_routes = array();
$route_count = 1;
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT nearby_places, transportation_routes FROM street_foods WHERE id=?");
...
<head>
    ...
    <title>Street Food Management</title>
...
<body>
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="page-title">Street Food Management</h1>
            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#foodModal">
                <i class="bi bi-plus-lg"></i> Add Street Food
            </button>
        </div>
        
        <?php if ($street_foods->num_rows == 0): ?>
            <div class="alert alert-info">
                No street foods found. Click "Add Street Food" to create one.
            </div>
        <?php else: ?>
            <div class="row row-cols-1 row-cols-md-3 row-cols-lg-4 g-4">
                <?php while ($food = $street_foods->fetch_assoc()): ?>
                    <div class="col">
                        <div class="card h-100">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-between">
                                    <a href="add_street_foods.php?edit=<?= $food['id'] ?>" class="btn btn-primary btn-sm">
                                        <i class="bi bi-pencil"></i> Edit
                                    </a>
                                    <a href="?delete=<?= $food['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this street food?')">
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="foodModalLabel"><?= isset($_GET['edit']) ? 'Edit Street Food' : 'Add Street Food' ?></h5>
                ...
                <div class="modal-body">
                    <?php 
                        ...
                        if (isset($_GET['edit'])) {
                            $edit_id = (int)$_GET['edit'];
                            $stmt = $conn->prepare("SELECT * FROM street_foods WHERE id=?");
                            $stmt->bind_param("i", $edit_id);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            if ($result->num_rows > 0) {
                    ...
                    ?>
                    <form method="POST" action="add_street_foods.php" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="details_link" value="<?= htmlspecialchars($foodData['details_link']) ?>" placeholder="e.g., street_food_details.php?id=1">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="nearby_places" value="<?= htmlspecialchars($nearby_places_str) ?>" placeholder="e.g., Park, School, Market">
                                        ...
                                        ?>
                                            <div class="image-preview">
                                                ...
                                                <div class="delete-image-btn" 
                                                     onclick="if(confirm('Delete this image?')) window.location.href='?delete_image=<?= urlencode($img) ?>&food_id=<?= $foodData['id'] ?>'">
                                                    ×
                                                </div>
                                            </div>
                                        <?php 
                        ...
                        <div class="form-section">
                            <h5>Street Food Locations</h5>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label class="form-label">Stores (separate with |)</label>
                                        <textarea class="form-control" name="stores" rows="3"><?= htmlspecialchars($foodData['stores']) ?></textarea>
                                        <small class="text-muted">Example: Store Name|Location|Hours</small>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Street Corners (separate with |)</label>
                                        <textarea class="form-control" name="corners" rows="3"><?= htmlspecialchars($foodData['corners']) ?></textarea>
                                        <small class="text-muted">Example: Corner Name|Intersection|Hours</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label class="form-label">Stalls (separate with |)</label>
                                        <textarea class="form-control" name="stalls" rows="3"><?= htmlspecialchars($foodData['stalls']) ?></textarea>
                                        <small class="text-muted">Example: Stall Name|Market Name|Hours</small>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Markets (separate with |)</label>
                                        <textarea class="form-control" name="markets" rows="3"><?= htmlspecialchars($foodData['markets']) ?></textarea>
                                        <small class="text-muted">Example: Market Name|Location|Operating Days</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                                            ...
                                            <tr>
                                                <td>
                                                    <select class="form-control" name="route_type[]">
                                                        ...
                                                        <option value="pedicab" <?php echo (isset($transportation_routes[$i]['type']) && $transportation_routes[$i]['type'] == 'pedicab') ? 'selected' : ''; ?>>Pedicab</option>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control" name="route_name[]" value="<?php echo isset($transportation_routes[$i]['name']) ? htmlspecialchars($transportation_routes[$i]['name']) : ''; ?>" placeholder="e.g., Route 101, Main Street, Market Alley">
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <textarea class="form-control" name="route_description[]" rows="2" placeholder="e.g., Main transportation to the food area, nearest stops"><?php echo isset($transportation_routes[$i]['description']) ? htmlspecialchars($transportation_routes[$i]['description']) : ''; ?></textarea>
                                                </td>
                        ...
                        <div class="form-section">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <small class="text-muted">Example: Monday-Friday: 8AM-8PM, Saturday: 9AM-6PM</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="url" class="form-control" name="map_embed" value="<?= htmlspecialchars($foodData['maps_embed']) ?>" placeholder="https://maps.google.com/embed...">
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <?= $foodData['id'] ? 'Update Street Food' : 'Save Street Food' ?>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        function addRow() {
            ...
            newRow.innerHTML = `
                ...
                <td>
                    <input type="text" class="form-control" name="route_name[]" placeholder="e.g., Route 101, Main Street, Market Alley">
                </td>
                <td>
                    <input type="text" class="form-control" name="route_link[]" placeholder="https://maps.google.com/...">
                </td>
                <td>
                    <textarea class="form-control" name="route_description[]" rows="2" placeholder="e.g., Main transportation to the food area, nearest stops"></textarea>
                </td>
                <td class="text-center">
                    <button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">
                        Remove
