...
$result = $conn->query("SELECT * FROM souvenirs");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'souvenirs') {
...
$souvenirs_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $souvenirs_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="souvenirs">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Iloilo City Souvenirs and Pasalubong</h1> 
        
        <div class="card-container">
            <?php if ($souvenirs_exist): ?>
                <?php while ($souvenir = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($souvenir['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($souvenir['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($souvenir['name']) ?></h6>
                            <?php if (!empty($souvenir['details_link'])): ?>
                                <a href="<?= htmlspecialchars($souvenir['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No souvenir  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
