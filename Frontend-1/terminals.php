...
$result = $conn->query("SELECT * FROM terminals");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'terminals') {
...
if ($stmt = $conn->prepare($query)) {
    ...
    $terminals_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div id="mySidebar" class="sidebar">
  ...
  <div class="mt-auto p-4 text-center">
    ...
    <div class="album py-5" id="terminals">
        <div class="container">
            <br>
            <br>
            <br>
            <br>
            <h1 class="page-title text-light text-center">Transport Terminals in Iloilo City</h1> 
            
            <div class="card-container">
                <?php if ($terminals_exist): ?>
                    <?php while ($terminal = $result->fetch_assoc()): ?>
                        <div class="card shadow-sm">
                            <div class="card-img-container">
                                <img src="<?= htmlspecialchars($terminal['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($terminal['name']) ?>">
                            </div>
                            <div class="card-body text-center">
                                <h6 class="card-title"><?= htmlspecialchars($terminal['name']) ?></h6>
                                <?php if (!empty($terminal['details_link'])): ?>
                                    <a href="<?= htmlspecialchars($terminal['details_link']) ?>" class="card-button">
                                        View Details
                                    </a>
                                <?php else: ?>
                ...
                <?php else: ?>
                    <div class="col-12">
                        <div class="alert alert-info text-center">No terminals found.</div>
                    </div>
                <?php endif; ?>
            </div>
        </div>
