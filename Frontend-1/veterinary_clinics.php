...
$result = $conn->query("SELECT * FROM veterinary_clinics");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'veterinary_clinics') {
...
$query = "SELECT * FROM veterinary_clinics";
$verinarys_exist = false; 

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $verinarys_exist = $result->num_rows > 0; 
    $stmt->close();
}
?>
...
<br>
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Veterinary Clinics in Iloilo City</h1>

        <div class="card-container">
            <?php if ($verinarys_exist): ?>
                <?php while ($veterinary = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($veterinary['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($veterinary['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($veterinary['name']) ?></h6>
                            <?php if (!empty($veterinary['details_link'])): ?>
                                <a href="<?= htmlspecialchars($veterinary['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No veterinary found.</div>
            <?php endif; ?>
        </div>
