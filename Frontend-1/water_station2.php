...
$query = "SELECT * FROM water_stations WHERE id = 2";
$stmt = $conn->prepare($query);
$stmt->execute();
$result = $stmt->get_result();
$water_station = $result->fetch_assoc();

$transport_routes = array();
if (!empty($water_station['transportation_routes'])) {
    $routes_data = json_decode($water_station['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($water_station['nearby_places'])) {
    $decoded = json_decode($water_station['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $water_station['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $water_station['social_media'] ?? '')));

$hours = [];
if (!empty($water_station['hours'])) {
    $hours = explode("\n", $water_station['hours']);
}

$services = [];
if (!empty($water_station['services'])) {
    $services = array_filter(array_map('trim', explode("\n", $water_station['services'])));
}

$branches = [];
if (!empty($water_station['branches'])) {
    $branches = array_filter(array_map('trim', explode("\n", $water_station['branches'])));
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($water_station['name']) ?> - Iloilo Water Stations</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($water_station['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($water_station['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($water_station['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($water_station['location'] ?? '') ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($water_station['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($water_station['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($water_station['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">💧 Water Station Information</h2>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🚰 Water Services</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <?php foreach ($services as $service): ?>
                                    <li class="list-group-item border-0 px-0 py-1">
                                        <div class="d-flex align-items-start">
                                            ...
                                            <span class="small"><?= htmlspecialchars($service) ?></span>
                                        </div>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <p class="text-muted small mb-0">Branch information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🚰 Water Services</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($services)): ?>
                                <ul class="list-group list-group-flush">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Station Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($water_station['contact'] ?? '') ?></p>
                            <?php if (!empty($water_station['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($water_station['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($water_station['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($water_station['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($water_station['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Station Details</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($water_station['contact'] ?? '') ?></p>
                            <?php if (!empty($water_station['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($water_station['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($water_station['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($water_station['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($water_station['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($water_station['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($water_station['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($water_station['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($water_station['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($water_station['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($water_station['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($water_station['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($water_station['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="contactModalLabel">Contact <?= htmlspecialchars($water_station['name'] ?? '') ?></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <p class="mb-2"><strong>Phone:</strong> <?= htmlspecialchars($water_station['contact'] ?? '') ?></p>
                    <?php if (!empty($water_station['email'])): ?>
                        <p class="mb-2"><strong>Email:</strong> <?= htmlspecialchars($water_station['email']) ?></p>
                    <?php endif; ?>
                    <?php if (!empty($water_station['website'])): ?>
                        <p class="mb-2"><strong>Website:</strong> <a href="<?= htmlspecialchars($water_station['website']) ?>" target="_blank"><?= htmlspecialchars($water_station['website']) ?></a></p>
                    <?php endif; ?>
                </div>
                <div class="mb-3">
                    <h6>Location</h6>
                    <p class="mb-0"><?= htmlspecialchars($water_station['location'] ?? '') ?></p>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <a href="tel:<?= htmlspecialchars($water_station['contact'] ?? '') ?>" class="btn btn-primary">Call Now</a>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="directionsModal" tabindex="-1" aria-labelledby="directionsModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    <p>Find the best route to <?= htmlspecialchars($water_station['name'] ?? '') ?></p>
                    <?php if (!empty($water_station['directions_link'])): ?>
                        <a href="<?= htmlspecialchars($water_station['directions_link']) ?>" class="btn btn-primary w-100 mb-2" target="_blank">
                            <i class="fas fa-directions me-2"></i> Open in Google Maps
                        </a>
                    <?php endif; ?>
                    <?php if (!empty($water_station['maps_link'])): ?>
                        <a href="<?= htmlspecialchars($water_station['maps_link']) ?>" class="btn btn-outline-primary w-100" target="_blank">
                ...
                <div class="transport-info">
                    ...
                    <?php
                    $transport_options = !empty($water_station['transportation_options']) 
                        ? explode("\n", $water_station['transportation_options']) 
                        : [
                            "Jeepney or Bus: Available routes pass near the location",
                            "Taxi: Available throughout the city",
                            "Ride-hailing: Grab and other services available",
                            "Walking: Accessible by foot from nearby areas"
                        ];
                    ...
                    ?>
                        <div class="d-flex align-items-start mb-2">
                            ...
                            <div>
                                ...
                                <p class="mb-0 small"><?= htmlspecialchars(trim($parts[1] ?? '')) ?></p>
                            </div>
                        </div>
                    <?php 
                        endif;
...
<div class="modal fade" id="servicesModal" tabindex="-1" aria-labelledby="servicesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="servicesModalLabel">Water Services</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?php if (!empty($services)): ?>
                ...
                <div class="mt-4">
                    <h6>Pricing</h6>
                    <div class="row">
                        <?php if (!empty($water_station['price_gallon'])): ?>
                            <div class="col-6">
                                <p class="mb-1"><strong>Per Gallon:</strong></p>
                                <p class="mb-0">₱<?= htmlspecialchars($water_station['price_gallon']) ?></p>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($water_station['price_liter'])): ?>
                            <div class="col-6">
                                <p class="mb-1"><strong>Per Liter:</strong></p>
                                <p class="mb-0">₱<?= htmlspecialchars($water_station['price_liter']) ?></p>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($water_station['delivery_fee'])): ?>
                            <div class="col-12 mt-2">
                                <p class="mb-1"><strong>Delivery Fee:</strong></p>
                                <p class="mb-0"><?= htmlspecialchars($water_station['delivery_fee']) ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
