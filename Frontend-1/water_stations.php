...
$result = $conn->query("SELECT * FROM water_stations");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'water_stations') {
...
$water_stations_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $water_stations_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>WATER RIFILLING STATIONS</title>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Water Rifilling Stations in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($water_stations_exist): ?>
                <?php while ($water_station = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($water_station['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($water_station['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($water_station['name']) ?></h6>
                            <?php if (!empty($water_station['details_link'])): ?>
                                <a href="<?= htmlspecialchars($water_station['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No water station found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
