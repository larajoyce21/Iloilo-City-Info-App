...
$result = $conn->query("SELECT * FROM agricultural_stores");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'agricultural_stores') {
...
$query = "SELECT * FROM agricultural_stores";
$agricultural_stores_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $agricultural_stores_exist = $result->num_rows > 0; 
    $stmt->close();
}
?>
...
<head>
    ...
    <title>AGRICULTURAL AND FARMING SUPPLY STORES</title>
...
<div id="mySidebar" class="sidebar">
  ...
  <div class="mt-auto p-4 text-center">
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Agricultural and Farming Supply Stores in Iloilo City</h1>

        <div class="card-container">
            <?php if ($agricultural_stores_exist): ?>
                <?php while ($agricultural_store = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($agricultural_store['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($agricultural_store['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($agricultural_store['name']) ?></h6>
                            <?php if (!empty($agricultural_store['details_link'])): ?>
                                <a href="<?= htmlspecialchars($agricultural_store['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No agricultural store found.</div>
            <?php endif; ?>
        </div>
