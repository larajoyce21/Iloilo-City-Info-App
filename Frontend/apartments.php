...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'apartments') {
...
$apartments_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $apartments_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Apartments in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($apartments_exist): ?>
                <?php while ($apartment = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($apartment['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($apartment['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($apartment['name']) ?></h6>
                            <?php if (!empty($apartment['details_link'])): ?>
                                <a href="<?= htmlspecialchars($apartment['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No apartment found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
