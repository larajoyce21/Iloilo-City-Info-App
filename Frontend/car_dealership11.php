...
$query = "SELECT * FROM car_dealerships WHERE id = 11";
...
if (!empty($dealership['transportation_routes'])) {
    $routes_data = json_decode($dealership['transportation_routes'], true);
...
$brands_available = [];
if (!empty($dealership['brands_available'])) {
    $brands_available = json_decode($dealership['brands_available'], true);
    if (!is_array($brands_available)) {
        $brands_available = [];
    }
}

$services_offered = [];
if (!empty($dealership['services_offered'])) {
    $services_offered = json_decode($dealership['services_offered'], true);
...
if (!empty($dealership['image_detail'])) {
    $detail_images = explode(',', $dealership['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $dealership['social_media'] ?? '')));

$hours = [];
if (!empty($dealership['hours'])) {
    $hours = explode("\n", $dealership['hours']);
}
$nearby_places = [];
if (!empty($dealership['nearby_places'])) {
    $nearby_places = json_decode($dealership['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$locations = array_filter(array_map('trim', explode('|', $dealership['location'] ?? '')));
?>
...
<head>
    ...
    <title><?= htmlspecialchars($dealership['name']) ?> - Iloilo Car Dealerships</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($dealership['image'] ?? '') ?>');
        ...
        /* Car Dealership Specific Styles */
        .brands-list {
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($dealership['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($dealership['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($dealership['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($dealership['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($dealership['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($dealership['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🚗 Dealership Information</h2>
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <?= nl2br(htmlspecialchars($dealership['hours'])) ?>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2">📲 Phone:  <?php echo htmlspecialchars($dealership['contact']); ?></p>
                        <?php if (!empty($dealership['email'])): ?>
                            <p class="mb-2">📧 Email: <?php echo htmlspecialchars($dealership['email']); ?></p>
                        <?php endif; ?>
                        <?php if (!empty($dealership['website'])): ?>
                            <p class="mb-2">🌐 Website: <a href="<?php echo htmlspecialchars($dealership['website']); ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body small">
                            <p class="mb-2">📲 Phone:  <?php echo htmlspecialchars($dealership['contact']); ?></p>
                            <?php if (!empty($dealership['email'])): ?>
                                <p class="mb-0">📧 Email: <?php echo htmlspecialchars($dealership['email']); ?></p>
                            <?php endif; ?>
                            <?php if (!empty($dealership['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?php echo htmlspecialchars($dealership['website']); ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
    ...
    <section class="content-section">
        <h2 class="section-title">🚘 Brands & Services</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏷️ Available Brands</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($brands_available)): ?>
                            <p class="mb-2 small">Explore these car brands available at <?= htmlspecialchars($dealership['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($brands_available as $brand): ?>
                                    <span class="badge bg-danger badge-custom"><?= htmlspecialchars($brand) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No brands listed.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($services_offered)): ?>
                            <p class="mb-2 small">Services available at <?= htmlspecialchars($dealership['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($services_offered as $service): ?>
                                    <span class="badge bg-success badge-custom"><?= htmlspecialchars($service) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No services listed.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🏷️ Available Brands</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($brands_available)): ?>
                                <p class="mb-2 small">Available brands:</p>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($brands_available as $brand): ?>
                                        <span class="badge bg-danger badge-custom"><?= htmlspecialchars($brand) ?></span>
                                    <?php endforeach; ?>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($services_offered)): ?>
                                <p class="mb-2 small">Available services:</p>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($services_offered as $service): ?>
                                        <span class="badge bg-success badge-custom"><?= htmlspecialchars($service) ?></span>
                                    <?php endforeach; ?>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($dealership['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($dealership['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($dealership['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($dealership['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($dealership['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($dealership['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($dealership['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($dealership['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($dealership['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
