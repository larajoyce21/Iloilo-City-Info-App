...
$result = $conn->query("SELECT * FROM car_dealerships");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'car_dealerships') {
...
$query = "SELECT * FROM car_dealerships";
$car_dealerships_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $car_dealerships_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Car Dealerships in Iloilo City</h1>

        <div class="card-container">
            <?php if ($car_dealerships_exist): ?>
                <?php while ($car_dealership = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($car_dealership['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($car_dealership['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($car_dealership['name']) ?></h6>
                            <?php if (!empty($car_dealership['details_link'])): ?>
                                <a href="<?= htmlspecialchars($car_dealership['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No car dealership found.</div>
            <?php endif; ?>
        </div>
