...
$query = "SELECT * FROM clothing_stores WHERE id = 28";
...
if (!empty($store['transportation_routes'])) {
    $routes_data = json_decode($store['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($store['nearby_places'])) {
    $nearby_places = json_decode($store['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $store['image_detail'] ?? ''));
...
<head>
    ...
    <title><?= htmlspecialchars($store['name'] ?? 'Fashion Store Iloilo') ?> - Iloilo Shopping Guide</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($store['image'] ?? '') ?>');
        ...
        /* Transport Options (like in apartment1.php) */
        .transport-option {
            margin-bottom: 10px;
        }
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($store['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($store['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($store['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($store['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($store['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($store['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">👕 Products & Brands</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($store['brands'])): ?>
                            <div class="mb-4">
                                ...
                                <div class="clothing-categories">
                                    ...
                                    foreach ($brands as $brand):
                                        if (trim($brand) != ''): ?>
                                            <span class="brand-tag"><?= htmlspecialchars(trim($brand)) ?></span>
                                        <?php endif;
                                    endforeach; ?>
                                </div>
                            </div>
                        ...
                        <?php if (!empty($store['price_range'])): ?>
                            <div class="mb-4">
                                <h6>Price Range</h6>
                                <div class="price-range"><?= htmlspecialchars($store['price_range']) ?></div>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏪 Store Features</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-4">
                            ...
                            <p class="ms-4"><?= !empty($store['payment_options']) ? htmlspecialchars($store['payment_options']) : '' ?></p>
                        </div>
                        
                        <div class="mb-4">
                            <h6 class="d-flex align-items-center">
                                <i class="fas fa-exchange-alt me-2"></i> Return Policy
                            </h6>
                            <p class="ms-4"><?= !empty($store['return_policy']) ? htmlspecialchars($store['return_policy']) : '' ?></p>
                        </div>
                        
                        <div class="mb-4">
                            <h6 class="d-flex align-items-center">
                                <i class="fas fa-truck me-2"></i> Delivery Services
                            </h6>
                            <p class="ms-4"><?= !empty($store['delivery']) ? htmlspecialchars($store['delivery']) : '' ?></p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                ...
                                <p class="mb-0"><?= !empty($store['established']) ? htmlspecialchars($store['established']) : '' ?></p>
                            </div>
                        </div>
                        
                        <?php if (!empty($store['size_availability'])): ?>
                            <div class="mt-3">
                                <h6>Available Sizes</h6>
                                <p><?= htmlspecialchars($store['size_availability']) ?></p>
                                <?php if (!empty($store['size_chart'])): ?>
                                    <a href="<?= htmlspecialchars($store['size_chart']) ?>" class="size-chart-link" target="_blank">
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">👕 Products & Brands</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($store['brands'])): ?>
                                <div class="mb-4">
                                    <h6 class="small">Featured Brands</h6>
                                    <div class="clothing-categories">
                                        <?php
                                        $brands = explode("\n", $store['brands']);
                                        foreach ($brands as $brand):
                            ...
                            <?php if (!empty($store['price_range'])): ?>
                                <div class="mb-4">
                                    <h6 class="small">Price Range</h6>
                                    <div class="price-range small"><?= htmlspecialchars($store['price_range']) ?></div>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-4">
                                ...
                                <p class="ms-4 small"><?= !empty($store['payment_options']) ? htmlspecialchars($store['payment_options']) : '' ?></p>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="ms-4 small"><?= !empty($store['return_policy']) ? htmlspecialchars($store['return_policy']) : '' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h6 class="mb-0 small">Established</h6>
                                    <p class="mb-0 small"><?= !empty($store['established']) ? htmlspecialchars($store['established']) : '' ?></p>
                                </div>
                            </div>
                            
                            <?php if (!empty($store['size_availability'])): ?>
                                <div class="mt-3">
                                    <h6 class="small">Available Sizes</h6>
                                    <p class="small"><?= htmlspecialchars($store['size_availability']) ?></p>
                                    <?php if (!empty($store['size_chart'])): ?>
                                        <a href="<?= htmlspecialchars($store['size_chart']) ?>" class="size-chart-link small" target="_blank">
                                            <i class="fas fa-ruler me-1"></i> View Size Chart
                                        </a>
                                    <?php endif; ?>
                                </div>
    ...
    <section class="content-section">
                        ...
                        <div class="row">
                            <?php if (!empty($store['price_range'])): ?>
                                <div class="col-md-6 mb-2">
                                    ...
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($store['price_range'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2 ">📲 Phone: <?= htmlspecialchars($store['contact'] ?? '') ?></p>
                            <?php if (!empty($store['email'])): ?>
                                <p class="mb-2 ">📧 Email: <?= htmlspecialchars($store['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($store['website'])): ?>
                                <p class="mb-2 ">🌐 Website: <a href="<?= htmlspecialchars($store['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="row">
                                <?php if (!empty($store['price_range'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">💵 Price Range</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($store['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($store['contact'] ?? '') ?></p>
                                <?php if (!empty($store['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($store['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($store['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($store['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                        ...
                        <div class="card-body small">
                            <?php if (!empty($hours)): ?>
                                <div class="hours-schedule">
                                    <?php foreach ($hours as $day => $times): ?>
                                        <div class="hours-day mb-2">
                                            <h6 class="fw-bold mb-1 small"><?= htmlspecialchars($day) ?></h6>
                                            <p class="mb-1 small"><?= implode('<br>', array_map('htmlspecialchars', $times)) ?></p>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-info mb-0 py-2 small">No operating hours available.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($store['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($store['specialty'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>💵 Price Range:</strong> <?= htmlspecialchars($store['price_range'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Branches in Iloilo City</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <p class="text-muted small mb-0">Store information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($store['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($store['specialty'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💵 Price Range:</strong> <?= htmlspecialchars($store['price_range'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-8 mb-3">
                <?php if (!empty($store['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($store['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($store['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($store['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($store['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($store['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($store['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($store['online_store'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($store['online_store']) ?>" class="btn btn-dark btn-sm" target="_blank">
                                        <i class="fas fa-download me-1"></i> Visit Online Store
                                    </a>
