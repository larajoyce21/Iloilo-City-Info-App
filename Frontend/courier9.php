...
$query = "SELECT * FROM couriers WHERE id = 9";
...
if (!empty($courier['transportation_routes'])) {
    $routes_data = json_decode($courier['transportation_routes'], true);
...
$service_areas = [];
if (!empty($courier['service_areas'])) {
    $service_areas = json_decode($courier['service_areas'], true) ?: [];
}

$detail_images = array_filter(explode(',', $courier['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $courier['social_media'] ?? '')));

$hours = [];
if (!empty($courier['hours'])) {
    $hours = explode("\n", $courier['hours']);
}

$branches = [];
if (!empty($courier['branches'])) {
    $branches = array_filter(array_map('trim', explode("\n", $courier['branches'])));
}
$nearby_places = [];
if (!empty($courier['nearby_places'])) {
    $decoded = json_decode($courier['nearby_places'], true);
...
<head>
    ...
    <title><?= htmlspecialchars($courier['name'] ?? 'Courier Service') ?> - Iloilo Courier Services</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($courier['image'] ?? '') ?>');
...
<br>
    <section class="hero-section" 
        style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($courier['image'] ?? '') ?>') center/cover no-repeat;">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($courier['name'] ?? '') ?></h1>
            <p class="lead"><?= htmlspecialchars($courier['location'] ?? '') ?></p>
        </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($courier['name'] ?? '') ?></h2>
            <div class="lead mb-4">
                <?= nl2br(htmlspecialchars($courier['description'] ?? '')) ?>
            </div>
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($courier['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            <h2 class="section-title">🚚 Courier Service Information</h2>
            ...
            <div class="row d-none d-md-flex">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <?php if (!empty($courier['price_range'])): ?>
                                    <div class="col-md-6 mb-2">
                                        <h6 class="fw-bold">💵 Price Range</h6>
                                        <p class="mb-2"><?= nl2br(htmlspecialchars($courier['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                    
                                <p class="mb-2 ">📲 Phone: <?= htmlspecialchars($courier['contact'] ?? '') ?></p>
                                <?php if (!empty($courier['email'])): ?>
                                    <p class="mb-2 ">📧 Email: <?= htmlspecialchars($courier['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($courier['website'])): ?>
                                    <p class="mb-2 ">🌐 Website: <a href="<?= htmlspecialchars($courier['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    <?php if (!empty($courier['price_range'])): ?>
                                        <div class="col-12 mb-2">
                                            ...
                                            <p class="mb-2 small"><?= nl2br(htmlspecialchars($courier['price_range'])) ?></p>
                                        </div>
                                    <?php endif; ?>
                                    <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($courier['contact'] ?? '') ?></p>
                                    <?php if (!empty($courier['email'])): ?>
                                        <p class="mb-2 small">📧 Email: <?= htmlspecialchars($courier['email']) ?> </p>
                                    <?php endif; ?>
                                    <?php if (!empty($courier['website'])): ?>
                                        <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($courier['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row d-none d-md-flex">
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?= nl2br(htmlspecialchars($courier['hours'])) ?>
            ...
            <div class="d-md-none mobile-side-by-side">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-code-branch me-2"></i> Branches in Iloilo City</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($branches)): ?>
                                <div class="list-group list-group-flush">
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($courier['established'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($courier['established'] ?? '') ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($courier['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($courier['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($courier['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($courier['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($courier['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($courier['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($courier['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                        ...
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($courier['tracking_link'])): ?>
                                    ...
                                    <h5 class="mb-2 small fw-bold">📦 Package Tracking</h5>
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($courier['tracking_link']) ?>" class="btn btn-dark btn-sm" target="_blank">
                                            <i class="fas fa-search me-1"></i> Track Package
                                        </a>
                                        <a href="#" class="btn btn-outline-dark btn-sm" target="_blank">
                                            <i class="fas fa-info-circle me-1"></i> Tracking Help
                                        </a>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="d-flex flex-wrap gap-1">
                                    <?php if (!empty($courier['contact'])): ?>
                                        <a href="tel:<?= htmlspecialchars(preg_replace('/[^0-9+]/', '', $courier['contact'])) ?>" class="btn btn-outline-primary btn-sm">
                                            <i class="fas fa-phone me-1"></i> Call Office
                                        </a>
                                    <?php endif; ?>
                                    
                                    <?php if (!empty($courier['email'])): ?>
                                        <a href="mailto:<?= htmlspecialchars($courier['email']) ?>" class="btn btn-outline-secondary btn-sm">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (shareBtn) {
            shareBtn.addEventListener('click', function() {
                if (navigator.share) {
                    navigator.share({
                        title: '<?= addslashes($courier['name'] ?? 'Courier Information') ?>',
                        text: 'Check out this courier service in Iloilo City',
                    ...
                    alert('Share this page using your browser\'s share functionality or copy the URL: ' + window.location.href);
                }
            });
        }
        ...
        const courierDays = document.querySelectorAll('.courier-day');
        courierDays.forEach(day => {
