...
$result = $conn->query("SELECT * FROM dental_clinics");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'dental_clinics') {
...
$query = "SELECT * FROM dental_clinics";
$dental_clinics_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $dental_clinics_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bars">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Dental Clinics in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($dental_clinics_exist): ?>
                <?php while ($dental_clinic = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($dental_clinic['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($dental_clinic['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($dental_clinic['name']) ?></h6>
                            <?php if (!empty($dental_clinic['details_link'])): ?>
                                <a href="<?= htmlspecialchars($dental_clinic['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No dental_clinic found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
