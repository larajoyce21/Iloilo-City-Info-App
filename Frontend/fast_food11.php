...
$query = "SELECT * FROM fast_foods WHERE id = 11";
$stmt = $conn->prepare($query);
$stmt->execute();
$result = $stmt->get_result();
$fast_food = $result->fetch_assoc();

$transport_routes = array();
if (!empty($fast_food['transportation_routes'])) {
    $routes_data = json_decode($fast_food['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($fast_food['nearby_places'])) {
    $nearby_places = json_decode($fast_food['nearby_places'], true);
...
$detail_images = [];
if (!empty($fast_food['image_detail'])) {
    $detail_images = explode(',', $fast_food['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $fast_food['social_media'] ?? '')));

$hours = [];
if (!empty($cultural['hours'])) {
    $hours = explode("\n", $cultural['hours']);
}
...
<head>
    ...
    <title><?= htmlspecialchars($fast_food['name']) ?> - Iloilo Fast Foods</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($fast_food['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($fast_food['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($fast_food['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($fast_food['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($fast_food['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($fast_food['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($fast_food['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Fast Food Information</h2>
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($fast_food['menu'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏷️ Special Offers</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($fast_food['offers'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($fast_food['menu'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🏷️ Special Offers</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($fast_food['offers'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($fast_food['price_range'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Price Range</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($fast_food['price_range'])) ?></p>
                                </div>
                            <?php endif; ?>
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($fast_food['contact'] ?? '') ?></p>
                            <?php if (!empty($fast_food['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($fast_food['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($fast_food['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($fast_food['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?= nl2br(htmlspecialchars($fast_food['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="row">
                                <?php if (!empty($fast_food['price_range'])): ?>
                                    <div class="col-12 mb-2">
                                        ...
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($fast_food['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($fast_food['contact'] ?? '') ?></p>
                                <?php if (!empty($fast_food['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($fast_food['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($fast_food['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($fast_food['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($fast_food['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($fast_food['specialty'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>💵 Price Range:</strong> <?= htmlspecialchars($fast_food['price_range'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($fast_food['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $fast_food['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($fast_food['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($fast_food['specialty'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💵 Price Range:</strong> <?= htmlspecialchars($fast_food['price_range'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($fast_food['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($fast_food['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($fast_food['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($fast_food['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($fast_food['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($fast_food['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($fast_food['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($fast_food['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($fast_food['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
