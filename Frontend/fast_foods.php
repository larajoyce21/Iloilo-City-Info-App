...
$result = $conn->query("SELECT * FROM fast_foods");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'fast_foods') {
...
$query = "SELECT * FROM fast_foods";
$fast_foods_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $fast_foods_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Fast Foods Restaurants in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($fast_foods_exist): ?>
                <?php while ($fast_food = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($fast_food['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($fast_food['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($fast_food['name']) ?></h6>
                            <?php if (!empty($fast_food['details_link'])): ?>
                                <a href="<?= htmlspecialchars($fast_food['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No fast food found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
