...
$query = "SELECT * FROM funeral_services WHERE id = 1";
...
if (!empty($funeral_service['transportation_routes'])) {
    $routes_data = json_decode($funeral_service['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($funeral_service['nearby_places'])) {
    $nearby_places = json_decode($funeral_service['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $funeral_service['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $funeral_service['social_media'] ?? '')));

$hours = [];
if (!empty($funeral_service['hours'])) {
    $hours = explode("\n", $funeral_service['hours']);
}
...
<head>
    ...
    <title><?= htmlspecialchars($funeral_service['name'] ?? 'Funeral Service') ?> - Iloilo Funeral Services</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($funeral_service['image'] ?? '') ?>');
       ...
       .slideshow-container {
        ...
        /* Fuel and Service Badges */
        .fuel-badge, .service-badge {
            background-color: #004a8d;
            color: white;
            margin-right: 5px;
            margin-bottom: 5px;
        ...
        .fuel-badge:hover, .service-badge:hover {
            background-color: var(--primary-color);
            transform: translateY(-1px);
        }
        
        /* Transport Options (Updated to match apartment1.php) */
        .transport-option {
            margin-bottom: 10px;
        }
...
<section class="hero-section" style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),
    url('<?= htmlspecialchars($funeral_service['image'] ?? '') ?>') center/cover no-repeat;">
    <div class="container">
        <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($funeral_service['name'] ?? 'Funeral Service Name') ?></h1>
        <p class="lead"><?= htmlspecialchars($funeral_service['location'] ?? 'Funeral Service Location') ?></p>
    </div>
</section>

<main class="container mb-4">
    <section class="content-section">
        <h2 class="section-title">About <?= htmlspecialchars($funeral_service['name'] ?? 'Our Funeral Service') ?></h2>
        <div class="lead mb-4">
            <?= nl2br(htmlspecialchars($funeral_service['description'] ?? 'Funeral service description goes here.')) ?>
        </div>
            ...
            <div class="slideshow-container">
                <?php foreach ($detail_images as $index => $image): ?>
                    <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                        <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($funeral_service['name'] ?? 'Funeral Service') ?> Detail <?= $index + 1 ?>" loading="lazy">
    ...
    <section class="content-section">
        <h2 class="section-title">⚰️ Funeral Services & Amenities</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-hands-helping me-2"></i> Core Services</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($funeral_service['services'])): ?>
                        ...
                        <?php else: ?>
                            <p class="text-muted mb-0">No services information available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-home me-2"></i> Facilities & Amenities</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-4">
                            <h6 class="fw-bold">🏛️ Chapel & Viewing Rooms</h6>
                            <p class="mb-2 small"><?= !empty($funeral_service['chapel_info']) ? htmlspecialchars($funeral_service['chapel_info']) : '' ?></p>
                        </div>
                        
                        <div class="mb-4">
                            <h6 class="fw-bold">🅿️ Parking & Accessibility</h6>
                            <p class="mb-2 small"><?= !empty($funeral_service['accessibility']) ? htmlspecialchars($funeral_service['accessibility']) : '' ?></p>
                        </div>
                        
                        <div>
                            <h6 class="fw-bold">👥 Capacity</h6>
                            <p class="mb-0 small"><?= !empty($funeral_service['capacity']) ? htmlspecialchars($funeral_service['capacity']) : '' ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Branch Locations</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($funeral_service['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $funeral_service['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
                                        <div class="list-group-item border-0 px-0 py-1">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <span class="small"><?= htmlspecialchars(trim($branch)) ?></span>
                                            </div>
                                        </div>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                            <p class="text-muted small mt-2">Total: <?= $branch_count ?> locations</p>
                        <?php else: ?>
                            <p class="text-muted mb-0">Branch information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-hands-helping me-2"></i> Core Services</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($funeral_service['services'])): ?>
                                <ul class="service-list">
                            ...
                            <?php else: ?>
                                <p class="text-muted mb-0 small">No services information available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-home me-2"></i> Facilities</h6>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <h6 class="fw-bold small">🏛️ Chapel</h6>
                                <p class="mb-2 small"><?= !empty($funeral_service['chapel_info']) ? htmlspecialchars($funeral_service['chapel_info']) : '' ?></p>
                            </div>
                            
                            <div class="mb-3">
                                <h6 class="fw-bold small">🅿️ Parking</h6>
                                <p class="mb-2 small"><?= !empty($funeral_service['accessibility']) ? htmlspecialchars($funeral_service['accessibility']) : '' ?></p>
                            </div>
                            
                            <div>
                                <h6 class="fw-bold small">👥 Capacity</h6>
                                <p class="mb-0 small"><?= !empty($funeral_service['capacity']) ? htmlspecialchars($funeral_service['capacity']) : '' ?></p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-code-branch me-2"></i> Branch Locations</h6>
                        </div>
                        <div class="card-body small">
                            <?php if (!empty($funeral_service['branches'])): ?>
                                ...
                                <p class="text-muted small mt-2">Total: <?= $branch_count ?> locations</p>
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Branch information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-8 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-tags me-2"></i> Package Options</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($funeral_service['packages'])): ?>
                            <div class="row">
                                <?php
                                $packages = explode("\n", $funeral_service['packages']);
                                ...
                                ?>
                                    <div class="col-md-6 mb-3">
                                        <div class="service-feature">
                                            <h6 class="mb-2 small fw-bold"><?= htmlspecialchars(trim($parts[0])) ?></h6>
                                            <p class="mb-0 small text-muted"><?= htmlspecialchars(trim($parts[1])) ?></p>
                                        </div>
                                    </div>
                                <?php
                                        endif;
                        ...
                        <?php else: ?>
                            <p class="text-muted mb-0">Package information not available.</p>
                        <?php endif; ?>
                        
                        <?php if (!empty($funeral_service['price_range'])): ?>
                            <div class="mt-4 p-3 bg-light rounded">
                                <h6 class="mb-2 fw-bold">Price Range:</h6>
                                <p class="price-range mb-0"><?= htmlspecialchars($funeral_service['price_range']) ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="col-lg-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-star me-2"></i> Quick Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <h6 class="fw-bold">Service Type</h6>
                            <p class="mb-2 small"><?= !empty($funeral_service['type']) ? htmlspecialchars($funeral_service['type']) : '' ?></p>
                        </div>
                        
                        <div class="mb-3">
                            <h6 class="fw-bold">Established</h6>
                            <p class="mb-2 small"><?= !empty($funeral_service['established']) ? htmlspecialchars($funeral_service['established']) : '' ?></p>
                        </div>
                        
                        <div class="mb-3">
                            <h6 class="fw-bold">Services Offered</h6>
                            <p class="mb-2 small"><?= !empty($funeral_service['services']) ? count(explode("\n", $funeral_service['services'])) . '' : '' ?></p>
                        </div>
                        
                        <div class="mb-3">
                            <h6 class="fw-bold">Availability</h6>
                            <p class="mb-2 small">24/7 Emergency Service</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-tags me-2"></i> Packages</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($funeral_service['packages'])): ?>
                                <div class="row">
                                    ...
                                    ?>
                                        <div class="col-12 mb-2">
                                            <div class="service-feature">
                                                <h6 class="mb-1 small fw-bold"><?= htmlspecialchars(trim($parts[0])) ?></h6>
                                                <p class="mb-0 small text-muted"><?= htmlspecialchars(trim($parts[1])) ?></p>
                                            </div>
                                        </div>
                                    <?php
                            ...
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Package information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-star me-2"></i> Quick Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="mb-2">
                                <h6 class="fw-bold small">Service Type</h6>
                                <p class="mb-1 small"><?= !empty($funeral_service['type']) ? htmlspecialchars($funeral_service['type']) : '' ?></p>
                            </div>
                            
                            <div class="mb-2">
                                <h6 class="fw-bold small">Established</h6>
                                <p class="mb-1 small"><?= !empty($funeral_service['established']) ? htmlspecialchars($funeral_service['established']) : '' ?></p>
                            </div>
                            
                            <div class="mb-2">
                                <h6 class="fw-bold small">Services</h6>
                                <p class="mb-1 small"><?= !empty($funeral_service['services']) ? count(explode("\n", $funeral_service['services'])) . '' : '' ?></p>
                            </div>
                            
                            <div>
                                <h6 class="fw-bold small">Availability</h6>
                                <p class="mb-0 small">24/7 Emergency</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-7 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Service Details</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-2">📲 Phone: <?= htmlspecialchars($funeral_service['contact'] ?? '') ?></p>
                        <?php if (!empty($funeral_service['email'])): ?>
                            <p class="mb-2">📧 Email: <?= htmlspecialchars($funeral_service['email']) ?> </p>
                        <?php endif; ?>
                        <?php if (!empty($funeral_service['website'])): ?>
                            <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($funeral_service['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?= nl2br(htmlspecialchars($funeral_service['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Service Details</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($funeral_service['contact'] ?? '') ?></p><br>
                            <?php if (!empty($funeral_service['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($funeral_service['email']) ?> </p>
                            <?php endif; ?><br>
                            <?php if (!empty($funeral_service['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($funeral_service['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($funeral_service['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($funeral_service['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Important locations near <?= htmlspecialchars($funeral_service['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($funeral_service['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($funeral_service['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($funeral_service['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($funeral_service['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <h5 class="mb-2 small fw-bold">📞 Need Immediate Assistance?</h5>
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($funeral_service['contact'])): ?>
                                    <a href="tel:<?= htmlspecialchars(preg_replace('/[^0-9+]/', '', $funeral_service['contact'])) ?>" class="btn btn-outline-primary btn-sm">
                                        <i class="fas fa-phone me-1"></i> Call Office
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($funeral_service['hotline'])): ?>
                                    <a href="tel:<?= htmlspecialchars(preg_replace('/[^0-9+]/', '', $funeral_service['hotline'])) ?>" class="btn btn-danger btn-sm">
                                        <i class="fas fa-ambulance me-1"></i> Emergency
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($funeral_service['email'])): ?>
                                    <a href="mailto:<?= htmlspecialchars($funeral_service['email']) ?>" class="btn btn-outline-secondary btn-sm">
                            ...
                            <div class="mt-3 p-2 bg-light rounded">
                                <h6 class="mb-1 small fw-bold">ℹ️ Important Notice</h6>
                                <p class="mb-0 small">We understand this is a difficult time. Our compassionate staff is available 24/7 to assist you with arrangements and answer any questions you may have.</p>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
        ...
        <div class="row row-cols-3 g-3 text-center text-sm-start">
            ...
            <div class="col">
                ...
                <ul class="list-unstyled mb-0 small">
                    ...
                    <li class="mb-1"><i class="fas fa-envelope me-2"></i>info@iloilocity.gov.ph</li>
                </ul>
            </div>
        </div>
