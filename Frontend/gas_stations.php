...
$result = $conn->query("SELECT * FROM gas_stations");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'gas_stations') {
...
$gas_stations_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $gas_stations_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div id="mySidebar" class="sidebar">
  ...
  <div class="mt-auto p-4 text-center">
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Gas Stations in Iloilo City</h1> 

        <div class="card-container">
            <?php if ($gas_stations_exist): ?>
                <?php while ($gas_station = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($gas_station['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($gas_station['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($gas_station['name']) ?></h6>
                            <?php if (!empty($gas_station['details_link'])): ?>
                                <a href="<?= htmlspecialchars($gas_station['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No gas station found.</div>
            <?php endif; ?>
        </div>
