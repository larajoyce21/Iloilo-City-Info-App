...
$result = $conn->query("SELECT * FROM highschools");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'highschools') {
...
$highschools_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $highschools_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="highschools">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">High Schools in Iloilo City</h1> 
        
        
        <div class="card-container">
            <?php if ($highschools_exist): ?>
                <?php while ($highschool = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($highschool['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($highschool['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($highschool['name']) ?></h6>
                            <?php if (!empty($highschool['details_link'])): ?>
                                <a href="<?= htmlspecialchars($highschool['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
