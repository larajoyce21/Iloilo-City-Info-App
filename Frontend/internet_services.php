...
$result = $conn->query("SELECT * FROM internet_services");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'internet_services') {
...
$query = "SELECT * FROM internet_services";
$internet_services_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $internet_services_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="resorts">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Internet Service Providers in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($internet_services_exist): ?>
                <?php while ($internet_service = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($internet_service['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($internet_service['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($internet_service['name']) ?></h6>
                            <?php if (!empty($internet_service['details_link'])): ?>
                                <a href="<?= htmlspecialchars($internet_service['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No internet service found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
