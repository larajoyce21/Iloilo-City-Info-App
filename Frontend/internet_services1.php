...
$query = "SELECT * FROM internet_services WHERE id = 1";
...
if (!empty($service['transportation_routes'])) {
    $routes_data = json_decode($service['transportation_routes'], true);
...
$coverage_areas = [];
if (!empty($service['coverage_areas'])) {
    $coverage_areas = json_decode($service['coverage_areas'], true);
    if (!is_array($coverage_areas)) {
        $coverage_areas = [];
    }
}
...
if (!empty($service['image_detail'])) {
    $detail_images = explode(',', $service['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $service['social_media'] ?? '')));

$hours = [];
if (!empty($service['hours'])) {
    $lines = explode("\n", $service['hours']);
...
$branches = [];
if (!empty($service['branches'])) {
    $branches = array_filter(array_map('trim', explode("\n", $service['branches'])));
}
$nearby_places = [];
if (!empty($service['nearby_places'])) {
    $decoded = json_decode($service['nearby_places'], true);
...
<head>
    ...
    <title><?= htmlspecialchars($service['name'] ?? 'Internet Service Provider') ?> - Iloilo Internet Services</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($service['image'] ?? '') ?>');
...
<br>
    <section class="hero-section" 
        style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($service['image'] ?? '') ?>') center/cover no-repeat;">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($service['name'] ?? '') ?></h1>
            <p class="lead"><?= htmlspecialchars($service['location'] ?? '') ?></p>
        </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($service['name'] ?? '') ?></h2>
            <div class="lead mb-4">
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($service['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            <h2 class="section-title">🌐 Internet Service Information</h2>
            ...
            <div class="row d-none d-md-flex">
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-2">
                                ...
                                <p class="mb-2"><?= htmlspecialchars($service['technology'] ?? 'Not specified') ?></p>
                            </div>
                            <div class="mb-2">
                                <h6 class="fw-bold">⚡ Speed</h6>
                                <p class="mb-0">
                                    <span class="speed-badge">Download: <?= htmlspecialchars($service['download_speed'] ?? 'N/A') ?></span>
                                    <span class="speed-badge">Upload: <?= htmlspecialchars($service['upload_speed'] ?? 'N/A') ?></span>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-5 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                      <?= nl2br(htmlspecialchars($service['hours'])) ?>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                                <div class="mb-2">
                                    <h6 class="fw-bold small">📶 Technology</h6>
                                    <p class="mb-2 small"><?= htmlspecialchars($service['technology'] ?? 'Not specified') ?></p>
                                </div>
                                <div class="mb-2">
                                    <h6 class="fw-bold small">⚡ Speed</h6>
                                    <p class="mb-0 small">
                                        <span class="speed-badge">Download: <?= htmlspecialchars($service['download_speed'] ?? 'N/A') ?></span>
                                        <span class="speed-badge">Upload: <?= htmlspecialchars($service['upload_speed'] ?? 'N/A') ?></span>
                                    </p>
                            ...
                            <div class="row">
                                <?php if (!empty($service['price_range'])): ?>
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <p class="mb-2"><?= nl2br(htmlspecialchars($service['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <p class="mb-2">📲 Phone: <?= htmlspecialchars($service['contact'] ?? '') ?></p>
                                <?php if (!empty($service['email'])): ?>
                                    <p class="mb-2">📧 Email: <?= htmlspecialchars($service['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($service['website'])): ?>
                                    <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($service['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    <?php if (!empty($service['price_range'])): ?>
                                        <div class="col-12 mb-2">
                                            ...
                                            <p class="mb-2 small"><?= nl2br(htmlspecialchars($service['price_range'])) ?></p>
                                        </div>
                                    <?php endif; ?>
                                    
                                    <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($service['contact'] ?? '') ?></p>
                                    <?php if (!empty($service['email'])): ?>
                                        <p class="mb-2 small">📧 Email: <?= htmlspecialchars($service['email']) ?> </p>
                                    <?php endif; ?>
                                    <?php if (!empty($service['website'])): ?>
                                        <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($service['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($service['year_established'] ?? '') ?></p>
                                    <p class="mb-2"><strong>⭐ Service Type:</strong> <?= htmlspecialchars($service['service_type'] ?? '') ?></p>
                                </div>
                                <div class="col-md-6">
                                    <p class="mb-2"><strong>💵 Price Range:</strong> <?= htmlspecialchars($service['price_range'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($service['year_established'] ?? '') ?></p>
                                        <p class="mb-1 small"><strong>⭐ Service Type:</strong> <?= htmlspecialchars($service['service_type'] ?? '') ?></p>
                                        <p class="mb-1 small"><strong>💵 Price Range:</strong> <?= htmlspecialchars($service['price_range'] ?? '') ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($service['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($service['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($service['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($service['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($service['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($service['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($service['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                    ...
                    <?php if (!empty($social_links)): ?>
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($service['mobile_app'])): ?>
                                    <hr class="my-3">
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($service['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
