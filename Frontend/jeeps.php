...
$result = $conn->query("SELECT * FROM jeeps");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'jeeps') {
...
<div class="album py-5" id="jeeps">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Jeepney Routes in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($jeeps_exist): ?>
                <?php while ($jeep = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($jeep['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($jeep['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($jeep['name']) ?></h6>
                            <?php if (!empty($jeep['details_link'])): ?>
                                <a href="<?= htmlspecialchars($jeep['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No jeep  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
