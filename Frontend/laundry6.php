...
$query = "SELECT * FROM laundry_services WHERE id = 6";
...
if (!empty($laundry['transportation_routes'])) {
    $routes_data = json_decode($laundry['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($laundry['nearby_places'])) {
    $decoded = json_decode($laundry['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $laundry['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $laundry['social_media'] ?? '')));

$hours = [];
if (!empty($laundry['hours'])) {
    $hours = explode("\n", $laundry['hours']);
}

$services = !empty($laundry['services'])
    ? array_filter(array_map('trim', explode("\n", $laundry['services'])))
    : [];

$special_services = !empty($laundry['special_services'])
    ? array_filter(array_map('trim', explode("\n", $laundry['special_services'])))
    : [];

$pricing_lines = !empty($laundry['pricing'])
    ? array_filter(array_map('trim', explode("\n", $laundry['pricing'])))
    : [];
...
<head>
    ...
    <title><?= htmlspecialchars($laundry['name']) ?> - Iloilo Laundry Services</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($laundry['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($laundry['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($laundry['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($laundry['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($laundry['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($laundry['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($laundry['name']) ?> Image <?= $index + 1 ?>">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🛎️ Services Offered</h5>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <p class="mb-0">Pricing information coming soon.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($pricing_lines)): ?>
                                <div class="pricing-list">
                                    <?php foreach ($pricing_lines as $line): ?>
                                        <div class="service-item">
                                            <span class="small"><?= htmlspecialchars($line) ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 small">Pricing information coming soon.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($laundry['contact'] ?? '') ?></p>
                            <?php if (!empty($laundry['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($laundry['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($laundry['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($laundry['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($laundry['pickup_delivery'])): ?>
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold">🚚 Pickup & Delivery</h6>
                                    <p class="mb-2"><?= htmlspecialchars($laundry['pickup_delivery']) ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($laundry['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Business Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($laundry['contact'] ?? '') ?></p>
                            <?php if (!empty($laundry['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($laundry['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($laundry['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($laundry['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                                
                                <?php if (!empty($laundry['pickup_delivery'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🚚 Pickup & Delivery</h6>
                                        <p class="mb-2 small"><?= htmlspecialchars($laundry['pickup_delivery']) ?></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($laundry['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $laundry['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($laundry['established'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>💳 Payment Options:</strong> <?= htmlspecialchars($laundry['payment_options'] ?? '') ?></p>
                            </div>
                        </div>
                        <hr>
                        <p class="mb-1"><strong>📍 Service Area:</strong> <?= htmlspecialchars($laundry['service_area'] ?? '') ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Branches</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($laundry['branches'])): ?>
                                <div class="list-group list-group-flush">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Company Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($laundry['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💳 Payment:</strong> <?= htmlspecialchars($laundry['payment_options'] ?? '') ?></p>
                                    <p class="mb-1"><strong>📍 Service Area:</strong> <?= htmlspecialchars($laundry['service_area'] ?? '') ?></p>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($laundry['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?php echo htmlspecialchars(convertToEmbedURL($laundry['maps_embed'])); ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($laundry['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($laundry['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($laundry['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($laundry['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($laundry['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($laundry['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($laundry['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
