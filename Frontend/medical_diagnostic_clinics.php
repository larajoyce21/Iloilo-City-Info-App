...
$result = $conn->query("SELECT * FROM medical_diagnostic_clinics");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'medical_diagnostic_clinics') {
...
$query = "SELECT * FROM medical_diagnostic_clinics";
$medical_diagnostic_clinics_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $medical_diagnostic_clinics_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>MEDICAL DIAGNOSTIC CLINICS</title>
...
<div class="album py-5" id="resorts">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Medical Diagnostic Clinics in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($medical_diagnostic_clinics_exist): ?>
                <?php while ($medical_diagnostic_clinic = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($medical_diagnostic_clinic['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($medical_diagnostic_clinic['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($medical_diagnostic_clinic['name']) ?></h6>
                            <?php if (!empty($medical_diagnostic_clinic['details_link'])): ?>
                                <a href="<?= htmlspecialchars($medical_diagnostic_clinic['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No medical diagnostic clinic found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
