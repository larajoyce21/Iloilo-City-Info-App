...
$query = "SELECT * FROM motorcycle_shops WHERE id = 20";
...
if (!empty($shop['transportation_routes'])) {
    $routes_data = json_decode($shop['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($shop['nearby_places'])) {
    $nearby_places = json_decode($shop['nearby_places'], true);
...
if (!empty($shop['image_detail'])) {
    $detail_images = explode(',', $shop['image_detail']);
}
...
if (!empty($shop['services'])) {
    $services = array_filter(array_map('trim', explode(',', $shop['services'])));
}

$brands = [];
if (!empty($shop['brands'])) {
    $brands = array_filter(array_map('trim', explode(',', $shop['brands'])));
}
...
$locations = array_filter(array_map('trim', explode('|', $shop['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($shop['name']) ?> - Iloilo Motorcycle Shops</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($shop['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($shop['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($shop['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($shop['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($shop['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($shop['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🏍️ Motorcycle Shop Information</h2>
        ...
        <div class="row d-none d-md-flex">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($services)): ?>
                            <h6 class="mt-3 fw-bold">🔧 Services Offered:</h6>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($shop['hours'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($shop['contact'] ?? '') ?></p>
                            <?php if (!empty($shop['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($shop['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($shop['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($shop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        <?php if (!empty($brands)): ?>
                            <h6 class="mt-3 fw-bold">🏍️ Brands Available:</h6>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($brands as $brand): ?>
                                    <span class="badge badge-custom brand-badge mb-2"><?= htmlspecialchars($brand) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($services)): ?>
                                <h6 class="mt-3 fw-bold small">🔧 Services:</h6>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($services as $service): ?>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">📞 Contact & Brands</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($shop['contact'] ?? '') ?></p>
                            <?php if (!empty($shop['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($shop['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($shop['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($shop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            <?php if (!empty($brands)): ?>
                                <h6 class="mt-3 fw-bold small">🏍️ Brands:</h6>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($brands as $brand): ?>
                                        <span class="badge badge-custom brand-badge mb-2 small"><?= htmlspecialchars($brand) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                        </div>
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        <?php if (!empty($shop['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $shop['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($shop['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($shop['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($shop['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($shop['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($shop['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($shop['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($shop['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($shop['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($shop['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
