...
$query = "SELECT * FROM pharmacies WHERE id = 23";
...
if (!empty($pharmacy['transportation_routes'])) {
    $routes_data = json_decode($pharmacy['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($pharmacy['nearby_places'])) {
    $decoded = json_decode($pharmacy['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $pharmacy['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $pharmacy['social_media'] ?? '')));

$hours = [];
if (!empty($pharmacy['hours'])) {
    $hours = explode("\n", $pharmacy['hours']);
}


$locations = array_filter(array_map('trim', explode('|', $pharmacy['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($pharmacy['name']) ?> - Iloilo Pharmacies</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($pharmacy['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($pharmacy['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($pharmacy['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($pharmacy['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($pharmacy['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($pharmacy['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($pharmacy['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏷️ Special Features</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-2">
                            <strong>🕛 24/7 Service:</strong> <?= htmlspecialchars($pharmacy['is_24_7'] ?? 'No') ?>
                        </div>
                        <div class="mb-2">
                            <strong>🚚 Delivery:</strong> <?= htmlspecialchars($pharmacy['has_delivery'] ?? 'No') ?>
                        </div>
                        <div>
                            <strong>🏥 Type:</strong> <?= htmlspecialchars($pharmacy['type'] ?? '') ?>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($pharmacy['contact'] ?? '') ?></p>
                            <?php if (!empty($pharmacy['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($pharmacy['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pharmacy['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($pharmacy['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body ">
                             <?= nl2br(htmlspecialchars($pharmacy['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($pharmacy['contact'] ?? '') ?></p>
                            <?php if (!empty($pharmacy['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($pharmacy['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pharmacy['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($pharmacy['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Pharmacy Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🏢 Type:</strong> <?= htmlspecialchars($pharmacy['type'] ?? '') ?></p>
                                <p class="mb-2"><strong>🕛 24/7:</strong> <?= htmlspecialchars($pharmacy['is_24_7'] ?? 'No') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🚚 Delivery:</strong> <?= htmlspecialchars($pharmacy['has_delivery'] ?? 'No') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Pharmacy Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🏢 Type:</strong> <?= htmlspecialchars($pharmacy['type'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🕛 24/7:</strong> <?= htmlspecialchars($pharmacy['is_24_7'] ?? 'No') ?></p>
                                    <p class="mb-1 small"><strong>🚚 Delivery:</strong> <?= htmlspecialchars($pharmacy['has_delivery'] ?? 'No') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($pharmacy['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($pharmacy['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($pharmacy['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($pharmacy['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($pharmacy['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($pharmacy['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($pharmacy['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($pharmacy['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($pharmacy['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
