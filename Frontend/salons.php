...
$result = $conn->query("SELECT * FROM salons");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'salons') {
...
if ($stmt = $conn->prepare($query)) {
    ...
    $salons_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="salons">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Beauty Salons in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($salons_exist): ?>
                <?php while ($salon = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($salon['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($salon['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($salon['name']) ?></h6>
                            <?php if (!empty($salon['details_link'])): ?>
                                <a href="<?= htmlspecialchars($salon['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No salon  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
