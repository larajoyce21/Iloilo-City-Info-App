...
$result = $conn->query("SELECT * FROM study_hubs");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'study_hubs') {
...
$query = "SELECT *FROM study_hubs";
$study_hubs_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $study_hubs_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>STUDY HUBS AND CO-WORKING SPACE</title>
...
<div class="album py-5" id="study_hubs">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Study Hubs and Co-working Space in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($study_hubs_exist): ?>
                <?php while ($study_hub = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($study_hub['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($study_hub['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($study_hub['name']) ?></h6>
                            <?php if (!empty($study_hub['details_link'])): ?>
                                <a href="<?= htmlspecialchars($study_hub['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No study hub  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
