...
$result = $conn->query("SELECT * FROM wellness_spas");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'wellness_spas') {
...
$query = "SELECT * FROM wellness_spas";
$wellness_spas_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $wellness_spas_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Wellness Spa in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($wellness_spas_exist): ?>
                <?php while ($wellness_spa = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($wellness_spa['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($wellness_spa['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($wellness_spa['name']) ?></h6>
                            <?php if (!empty($wellness_spa['details_link'])): ?>
                                <a href="<?= htmlspecialchars($wellness_spa['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No wellness spa found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
