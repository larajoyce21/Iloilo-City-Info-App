...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conn, "DELETE FROM jeeps WHERE id=$delete_id");
    $_SESSION['message'] = "Jeepney deleted successfully!";
    header("Location: add_jeeps.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 0; $i < $operator_count; $i++) {
        if (!empty($operator_names[$i])) {
            ...
            $operating_hours_operator = $hours_start . ' - ' . $hours_end;
            $social_input = trim($operator_social_media[$i] ?? '');
            $social_links = $social_input ? explode(' ', $social_input) : [];
            $line = "$name_clean|$operating_hours_operator|$contact|$website|$address";
            ...
            $jeep_operators_lines[] = $line;
        }
    }
    
    $jeep_operators = implode("\n", $jeep_operators_lines);
    ...
    if ($id > 0) {
        $query = "UPDATE jeeps SET 
                 ...
                 off_hours='$off_hours',
                 jeep_operators='$jeep_operators',
                 details_link='$details_link' 
                 WHERE id=$id";
    } else {
        $query = "INSERT INTO jeeps 
                 (name, image, image_detail, routes, location, maps_embed, maps_link, peak_hours, off_hours, jeep_operators, details_link) 
                 VALUES 
                 ('$name', '$image', '$image_detail', '$routes', '$location', '$maps_embed', '$maps_link', '$peak_hours', '$off_hours', '$jeep_operators', '$details_link')";
    }
    
    if (mysqli_query($conn, $query)) {
        $_SESSION['message'] = $id > 0 ? "Jeepney updated successfully!" : "Jeepney added successfully!";
    } else {
...
$jeeps = mysqli_query($conn, "SELECT * FROM jeeps ORDER BY id");
...
if (isset($_GET['edit'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM jeeps WHERE id=$edit_id");
    if (mysqli_num_rows($result) > 0) {
        $jeepData = mysqli_fetch_assoc($result);
        
        if (!empty($jeepData['peak_hours'])) {
            $peak_parts = explode(' - ', $jeepData['peak_hours']);
        ...
        if (!empty($jeepData['off_hours'])) {
            $off_parts = explode(' - ', $jeepData['off_hours']);
        ...
        if (!empty($jeepData['jeep_operators'])) {
            $lines = explode("\n", $jeepData['jeep_operators']);
...
foreach ($jeepData as $key => $value) {
    $jeepData[$key] = htmlspecialchars($value);
}
?>
...
<head>
    ...
    <title>Jeepney Management</title>
...
<body>
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1><i class="fas fa-shuttle-van me-2"></i>Jeepney Management</h1>
                <p class="text-muted">Add and manage jeepney routes</p>
            </div>
        ...
        <div class="text-end mb-4">
            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#jeepModal">
                <i class="fas fa-plus me-2"></i>Add New Jeepney
            </button>
        </div>
        
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="fas fa-list me-2"></i>Jeepney Routes</h5>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php while ($jeep = mysqli_fetch_assoc($jeeps)): ?>
                            <tr>
                                <td><?php echo $jeep['id']; ?></td>
                                <td><?php echo $jeep['name']; ?></td>
                                <td><?php echo $jeep['location']; ?></td>
                                <td>
                                    <a href="?edit=<?php echo $jeep['id']; ?>" class="btn btn-sm btn-outline-primary">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="?delete=<?php echo $jeep['id']; ?>" class="btn btn-sm btn-outline-danger" 
                                       onclick="return confirm('Delete this jeepney?')">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                    <a href="jeep1.php?id=<?php echo $jeep['id']; ?>" target="_blank" class="btn btn-sm btn-outline-success">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        ...
                        <?php echo isset($_GET['edit']) ? 'Edit Jeepney' : 'Add New Jeepney'; ?>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="clearEditMode()"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" enctype="multipart/form-data" id="jeepForm">
                        <input type="hidden" name="id" value="<?php echo $jeepData['id']; ?>">
                        <input type="hidden" name="existing_image" value="<?php echo $jeepData['image']; ?>">
                        <input type="hidden" name="existing_image_detail" value="<?php echo $jeepData['image_detail']; ?>">
                        <input type="hidden" name="peak_hours" id="peak_hours_input" value="<?php echo $jeepData['peak_hours']; ?>">
                        <input type="hidden" name="off_hours" id="off_hours_input" value="<?php echo $jeepData['off_hours']; ?>">
                        
                        <div class="mb-3">
                            <label class="form-label">Jeepney Name *</label>
                            <input type="text" class="form-control" name="name" value="<?php echo $jeepData['name']; ?>">
                        </div>
                        
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label">Location *</label>
                                <input type="text" class="form-control" name="location" value="<?php echo $jeepData['location']; ?>">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Main Image</label>
                                <input type="file" class="form-control" name="image" accept="image/*">
                                <?php if ($jeepData['image']): ?>
                                <small class="text-muted">Current: <?php echo basename($jeepData['image']); ?></small>
                                <br>
                                <img src="<?php echo $jeepData['image']; ?>" class="preview-image mt-2" alt="Current Image">
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="details_link" 
                                       value="<?php echo $jeepData['details_link']; ?>"
                                       placeholder="e.g., jeep1.php?id=1">
                            </div>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label">Routes (one per line) *</label>
                            <textarea class="form-control" name="routes" rows="4"><?php echo $jeepData['routes']; ?></textarea>
                            <small class="text-muted">Example: Terminal 1 → Molo Plaza → City Proper</small>
                        </div>
                        
                        <div class="mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5>Jeepney Operators</h5>
                                ...
                                <div class="operator-form" id="operatorForm<?php echo $index; ?>">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label class="form-label">Company/Operator Name *</label>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control" name="maps_embed" rows="3"><?php echo $jeepData['maps_embed']; ?></textarea>
                                <small class="text-muted">Paste the iframe src URL</small>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Google Maps Link</label>
                                <input type="text" class="form-control" name="maps_link" value="<?php echo $jeepData['maps_link']; ?>">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <?php if ($jeepData['image_detail']): ?>
                            <div class="mt-2">
                                <small class="text-muted">Current images:</small>
                                <?php 
                                $images = explode(',', $jeepData['image_detail']);
                        ...
                        <div class="text-end">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Save Jeepney
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var modal = new bootstrap.Modal(document.getElementById('jeepModal'));
            modal.show();
        });
        <?php endif; ?>
        ...
        document.getElementById('jeepForm').addEventListener('submit', function(e) {
