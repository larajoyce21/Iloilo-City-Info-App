...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("SELECT image, image_detail FROM review_centers WHERE id=?");
    $stmt->bind_param("i", $delete_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $center = $result->fetch_assoc();
    
    $stmt = $conn->prepare("DELETE FROM review_centers WHERE id=?");
    $stmt->bind_param("i", $delete_id);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['message'] = "Review Center deleted successfully";
    } else {
        $_SESSION['error'] = "Error deleting review center: " . $conn->error;
    }
    
    header("Location: add_review_centers.php");
    exit();
}
...
if (isset($_GET['delete_image']) && isset($_GET['center_id'])) {
    ...
    if ($is_card_image) {
        $stmt = $conn->prepare("UPDATE review_centers SET image='' WHERE id=?");
        $stmt->bind_param("i", $center_id);
        $stmt->execute();
    } else {
        $stmt = $conn->prepare("SELECT image_detail FROM review_centers WHERE id=?");
        $stmt->bind_param("i", $center_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $center = $result->fetch_assoc();
        ...
        if ($center) {
            ...
            $stmt = $conn->prepare("UPDATE review_centers SET image_detail=? WHERE id=?");
    ...
    header("Location: add_review_centers.php?edit=" . $center_id);
    exit();
}
...
$query = "SELECT id, name, image, description, location, maps_embed, directions_link, maps_link, website, hours, contact, nearby_places, social_media, details_link, courses, email FROM review_centers";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $courses_json = json_encode(array_filter($courses_array));
    ...
    // Update 
    if ($id) {
        $query = "UPDATE review_centers SET name=?, image=?, image_detail=?, description=?, location=?, maps_embed=?, maps_link=?, website=?, directions_link=?, hours=?, contact=?, nearby_places=?, social_media=?, details_link=?, courses=?, email=?, transportation_routes=? WHERE id=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssssssssssssssi", $name, $target_file, $target_file_detail, $description, $location, $mapEmbedURL, $maps_link, $website, $directions_link, $hours, $contact, $nearby_places_json, $social_media, $details_link, $courses_json, $email, $transportation_routes_json, $id);
    } else {
        $query = "INSERT INTO review_centers (name, image, image_detail, description, location, maps_embed, maps_link, website, directions_link, hours, contact, nearby_places, social_media, details_link, courses, email, transportation_routes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssssssssssssss", $name, $target_file, $target_file_detail, $description, $location, $mapEmbedURL, $maps_link, $website, $directions_link, $hours, $contact, $nearby_places_json, $social_media, $details_link, $courses_json, $email, $transportation_routes_json);
    }
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Review Center " . ($id ? "updated" : "added") . " successfully";
    } else {
        $_SESSION['error'] = "Error saving review center: " . $conn->error;
    }
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT nearby_places, courses, transportation_routes FROM review_centers WHERE id=?");
    ...
    if ($result->num_rows > 0) {
        ...
        if (!empty($row['courses'])) {
            $decoded_courses = json_decode($row['courses'], true);
            if (is_array($decoded_courses)) {
                $courses_str = implode(', ', $decoded_courses);
            }
        }
...
<head>
    ...
    <title>Review Centers Management</title>
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="text-light">Review Centers Management</h1>
            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#centerModal">
                <i class="bi bi-plus-lg"></i> Add Review Center
            </button>
        </div>
        
        <?php if ($centers->num_rows == 0): ?>
            <div class="alert alert-info">
                No review centers found. Click "Add Review Center" to create one.
            </div>
        <?php else: ?>
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-between">
                                    <a href="add_review_centers.php?edit=<?php echo $center['id']; ?>" class="btn btn-primary btn-sm">
                                        <i class="bi bi-pencil"></i> Edit
                                    </a>
                                    <a href="?delete=<?php echo $center['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this review center?')">
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="centerModalLabel"><?php echo isset($_GET['edit']) ? 'Edit Review Center' : 'Add Review Center'; ?></h5>
                ...
                <div class="modal-body">
                    <?php 
                        ...
                        if (isset($_GET['edit'])) {
                            ...
                            $stmt = $conn->prepare("SELECT * FROM review_centers WHERE id=?");
                            $stmt->bind_param("i", $edit_id);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            if ($result->num_rows > 0) {
                    ...
                    ?>
                    <form method="POST" action="add_review_centers.php" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" name="hours" rows="5" placeholder="e.g., Monday: 9AM - 5PM"><?php echo htmlspecialchars($centerData['hours']); ?></textarea>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Contact Information</label>
                                        <input type="text" class="form-control" name="contact" value="<?php echo htmlspecialchars($centerData['contact']); ?>" placeholder="Phone, email, social media">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Email Address</label>
                                        <input type="email" class="form-control" name="email" value="<?php echo $centerData['email']; ?>" placeholder="Email address">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="mb-3">
                                ...
                                <input name="courses" class="form-control" value="<?= htmlspecialchars($courses_str); ?>" placeholder="Add courses separated by commas">
                                <small class="text-muted">Example: Civil Engineering, Nursing, Education</small>
                            </div>
                        </div>
                                            ...
                                            <tr>
                                                <td>
                                                    <select class="form-control" name="route_type[]">
                                                        ...
                                                        <option value="fx" <?php echo (isset($transportation_routes[$i]['type']) && $transportation_routes[$i]['type'] == 'fx') ? 'selected' : ''; ?>>FX/UV Express</option>
                                                    </select>
                                                </td>
                                                <td>
                        ...
                        <div class="form-section">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3"> 
                                    ...
                                    <small class="text-muted">Example: Library, Coffee Shop, Park</small>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Social Media Links</label>
                                    <textarea class="form-control" name="social_media" rows="5" placeholder="Enter one URL per line, e.g.https://facebook.com/center"><?php echo isset($centerData['social_media']) ? htmlspecialchars($centerData['social_media']) : ''; ?></textarea>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <?php echo isset($_GET['edit']) ? 'Update Review Center' : 'Save Review Center'; ?>
                            </button>
                        </div>
                    </form>
                </div>
            ...
            newRow.innerHTML = `
                <td>
                    <select class="form-control" name="route_type[]">
                        ...
                        <option value="fx">FX/UV Express</option>
                    </select>
                </td>
                <td>
