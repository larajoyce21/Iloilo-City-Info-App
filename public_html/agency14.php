...
$searchData = isset($_SESSION['search_trigger']) ? $_SESSION['search_trigger'] : null;
if (isset($_SESSION['search_trigger'])) {
    unset($_SESSION['search_trigger']);
}
$agency_id = isset($_GET['id']) ? intval($_GET['id']) : 14;
if ($agency_id <= 0) {
    $agency_id = 1;
}
...
$stmt->bind_param("i", $agency_id);
$stmt->execute();
$agency_documents_result = $stmt->get_result();
while ($row = $agency_documents_result->fetch_assoc()) {
    $agency_documents[] = $row;
}

$nearby_places = [];
if (!empty($agency['nearby_places'])) {
    $decoded = json_decode($agency['nearby_places'], true);
    $nearby_places = (json_last_error() === JSON_ERROR_NONE && is_array($decoded))
        ? $decoded
        : array_filter(array_map('trim', explode(',', $agency['nearby_places'])));
}

$detail_images = !empty($agency['image_detail']) ? array_filter(explode(',', $agency['image_detail'])) : [];

$transport_routes = array();
if (!empty($agency['transportation_routes'])) {
    $routes_data = json_decode($agency['transportation_routes'], true);
...
$social_links = !empty($agency['social_media']) ? array_filter(array_map('trim', explode("\n", $agency['social_media']))) : [];

$locations = !empty($agency['location']) ? array_filter(array_map('trim', explode('|', $agency['location']))) : [];

$agency_name = $agency['name'] ?? 'Agency Not Found';
$agency_location = $agency['location'] ?? 'Location not specified';
$agency_description = nl2br($agency['description'] ?? 'No description available.');
...
<head>
    ...
    <title><?= $agency_name ?> - Iloilo Agencies</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= $agency_image ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= $agency_image ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= $agency_name ?></h1>
    <p class="lead"><?= $agency_location ?></p>
  </div>
</section>
  ...
  <main class="container mb-4">
    <section class="content-section">
        <h4>About</h4>
      <h2 class="section-title"> <?= $agency_name ?></h2>
      <div class="lead mb-4">
        <?= $agency_description ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= $image ?>" alt="<?= $agency_name ?> Image <?= $index + 1 ?>">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($agency['services'])): ?>
                            <ul class="mb-0">
                                <?php foreach (explode("\n", $agency['services']) as $service): ?>
                                    <?php if (trim($service) != ''): ?>
                                        <li>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($agency['eligibility'])): ?>
                            <ul class="mb-0">
                                <?php foreach (explode("\n", $agency['eligibility']) as $requirement): ?>
                        ...
                        <?php else: ?>
                            <p class="mb-0">No eligibility requirements listed.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($agency['required_docs'])): ?>
                            <p class="mb-0"><?= nl2br($agency['required_docs']) ?></p>
                        <?php else: ?>
                            <p class="mb-0">No document requirements listed.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <p class="mb-0 small">No eligibility requirements listed.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <?php if (!empty($agency['required_docs'])): ?>
                                <p class="mb-0 small"><?= nl2br($agency['required_docs']) ?></p>
                            <?php else: ?>
                                <p class="mb-0 small">No document requirements listed.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-12">
                                ...
                                <p class="mb-2">📲 Phone: <?= $agency['contact'] ?? 'Not available' ?></p>
                                <?php if (!empty($agency['email'])): ?>
                                    <p class="mb-2">📧 Email: <?= $agency['email'] ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($agency['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= $agency['website'] ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-12">
                                    ...
                                    <p class="mb-2 small">📲 Phone: <?= $agency['contact'] ?? 'Not available' ?></p>
                                    <?php if (!empty($agency['email'])): ?>
                                        <p class="mb-2 small">📧 Email: <?= $agency['email'] ?> </p>
                                    <?php endif; ?>
                                    <?php if (!empty($agency['website'])): ?>
                                        <p class="mb-2 small">🌐 Website: <a href="<?= $agency['website'] ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                    <?php endif; ?>
    ...
    <section class="content-section">
        <h2 class="section-title">🗂️ Documents</h2>
        
        <?php if (!empty($agency_documents)): ?>
            <div class="card info-card">
                <div class="card-header">
                    <h5 class="mb-0">⬇️ Available Downloads</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <?php foreach ($agency_documents as $doc): ?>
                            <div class="col-md-6 mb-3">
                                <div class="d-flex align-items-center">
                                    ...
                                    $icon = $ext == 'pdf' ? 'file-pdf' : ($ext == 'doc' || $ext == 'docx' ? 'file-word' : 'file-alt');
                                    ?>
                                    <i class="fas fa-<?= $icon ?> fa-2x me-3 text-primary"></i>
                                    <div>
                                        <h6 class="small"><?= $doc['document_name'] ?></h6>
                                        <div class="btn-group btn-group-sm">
                                            <a href="agencies.php?view_document=<?= $doc['id'] ?>" class="btn btn-outline-primary btn-sm" target="_blank"><i class="fas fa-eye me-1"></i> View</a>
                                            <a href="<?= $doc['file_path'] ?>" class="btn btn-primary btn-sm" download><i class="fas fa-download me-1"></i> Download</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <?php else: ?>
            <div class="alert alert-info">No documents available for download.</div>
        <?php endif; ?>
    </section>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($agency['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= $agency['maps_embed'] ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= $agency_name ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($agency['maps_link'])): ?>
                                <a href="<?= $agency['maps_link'] ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($agency['directions_link'])): ?>
                                <a href="<?= $agency['directions_link'] ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($agency['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= $agency['mobile_app'] ?>" class="btn btn-dark btn-sm" target="_blank">
