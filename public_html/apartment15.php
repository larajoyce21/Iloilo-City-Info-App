...
$query = "SELECT * FROM apartments WHERE id = 15";
...
if (!empty($apartment['transportation_routes'])) {
    $routes_data = json_decode($apartment['transportation_routes'], true);
...
if (!empty($apartment['amenities'])) {
    $amenities = json_decode($apartment['amenities'], true);
...
$nearby_places = [];
if (!empty($apartment['nearby_places'])) {
    $nearby_places = json_decode($apartment['nearby_places'], true);
...
if (!empty($apartment['image_detail'])) {
    $detail_images = explode(',', $apartment['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $apartment['social_media'] ?? '')));

$hours = [];
if (!empty($apartment['hours'])) {
    $lines = explode("\n", $apartment['hours']);
...
<head>
    ...
    <title><?php echo htmlspecialchars($apartment['name']); ?></title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($apartment['image'] ?? '') ?>');
...
<br>
    <section class="hero-section" 
        style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($apartment['image'] ?? '') ?>') center/cover no-repeat;">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($apartment['name'] ?? '') ?></h1>
            <p class="lead"><?= htmlspecialchars($apartment['location'] ?? '') ?></p>
            <?php if (!empty($apartment['price_range'])): ?>
                <div class="price-tag mt-2"><?= htmlspecialchars($apartment['price_range']) ?></div>
            <?php endif; ?>
        </div>
    </section>
    ...
    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($apartment['name'] ?? '') ?></h2>
            <div class="lead mb-4">
                <?= nl2br(htmlspecialchars($apartment['description'] ?? '')) ?>
            </div>
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($apartment['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            <h2 class="section-title">🏢 Apartment Information</h2>
            ...
            <div class="row d-none d-md-flex">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($apartment['property_type'])): ?>
                                <p class="mb-0 mt-2"><strong>Property Type:</strong> <?php echo htmlspecialchars($apartment['property_type']); ?></p>
                            <?php endif; ?>
                            <?php if (!empty($apartment['year_built'])): ?>
                                <p class="mb-0"><strong>Year Built:</strong> <?php echo htmlspecialchars($apartment['year_built']); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-2">📲 Phone: <?php echo htmlspecialchars($apartment['contact']); ?></p>
                            <p class="mb-2">📧 Email: <?php echo htmlspecialchars($apartment['email']); ?></p>
                            <?php if (!empty($apartment['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($apartment['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-8 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                                <?php if (!empty($apartment['property_type'])): ?>
                                    <p class="mb-0 mt-2 small"><strong>Property Type:</strong> <?php echo htmlspecialchars($apartment['property_type']); ?></p>
                                <?php endif; ?>
                                <?php if (!empty($apartment['year_built'])): ?>
                                    <p class="mb-0 small"><strong>Year Built:</strong> <?php echo htmlspecialchars($apartment['year_built']); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-lg-8 mb-3">
                    <?php if (!empty($apartment['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?php echo htmlspecialchars(convertToEmbedURL($apartment['maps_embed'])); ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($nearby_places)): ?>
                                <p class="mb-2 small">Explore these places near <?= htmlspecialchars($apartment['name'] ?? '') ?>:</p>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($nearby_places as $place): ?>
                                        <span class="badge bg-primary badge-custom"><?php echo htmlspecialchars($place); ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                            ...
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($apartment['maps_link'])): ?>
                                    <a href="<?= htmlspecialchars($apartment['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                        <i class="fas fa-map-marked-alt me-1"></i> Maps
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($apartment['directions_link'])): ?>
                                    <a href="<?= htmlspecialchars($apartment['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                    ...
                    <?php if (!empty($social_links)): ?>
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($apartment['mobile_app'])): ?>
                                    <hr class="my-3">
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($apartment['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
