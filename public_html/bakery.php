...
$result = $conn->query("SELECT * FROM bakeries");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'bakeries') {
...
$query = "SELECT * FROM bakeries";
$bakeries_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $bakeries_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Bakeries in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($bakeries_exist): ?>
                <?php while ($bakery = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($bakery['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($bakery['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($bakery['name']) ?></h6>
                            <?php if (!empty($bakery['details_link'])): ?>
                                <a href="<?= htmlspecialchars($bakery['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No bakery found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
