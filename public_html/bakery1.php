...
$query = "SELECT * FROM bakeries WHERE id = 1";
$stmt = $conn->prepare($query);
$stmt->execute();
$result = $stmt->get_result();
$bakery = $result->fetch_assoc();

$transport_routes = array();
if (!empty($bakery['transportation_routes'])) {
    $routes_data = json_decode($bakery['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($bakery['nearby_places'])) {
    $decoded = json_decode($bakery['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $bakery['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $bakery['social_media'] ?? '')));

$hours = [];
if (!empty($bakery['hours'])) {
    $hours = explode("\n", $bakery['hours']);
}

$locations = array_filter(array_map('trim', explode('|', $bakery['location'] ?? '')));
...
<head>
    ...
    <title><?= $bakery['name'] ?> - Iloilo Bakeries</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= $bakery['image'] ?? '' ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= $bakery['image'] ?? '' ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= $bakery['name'] ?? '' ?></h1>
    <p class="lead"><?= $bakery['location'] ?? '' ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= $bakery['name'] ?? '' ?></h2>
      <div class="lead mb-4">
        <?= nl2br($bakery['description'] ?? '') ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= $image ?>" alt="<?= $bakery['name'] ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Bakery Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🍽️ Bakery Background</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br($bakery['history'] ?? '') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br($bakery['menu'] ?? '') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br($bakery['offers'] ?? '') ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🍽️ Bakery Background</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br($bakery['history'] ?? '') ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br($bakery['menu'] ?? '') ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br($bakery['offers'] ?? '') ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">                           
                            <p class="mb-2">📲 Phone: <?php echo $bakery['contact']; ?></p>

                             <p class="mb-2" >📧 Email:  <?php echo $bakery['email']; ?></p>
                            <?php if (!empty($bakery['website'])): ?>
                                        <p class="mb-2">🌐 Website: <a href="<?= $bakery['website'] ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                            <?php endif; ?>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">                                
                                <?php if (!empty($bakery['contact']) || !empty($bakery['email'])): ?>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= $bakery['year_established'] ?? '' ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= $bakery['specialty'] ?? '' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($bakery['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $bakery['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
                        ...
                        <?php else: ?>
                            <p class="text-muted small mb-0">Bakery information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= $bakery['year_established'] ?? '' ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= $bakery['specialty'] ?? '' ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($bakery['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= $bakery['maps_embed'] ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= $bakery['name'] ?? '' ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($bakery['maps_link'])): ?>
                                <a href="<?= $bakery['maps_link'] ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($bakery['directions_link'])): ?>
                                <a href="<?= $bakery['directions_link'] ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($bakery['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= $bakery['mobile_app'] ?>" class="btn btn-dark btn-sm" target="_blank">
