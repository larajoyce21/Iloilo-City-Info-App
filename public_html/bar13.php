...
$query = "SELECT * FROM bars WHERE id = 13";
...
if (!empty($bar['transportation_routes'])) {
    $routes_data = json_decode($bar['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($bar['nearby_places'])) {
    $decoded = json_decode($bar['nearby_places'], true);
    $nearby_places = (json_last_error() === JSON_ERROR_NONE && is_array($decoded))
        ? $decoded
        : array_filter(array_map('trim', explode(',', $bar['nearby_places'])));
}

$detail_images = array_filter(explode(',', $bar['image_detail'] ?? ''));
...
<head>
    ...
    <title><?php echo htmlspecialchars($bar['name']); ?></title>
...
<br>
    <section class="hero-section"
        ...
        url('<?= htmlspecialchars($bar['image'] ?? 'img/default-bar.jpg') ?>') center/cover no-repeat;">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($bar['name'] ?? '') ?></h1>
            <p class="lead"><?= htmlspecialchars($bar['location'] ?? '') ?></p>
        </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($bar['name'] ?? '') ?></h2>
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($bar['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            <h2 class="section-title">🍻 Bar Information</h2>
            <div class="lead mb-4">
                <?= nl2br(htmlspecialchars($bar['description'] ?? '')) ?>
            </div>
            <!-- Desktop  -->
            <div class="row d-none d-md-flex">
                <div class="col-lg-6 mb-4">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($bar['hours'])): ?>
                                <?= nl2br(htmlspecialchars($bar['hours'])) ?>
                            <?php else: ?>
                ...
                <div class="col-lg-6 mb-4">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($bar['contact'])): ?>
                                <p>📲 Phone: <?= htmlspecialchars($bar['contact']) ?></p>
                            <?php endif; ?>
                             <?php if (!empty($bar['email'])): ?>
                                <p>📧 Email: <?= htmlspecialchars($bar['email']) ?></p>
                            <?php endif; ?>
                            <?php if (!empty($bar['website'])): ?>
                                <p>🌐 Website: <a href="<?= htmlspecialchars($bar['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                                <?php if (!empty($bar['hours'])): ?>
                                    <p class="small"><?= nl2br(htmlspecialchars($bar['hours'])) ?></p>
                                <?php else: ?>
                                    <p class="small">Hours not specified</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            ...
                            <div class="card-body">
                             <?php if (!empty($bar['contact'])): ?>
                                <p class="small">📲 Phone: <?= htmlspecialchars($bar['contact']) ?></p>
                            <?php endif; ?>
                             <?php if (!empty($bar['email'])): ?>
                                <p class="small">📧 Email: <?= htmlspecialchars($bar['email']) ?></p>
                            <?php endif; ?>
                            <?php if (!empty($bar['website'])): ?>
                                <p class="small">🌐 Website: <a href="<?= htmlspecialchars($bar['website']) ?>" target="_blank" class="text-decoration-none" > Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-lg-8 mb-4">
                    <?php if (!empty($bar['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?= htmlspecialchars(convertToEmbedURL($bar['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($nearby_places)): ?>
                                <p class="mb-3">Explore these places near <?= htmlspecialchars($bar['name'] ?? '') ?>:</p>
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <?php if (!empty($bar['maps_link'])): ?>
                                    <a href="<?= htmlspecialchars($bar['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                        <i class="fas fa-map-marked-alt me-1"></i> Google Maps
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($bar['directions_link'])): ?>
                                    <a href="<?= htmlspecialchars($bar['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                    ...
                    <?php if (!empty($social_links)): ?>
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($bar['mobile_app'])): ?>
                                    <hr class="my-3">
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($bar['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
