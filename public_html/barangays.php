...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'barangays') {
...
$barangays_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $barangays_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="barangays">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Barangays in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($barangays_exist): ?>
                <?php while ($barangay = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($barangay['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($barangay['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($barangay['name']) ?></h6>
                            <?php if (!empty($barangay['details_link'])): ?>
                                <a href="<?= htmlspecialchars($barangay['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No barangay found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
