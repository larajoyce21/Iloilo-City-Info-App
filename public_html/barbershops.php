...
$result = $conn->query("SELECT * FROM barbershops");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'barbershops') {
...
$barbershops_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $barbershops_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bars">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Barbarshops in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($barbershops_exist): ?>
                <?php while ($barbershop = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($barbershop['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($barbershop['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($barbershop['name']) ?></h6>
                            <?php if (!empty($barbershop['details_link'])): ?>
                                <a href="<?= htmlspecialchars($barbershop['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No barbershops found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
