...
$result = $conn->query("SELECT * FROM bars");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'bars') {
...
<div class="album py-5" id="bars">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Bars and Clubs in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($bars_exist): ?>
                <?php while ($bar = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($bar['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($bar['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($bar['name']) ?></h6>
                            <?php if (!empty($bar['details_link'])): ?>
                                <a href="<?= htmlspecialchars($bar['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
