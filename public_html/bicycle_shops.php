...
$result = $conn->query("SELECT * FROM bicycle_shops");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'bicycle_shops') {
...
$query = "SELECT * FROM bicycle_shops";
$bicycle_shops_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $bicycle_shops_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Bicycle Shops in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($bicycle_shops_exist): ?>
                <?php while ($bicycle_shop = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($bicycle_shop['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($bicycle_shop['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($bicycle_shop['name']) ?></h6>
                            <?php if (!empty($bicycle_shop['details_link'])): ?>
                                <a href="<?= htmlspecialchars($bicycle_shop['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No bicycle shop found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
