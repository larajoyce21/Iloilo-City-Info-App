...
$result = $conn->query("SELECT * FROM birth_centers");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'birth_centers') {
...
$query = "SELECT * FROM birth_centers";
$birth_centers_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $birth_centers_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bars">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Birth Centers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($birth_centers_exist): ?>
                <?php while ($birth_center = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($birth_center['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($birth_center['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($birth_center['name']) ?></h6>
                            <?php if (!empty($birth_center['details_link'])): ?>
                                <a href="<?= htmlspecialchars($birth_center['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No birth center found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
