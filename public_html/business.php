...
include 'conn.php';
$businesses_result = $conn->query("SELECT * FROM business_listings WHERE status = 1 ORDER BY name");
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Businesses in Iloilo City</title>
...
<main class="container mt-5">
  <div class="text-center my-5">
    ...
    <h1 class="page-title text-white">All Businesses in Iloilo City</h1>
  </div>

  <?php if ($businesses_result && $businesses_result->num_rows > 0): ?>
    <div class="row row-cols-3 row-cols-lg-4 g-3 g-lg-4 mb-5">
      <?php while ($business = $businesses_result->fetch_assoc()): 
        $link = !empty($business['filename']) ? $business['filename'] : "business_details.php?id={$business['id']}";
        $iconClass = !empty($business['icon']) ? $business['icon'] : 'fas fa-store';
      ?>
        <div class="col">
          <a href="<?= htmlspecialchars($link) ?>" class="text-decoration-none text-dark">
            <div class="business-card h-100">
              ...
              <h5 class="mt-2"><?= htmlspecialchars($business['name']) ?></h5>
            </div>
          </a>
        </div>
      <?php endwhile; ?>
