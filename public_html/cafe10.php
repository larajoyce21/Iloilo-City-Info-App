...
$query = "SELECT * FROM cafes WHERE id = 10";
...
if (!empty($cafe['transportation_routes'])) {
    $routes_data = json_decode($cafe['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($cafe['nearby_places'])) {
    $decoded = json_decode($cafe['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $cafe['image_detail'] ?? ''));
...
$menu_items = [];
if (!empty($cafe['menu'])) {
    $menu_items = array_filter(array_map('trim', explode("\n", $cafe['menu'])));
}

$branches = [];
if (!empty($cafe['branches'])) {
...
<head>
    ...
    <title><?= htmlspecialchars($cafe['name']) ?> - Iloilo City Cafes</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($cafe['image'] ?? '') ?>');
        ...
        /* Transport Options - Updated like apartment1.php */
        .transport-option {
            margin-bottom: 10px;
        }
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($cafe['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($cafe['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($cafe['location'] ?? '') ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($cafe['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($cafe['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($cafe['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">☕ Cafe Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏬 Cafe Branches</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No branch information available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <div class="col-12 mb-2">
                                <p class="mb-0 small">
                                    <?= htmlspecialchars($cafe['wifi'] ?? '') ?><br>
                                    <?= htmlspecialchars($cafe['seating'] ?? '') ?><br>
                                    <?= htmlspecialchars($cafe['accessibility'] ?? '') ?>
                                </p>
                            </div>
                    </div>
                </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📜 Cafe Culture</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($cafe['culture'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🏬 Cafe Branches</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($branches)): ?>
                                <div class="list-group list-group-flush">
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">📜 Cafe Culture</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($cafe['culture'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <?= nl2br(htmlspecialchars($cafe['hours'])) ?>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <?php if (!empty($menu_items)): ?>
                            <div class="menu-items">
                                <?php foreach ($menu_items as $item): ?>
                                    <div class="menu-item">
                                        <span class="menu-item-name small"><?= htmlspecialchars($item) ?></span>
                                        <?php if (strpos($item, '₱') !== false): ?>
                                            <span class="menu-item-price small"><?= substr($item, strpos($item, '₱')) ?></span>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">Menu information coming soon.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <div class="col-md-6 mb-2">
                                <p class="mb-2">📲 Phone: <?= htmlspecialchars($cafe['contact'] ?? '') ?></p>

                                <p class="mb-2">📧 Email: <?= htmlspecialchars($cafe['email'] ?? '') ?></p>

                                
                                <?php if (!empty($cafe['website'])): ?>
                                    <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($cafe['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                    </div>
                </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">📖 Menu Highlights</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($menu_items)): ?>
                                <div class="menu-items">
            ...
            <div class="col-12 mb-3">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($cafe['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🍽️ Cuisine:</strong> <?= htmlspecialchars($cafe['cuisine'] ?? '') ?></p>
                            </div>
                            <div class="col-md-4">
                                <p class="mb-2"><strong>💰 Price Range:</strong> <?= htmlspecialchars($cafe['price_range'] ?? '') ?></p>
                                <p class="mb-2"><strong>💳 Payment Options:</strong> <?= htmlspecialchars($cafe['payment_options'] ?? '') ?></p>
                            </div>
                            <div class="col-md-4">
                                <p class="mb-2"><strong>🐶 Pet Policy:</strong> <?= htmlspecialchars($cafe['pet_policy'] ?? '') ?></p>
                                <p class="mb-0"><strong>🏠 Amenities:</strong> <?= htmlspecialchars($cafe['wifi'] ?? '') ?> <?= htmlspecialchars($cafe['seating'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($cafe['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🍽️ Cuisine:</strong> <?= htmlspecialchars($cafe['cuisine'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💰 Price Range:</strong> <?= htmlspecialchars($cafe['price_range'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💳 Payment Options:</strong> <?= htmlspecialchars($cafe['payment_options'] ?? '') ?></p>
                                    <p class="mb-0 small"><strong>🐶 Pet Policy:</strong> <?= htmlspecialchars($cafe['pet_policy'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($cafe['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($cafe['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($cafe['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($cafe['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($cafe['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($cafe['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($cafe['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($cafe['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($cafe['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
