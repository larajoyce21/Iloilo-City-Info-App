...
$result = $conn->query("SELECT * FROM call_centers");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'call_centers') {
...
$query = "SELECT * FROM call_centers";
$call_centers_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $call_centers_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Call Centers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($call_centers_exist): ?>
                <?php while ($call_center = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($call_center['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($call_center['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($call_center['name']) ?></h6>
                            <?php if (!empty($call_center['details_link'])): ?>
                                <a href="<?= htmlspecialchars($call_center['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No call center found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
