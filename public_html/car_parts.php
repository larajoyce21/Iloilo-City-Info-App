...
$result = $conn->query("SELECT * FROM car_parts");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'car_parts') {
...
$car_parts_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $car_parts_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>CARS AND CAR PARTS</title>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Cars and Car Parts in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($car_parts_exist): ?>
                <?php while ($car_part = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($car_part['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($car_part['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($car_part['name']) ?></h6>
                            <?php if (!empty($car_part['details_link'])): ?>
                                <a href="<?= htmlspecialchars($car_part['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No car part found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
