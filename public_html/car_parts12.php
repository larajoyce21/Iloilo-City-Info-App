...
$query = "SELECT * FROM car_parts WHERE id = 12";
...
if (!empty($car_part['transportation_routes'])) {
    $routes_data = json_decode($car_part['transportation_routes'], true);
...
$compatible_cars = [];
if (!empty($car_part['compatible_cars'])) {
    $compatible_cars = json_decode($car_part['compatible_cars'], true);
    if (!is_array($compatible_cars)) {
        $compatible_cars = [];
    }
}
...
if (!empty($car_part['image_detail'])) {
    $detail_images = explode(',', $car_part['image_detail']);
}

$specifications = [];
if (!empty($car_part['specifications'])) {
    $specifications = json_decode($car_part['specifications'], true);
    if (!is_array($specifications)) {
        $specifications = [];
    }
}

$social_links = array_filter(array_map('trim', explode("\n", $car_part['social_media'] ?? '')));

$hours = [];
if (!empty($car_part['hours'])) {
    $hours = explode("\n", $car_part['hours']);
}
$nearby_places = [];
if (!empty($car_part['nearby_places'])) {
    $nearby_places = json_decode($car_part['nearby_places'], true);
...
<head>
    ...
    <title><?= htmlspecialchars($car_part['name']) ?> - Iloilo Car Parts</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($car_part['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($car_part['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($car_part['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($car_part['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($car_part['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($car_part['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($car_part['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🔧 Car Part Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2"><strong>Brand:</strong> <?= htmlspecialchars($car_part['brand'] ?? '') ?></p>
                        <p class="mb-2"><strong>Part Number:</strong> <?= htmlspecialchars($car_part['part_number'] ?? '') ?></p>
                        <p class="mb-0"><strong>Warranty:</strong> <?= htmlspecialchars($car_part['warranty'] ?? '') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No specifications available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🚗 Compatible Cars</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($compatible_cars)): ?>
                            <p class="mb-2 small">This part is compatible with:</p>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($compatible_cars as $car): ?>
                                    <span class="badge bg-danger badge-custom"><?= htmlspecialchars($car) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No compatible cars listed.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-1 small"><strong>Brand:</strong> <?= htmlspecialchars($car_part['brand'] ?? 'N/A') ?></p>
                            <p class="mb-1 small"><strong>Part Number:</strong> <?= htmlspecialchars($car_part['part_number'] ?? 'N/A') ?></p>
                            <p class="mb-0 small"><strong>Warranty:</strong> <?= htmlspecialchars($car_part['warranty'] ?? 'N/A') ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🚗 Compatible Cars</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($compatible_cars)): ?>
                                <p class="mb-2 small">Compatible with:</p>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($compatible_cars as $car): ?>
                                        <span class="badge bg-danger badge-custom"><?= htmlspecialchars($car) ?></span>
                                    <?php endforeach; ?>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-store me-2"></i> Supplier Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                <h6 class="fw-bold">🏢 Supplier: </h6>
                                <p class="mb-2"><?= htmlspecialchars($car_part['supplier'] ?? '') ?></p>
                            </div>
                            <br>
                                <p class="mb-2">📲 Phone: <?= htmlspecialchars($car_part['contact'] ?? '') ?></p>

                                <p class="mb-2">📧 Email: <?= htmlspecialchars($car_part['email'] ?? '') ?></p>
                            
                            <?php if (!empty($car_part['website'])): ?>
                                <div class="col-md-6 mb-2">
                                    <p class="mb-2">🌐 Website:  <a href="<?= htmlspecialchars($car_part['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <?= nl2br(htmlspecialchars($car_part['hours'])) ?>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-store me-2"></i> Supplier Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold small">🏢 Supplier</h6>
                                    <p class="mb-2 small"><?= htmlspecialchars($car_part['supplier'] ?? '') ?></p>
                                </div>
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($car_part['contact'] ?? '') ?></p>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($car_part['email'] ?? '') ?></p>
                                <?php if (!empty($car_part['website'])): ?>
                                    <div class="col-12 mb-2">
                                        <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($car_part['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($car_part['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($car_part['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($car_part['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($car_part['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($car_part['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($car_part['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($car_part['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($car_part['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($car_part['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
