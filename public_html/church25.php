...
$query = "SELECT * FROM churches WHERE id = 25";
...
if (!empty($church['transportation_routes'])) {
    $routes_data = json_decode($church['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($church['nearby_places'])) {
    $nearby_places = json_decode($church['nearby_places'], true);
...
if (!empty($church['image_detail'])) {
    $detail_images = explode(',', $church['image_detail']);
}
...
<head>
    ...
    <title><?= htmlspecialchars($church['name']) ?> - Iloilo Churches</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($church['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($church['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($church['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($church['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($church['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($church['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($church['name']) ?> Image <?= $index + 1 ?>">
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🕰️ Mass Schedule</h5>
                    </div>
                    <div class="card-body">
                            <?= nl2br(htmlspecialchars($church['hours'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2">📲 Phone: <?= htmlspecialchars($church['contact'] ?? '') ?></p>
                        <?php if (!empty($church['email'])): ?>
                            <p class="mb-2">📧 Email: <?= htmlspecialchars($church['email']) ?></p>
                        <?php endif; ?>
                        <?php if (!empty($church['website'])): ?>
                            <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($church['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                        <?php endif; ?>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🕰️ Mass Schedule</h6>
                        </div>
                        <div class="card-body">
                            <?= nl2br(htmlspecialchars($church['hours'])) ?>
                        </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($church['contact'] ?? '') ?></p>
                            <?php if (!empty($church['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($church['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($church['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($church['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($church['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($church['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($church['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($church['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($church['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($church['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($church['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($church['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($church['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
