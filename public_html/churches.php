...
$result = $conn->query("SELECT * FROM churches");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'churches') {
...
$churches_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $churches_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Churches in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($churches_exist): ?>
                <?php while ($church = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($church['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($church['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($church['name']) ?></h6>
                            <?php if (!empty($church['details_link'])): ?>
                                <a href="<?= htmlspecialchars($church['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No church found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
