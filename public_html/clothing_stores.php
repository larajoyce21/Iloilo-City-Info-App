...
$result = $conn->query("SELECT * FROM clothing_stores");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'clothing_stores') {
...
$query = "SELECT * FROM clothing_stores";
$clothing_stores_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $clothing_stores_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Clothing Stores in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($clothing_stores_exist): ?>
                <?php while ($clothing_store = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($clothing_store['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($clothing_store['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($clothing_store['name']) ?></h6>
                            <?php if (!empty($clothing_store['details_link'])): ?>
                                <a href="<?= htmlspecialchars($clothing_store['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No clothing store found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
