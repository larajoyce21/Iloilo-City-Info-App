...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'colleges') {
...
$colleges_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $colleges_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
<div class="album py-5" id="colleges">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Colleges and Universities in Iloilo City</h1> 
        <div class="card-container">
            <?php if ($colleges_exist): ?>
                <?php while ($college = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($college['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($college['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($college['name']) ?></h6>
                            <?php if (!empty($college['details_link'])): ?>
                                <a href="<?= htmlspecialchars($college['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No bus landmarks found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
