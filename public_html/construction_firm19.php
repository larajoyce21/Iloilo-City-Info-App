...
$query = "SELECT * FROM construction_firms WHERE id = 19";
...
if (!empty($firm['transportation_routes'])) {
    $routes_data = json_decode($firm['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($firm['nearby_places'])) {
    $nearby_places = json_decode($firm['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $firm['image_detail'] ?? ''));
...
<head>
    ...
    <title><?= htmlspecialchars($firm['name'] ?? 'Construction Firm Iloilo') ?> - Iloilo Construction Services</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($firm['image'] ?? '') ?>');
    ...
    @media (max-width: 768px) {
        ...
        .construction-badge {
            background: linear-gradient(45deg, #004a8d, #0066cc);
            color: white;
            padding: 5px 12px;
            border-radius: 20px;
            font-size: 0.8rem;
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($firm['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($firm['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($firm['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($firm['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($firm['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($firm['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🏗️ Construction Services</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🛠️ Construction Services</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($firm['services'])): ?>
                        ...
                        <?php else: ?>
                            <p class="text-muted">No services information available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🔧 Specializations & Equipment</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-4">
                            <h6 class="d-flex align-items-center">
                                <i class="fas fa-cogs me-2"></i> Specializations
                            </h6>
                            <p class="ms-4"><?= !empty($firm['specializations']) ? htmlspecialchars($firm['specializations']) : '' ?></p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <p class="ms-4"><?= !empty($firm['equipment']) ? htmlspecialchars($firm['equipment']) : '' ?></p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                <h6 class="mb-0"><p class="mb-0"> Established: <?= !empty($firm['established']) ? htmlspecialchars($firm['established']) : '' ?></p></h6>
                            </div>
                        </div>
                        
                        <div class="mt-3">
                            <h6 class="d-flex align-items-center">
                                <i class="fas fa-building me-2"></i><p> Firm Type <?= !empty($firm['type']) ? htmlspecialchars($firm['type']) : '' ?></p>
            ...
            <div class="row">
                                    ...
                                    foreach ($services as $service):
                                        if (trim($service) != ''): ?>
                                            <li>
                                                ...
                                                <span class="small"><?= htmlspecialchars(trim($service)) ?></span>
                                            </li>
                                        <?php endif;
                                    endforeach; ?>
                                </ul>
                            <?php else: ?>
                                <p class="text-muted small">No services information available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-4">
                                <h6 class="d-flex align-items-center small">
                                    <i class="fas fa-cogs me-2"></i> Specializations
                                </h6>
                                <p class="ms-4 small"><?= !empty($firm['specializations']) ? htmlspecialchars($firm['specializations']) : 'Not available' ?></p>
                            </div>
                            
                            <div class="mb-4">
                                <h6 class="d-flex align-items-center small">
                                    <i class="fas fa-building me-2"></i> Firm Type
                                </h6>
                                <p class="ms-4 small"><?= !empty($firm['type']) ? htmlspecialchars($firm['type']) : 'Not available' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div>
                                    <h5 class="mb-0 small">Established: <?= !empty($firm['established']) ? htmlspecialchars($firm['established']) : '' ?></h5>
                                </div>
                            </div>
                            
                            <div class="mt-3">
                                <h6 class="d-flex align-items-center small">
                                    <i class="fas fa-toolbox me-2"></i> Equipment: <?= !empty($firm['equipment']) ? htmlspecialchars($firm['equipment']) : '' ?>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($firm['license_number'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">📋 License Number</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($firm['license_number'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2 ">📲 Phone: <?= htmlspecialchars($firm['contact'] ?? '') ?></p>
                            <?php if (!empty($firm['email'])): ?>
                                <p class="mb-2 ">📧 Email: <?= htmlspecialchars($firm['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($firm['website'])): ?>
                                <p class="mb-2 ">🌐 Website: <a href="<?= htmlspecialchars($firm['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($firm['emergency_contact'])): ?>
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold">🚨 Emergency Contact</h6>
                                    <p class="mb-2"><?= htmlspecialchars($firm['emergency_contact']) ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏗️ Projects in Iloilo City</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($firm['projects'])): ?>
                            <div class="projects-list">
                                <?php 
                                $project_count = 0;
                                foreach (explode("\n", $firm['projects']) as $project):
                                    if (trim($project) != ''): 
                                        $project_count++;
                                        ?>
                                        <div class="project-item mb-2">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <span class="small"><?= htmlspecialchars(trim($project)) ?></span>
                                            </div>
                                        </div>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                            <p class="text-muted small mt-2">Total: <?= $project_count ?> projects</p>
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No project information available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Company Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <?php if (!empty($firm['license_number'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">📋 License Number</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($firm['license_number'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($firm['contact'] ?? '') ?></p>
                                <?php if (!empty($firm['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($firm['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($firm['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($firm['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($firm['projects'])): ?>
                                ...
                                <p class="text-muted small mt-1">Total: <?= $project_count ?> projects</p>
                            <?php else: ?>
                                <div class="alert alert-info mb-0 py-2 small">No project information available.</div>
                            <?php endif; ?>
                        </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Company Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($firm['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏢 Type:</strong> <?= htmlspecialchars($firm['type'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📋 License:</strong> <?= htmlspecialchars($firm['license_number'] ?? '') ?></p>
                                <p class="mb-2"><strong>🛠️ Services:</strong> <?= !empty($firm['services']) ? count(explode("\n", $firm['services'])) . ' services' : '' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <?= nl2br(htmlspecialchars($firm['hours'])) ?>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Company Facts</h5>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($firm['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏢 Type:</strong> <?= htmlspecialchars($firm['type'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📋 License:</strong> <?= htmlspecialchars($firm['license_number'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🛠️ Services:</strong> <?= !empty($firm['services']) ? count(explode("\n", $firm['services'])) . ' services' : '' ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($firm['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($firm['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($firm['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($firm['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($firm['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($firm['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($firm['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($firm['portfolio_link'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($firm['portfolio_link']) ?>" class="btn btn-dark btn-sm" target="_blank">
                                        <i class="fas fa-download me-1"></i> View Portfolio
                                    </a>
                                    <a href="" class="btn btn-outline-dark btn-sm" target="_blank">
                                        <i class="fas fa-info-circle me-1"></i> Project Gallery
                                    </a>
                                </div>
                            <?php endif; ?>
                        </div>
