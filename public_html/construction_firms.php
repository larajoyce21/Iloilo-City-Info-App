...
$result = $conn->query("SELECT * FROM construction_firms");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'construction_firms') {
...
$query = "SELECT * FROM construction_firms";
$construction_firms_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $construction_firms_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>CONSTRUCTION FIRMS</title>
...
<br>
<div class="album py-5" id="construction_firms">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Construction Firms in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($construction_firms_exist): ?>
                <?php while ($construction_firm = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($construction_firm['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($construction_firm['name']) ?>">
                         <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($construction_firm['name']) ?></h6>
                            <?php if (!empty($construction_firm['details_link'])): ?>
                                <a href="<?= htmlspecialchars($construction_firm['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No construction firm found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
