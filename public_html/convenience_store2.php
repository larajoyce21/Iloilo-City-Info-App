...
$query = "SELECT * FROM convenience_stores WHERE id = 2";
...
if (!empty($store['image_detail'])) {
    $detail_images = explode(',', $store['image_detail']);
}
...
<head>
    ...
    <title><?= htmlspecialchars($store['name'] ?? 'Convenience Store') ?> - Iloilo Shopping</title>
    ...
    <style>
       ...
       .slideshow-container {
        ...
        /* Convenience Store Specific Styles */
        .services-list {
...
<section class="hero-section" 
  ...
  <main class="container mb-4">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-shopping-basket me-2"></i> Available Products</h5>
                    </div>
                    <div class="card-body">
                        <?php
                        $products = !empty($store['products']) ? json_decode($store['products'], true) : [];
                                    ...
                                    <div class="product-category">
                                        ...
                                        <div class="product-tags">
                                            <?php foreach ($items as $item): ?>
                                                <span class="product-tag"><?= htmlspecialchars($item) ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    </div>
                                <?php endif;
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Store Services</h5>
                    </div>
                    <div class="card-body">
                        <?php
                        $services = !empty($store['services']) ? explode(',', $store['services']) : [];
                        if (!empty($services)): ?>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-shopping-basket me-2"></i> Products</h6>
                        </div>
                        <div class="card-body small">
                            <?php
                            $products = !empty($store['products']) ? json_decode($store['products'], true) : [];
                            if (!empty($products) && is_array($products)): 
                                $first_category = true;
                                foreach ($products as $category => $items): 
                                    if (!empty($items) && $first_category): 
                                        $first_category = false; ?>
                                        <div class="product-category">
                                            <h6 class="small"><?= htmlspecialchars($category) ?></h6>
                                            <div class="product-tags">
                                                <?php 
                                                $count = 0;
                                                foreach ($items as $item): 
                                                    if ($count < 3): ?>
                                                        <span class="product-tag small"><?= htmlspecialchars($item) ?></span>
                                                    <?php 
                                                    $count++;
                                                    endif;
                                                endforeach; ?>
                                            ...
                                            <?php if (count($items) > 3): ?>
                                                <p class="text-muted small mt-2">+<?= count($items) - 3 ?> more items</p>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif;
                                endforeach;
                            else: ?>
                                <p class="text-muted small">Product information not available</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                            ...
                            if (!empty($services)): ?>
                                <ul class="services-list">
                                    ...
                                    foreach ($services as $service): 
                                        if ($count < 4): ?>
                                            <li class="small"><i class="fas fa-check"></i> <?= htmlspecialchars(trim($service)) ?></li>
                                        <?php 
                                        $count++;
                                        endif;
                                    endforeach; ?>
                                ...
                                <?php if (count($services) > 4): ?>
                                    <p class="text-muted small mt-2">+<?= count($services) - 4 ?> more services</p>
                                <?php endif; ?>
                            <?php else: ?>
                                <p class="text-muted small">Service information not available</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
    <h2 class="section-title">🏪 Store Branches</h2>
            ...
            <div class="card info-card">
                ...
                <div class="card-body">
                    <?php if (!empty($branches)): ?>
                        ...
                        <p class="text-muted small mt-2">Total: <?= $branch_count ?> branches in Iloilo City</p>
                    <?php else: ?>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>💵 Cash:</strong> Accepted</p>
                                <p class="mb-2"><strong>💳 Credit/Debit:</strong> <?= !empty($store['credit_cards']) ? 'Accepted' : 'Not available' ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📱 Mobile Pay:</strong> <?= !empty($store['mobile_payment']) ? 'Available' : 'Not available' ?></p>
                                <p class="mb-2"><strong>🏪 Store Type:</strong> <?= htmlspecialchars($store['type'] ?? 'Convenience Store') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="mb-2"><strong>🛍️ Products:</strong> <?= !empty($products) ? 'Multiple categories' : 'Not specified' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>💵 Cash:</strong> Accepted</p>
                                    <p class="mb-1 small"><strong>💳 Cards:</strong> <?= !empty($store['credit_cards']) ? 'Accepted' : 'Not available' ?></p>
                                    <p class="mb-1 small"><strong>📱 Mobile:</strong> <?= !empty($store['mobile_payment']) ? 'Available' : 'Not available' ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($store['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($store['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
...
function toggleTransport(type) {
    ...
    const allTypes = ['jeepney', 'bus', 'taxi', 'tricycle', 'parking', 'bike'];
