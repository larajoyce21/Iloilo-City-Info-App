...
$result = $conn->query("SELECT * FROM convenience_stores");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'convenience_stores') {
...
$query = "SELECT * FROM convenience_stores";
$convenience_stores_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $convenience_stores_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
<div class="album py-5" id="bars">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Convenience Stores in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($convenience_stores_exist): ?>
                <?php while ($convenience_store = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($convenience_store['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($convenience_store['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($convenience_store['name']) ?></h6>
                            <?php if (!empty($convenience_store['details_link'])): ?>
                                <a href="<?= htmlspecialchars($convenience_store['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No convenience store found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
