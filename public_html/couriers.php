...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'couriers') {
...
$couriers_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $couriers_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Couriers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($couriers_exist): ?>
                <?php while ($courier = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($courier['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($courier['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($courier['name']) ?></h6>
                            <?php if (!empty($courier['details_link'])): ?>
                                <a href="<?= htmlspecialchars($courier['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No courier found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
