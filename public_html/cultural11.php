...
$cultural_id = isset($_GET['id']) ? (int)$_GET['id'] : 11;

if ($cultural_id <= 0) {
    die("Invalid cultural site ID");
}

$query = "SELECT * FROM culturals WHERE id = ?";
$stmt = $conn->prepare($query); 
$stmt->bind_param("i", $cultural_id); 
$stmt->execute();
$result = $stmt->get_result();
$cultural = $result->fetch_assoc();

if (!$cultural) {
    die("Cultural site not found");
}

$transport_routes = array();
if (!empty($cultural['transportation_routes'])) {
    $routes_data = json_decode($cultural['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($cultural['nearby_places'])) {
    $decoded = json_decode($cultural['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $cultural['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $cultural['social_media'] ?? '')));
...
$locations = array_filter(array_map('trim', explode('|', $cultural['location'] ?? '')));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($cultural['name']) ?> - Iloilo Cultural Sites</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($cultural['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($cultural['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($cultural['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($cultural['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($cultural['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($cultural['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($cultural['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Cultural Site Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($cultural['cultural_background'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🗂️ Collections</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($cultural['collections'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($cultural['significance'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($cultural['cultural_background'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🗂️ Collections</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($cultural['collections'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($cultural['significance'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($cultural['fees'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Admission Fees</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($cultural['fees'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($cultural['contact'] ?? '') ?></p>
                            <?php if (!empty($cultural['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($cultural['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($cultural['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($cultural['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                            <?= nl2br(htmlspecialchars($cultural['hours'])) ?>
                            ...
                            <div class="row">
                                <?php if (!empty($cultural['fees'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">💵 Admission Fees</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($cultural['fees'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($cultural['contact'] ?? '') ?></p>
                                <?php if (!empty($cultural['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($cultural['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($cultural['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($cultural['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🏛️ Established:</strong> <?= htmlspecialchars($cultural['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏷️ Type:</strong> <?= htmlspecialchars($cultural['type'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🏛️ Established:</strong> <?= htmlspecialchars($cultural['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏷️ Type:</strong> <?= htmlspecialchars($cultural['type'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($cultural['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($cultural['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($cultural['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($cultural['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($cultural['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($cultural['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($cultural['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
