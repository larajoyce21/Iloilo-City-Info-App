...
$result = $conn->query("SELECT * FROM culturals");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'culturals') {
...
$query = "SELECT * FROM culturals";
$culturals_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $culturals_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="culturals">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Museums and Art Galleries in Iloilo City</h1> 
        
        
        <div class="card-container">
            <?php if ($culturals_exist): ?>
                <?php while ($cultural = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($cultural['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($cultural['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($cultural['name']) ?></h6>
                            <?php if (!empty($cultural['details_link'])): ?>
                                <a href="<?= htmlspecialchars($cultural['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No cultural landmarks found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
