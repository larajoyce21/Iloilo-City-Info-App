...
$result = $conn->query("SELECT * FROM dialysis_centers");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'dialysis_centers') {
...
$query = "SELECT * FROM dialysis_centers";
$dialysis_centers_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $dialysis_centers_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bars">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Dialysis Centers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($dialysis_centers_exist): ?>
                <?php while ($dialysis_center = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($dialysis_center['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($dialysis_center['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($dialysis_center['name']) ?></h6>
                            <?php if (!empty($dialysis_center['details_link'])): ?>
                                <a href="<?= htmlspecialchars($dialysis_center['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No dialysis center found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
