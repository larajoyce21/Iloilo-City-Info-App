...
$query = "SELECT * FROM districts WHERE id = 6";
...
if (!empty($district['transportation_routes'])) {
    $routes_data = json_decode($district['transportation_routes'], true);
...
if (!empty($district['nearby_places'])) {
    $decoded = json_decode($district['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $district['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $district['social_media'] ?? '')));

$hours = [];
if (!empty($district['hours'])) {
    $lines = explode("\n", $district['hours']);
...
<head>
    ...
    <title><?= htmlspecialchars($district['name']) ?> - Iloilo City Districts</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($district['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($district['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($district['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($district['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($district['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($district['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($district['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">📜 History & Culture</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏛️ Historical Background</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($district['history'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🎭 Cultural Information</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($district['feast'])): ?>
                            <div class="mb-2">
                                <h6 class="fw-bold small">📅 Feast Day</h6>
                                <p class="mb-0 small"><?= htmlspecialchars($district['feast']) ?></p>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($district['tribe'])): ?>
                            <div class="mb-2">
                                <h6 class="fw-bold small">👥 Indigenous Tribe</h6>
                                <p class="mb-0 small"><?= htmlspecialchars($district['tribe']) ?></p>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($district['source'])): ?>
                            <div class="mt-3">
                                <h6 class="fw-bold small">📚 Source</h6>
                                <p class="mb-0 small"><?= htmlspecialchars($district['source']) ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($district['fun_fact'])): ?>
                            <div class="fun-fact-alert">
                                <i class="fas fa-lightbulb me-2"></i>
                                <span class="small"><?= htmlspecialchars($district['fun_fact']) ?></span>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No fun facts available for this district.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <!-- Mobile Layout - History & Culture side by side -->
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($district['history'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
        ...
        <!-- Fun Fact for mobile (full width) -->
        <div class="d-md-none">
            <div class="row">
    ...
    <section class="content-section">
        <h2 class="section-title">ℹ️ District Information</h2>
        
        <!-- Desktop Layout - Visitor Details & Facts side by side -->
        <div class="row d-none d-md-flex">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <?php if (!empty($district['contact']) || !empty($district['email'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">📞 Contact</h6>
                                    <p class="mb-2">
                                        <?php if (!empty($district['contact'])): ?>
                                            <?= htmlspecialchars($district['contact']) ?><br>
                                        <?php endif; ?>
                                        <?php if (!empty($district['email'])): ?>
                                            <?= htmlspecialchars($district['email']) ?>
                                        <?php endif; ?>
                                    </p>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($district['website'])): ?>
                                <div class="col-12 mb-2">
                                    ...
                                    <p class="mb-2"><a href="<?= htmlspecialchars($district['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($district['hotlines'])): ?>
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold">☎️ Emergency Hotline</h6>
                                    <p class="mb-0"><?= htmlspecialchars($district['hotlines']) ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100 quick-facts-card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🧑‍🤝‍🧑 Population:</strong> <?= htmlspecialchars($district['population'] ?? '') ?></p>
                                <p class="mb-2"><strong>🗺️ Area:</strong> <?= htmlspecialchars($district['area'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏘️ Barangays:</strong> <?= htmlspecialchars($district['barangays'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📮 Zip Code:</strong> <?= htmlspecialchars($district['zip_code'] ?? '') ?></p>
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($district['established'] ?? '') ?></p>
                                <?php if (!empty($district['fee'])): ?>
                                    <p class="mb-0"><strong>💵 Entrance Fee:</strong> <?= htmlspecialchars($district['fee']) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <?php if (!empty($district['contact']) || !empty($district['email'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">📞 Contact</h6>
                                        <p class="mb-2 small">
                                            <?php if (!empty($district['contact'])): ?>
                                                <?= htmlspecialchars($district['contact']) ?><br>
                                            <?php endif; ?>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100 quick-facts-card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🧑‍🤝‍🧑 Population:</strong> <?= htmlspecialchars($district['population'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🗺️ Area:</strong> <?= htmlspecialchars($district['area'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏘️ Barangays:</strong> <?= htmlspecialchars($district['barangays'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📮 Zip Code:</strong> <?= htmlspecialchars($district['zip_code'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($district['established'] ?? '') ?></p>
                                    <?php if (!empty($district['fee'])): ?>
                                        <p class="mb-0 small"><strong>💵 Entrance Fee:</strong> <?= htmlspecialchars($district['fee']) ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <?php if (!empty($district['website'])): ?>
                                <p class="mb-2 small"><strong>🌐 Website:</strong> <a href="<?= htmlspecialchars($district['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($district['hotlines'])): ?>
                                <p class="mb-0 small"><strong>☎️ Emergency Hotline:</strong> <?= htmlspecialchars($district['hotlines']) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($district['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($district['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($district['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($district['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($district['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($district['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($district['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
