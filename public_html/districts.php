...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'districts') {
...
$districts_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $districts_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
<div class="album py-5" id="districts">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Districts of Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($districts_exist): ?>
                <?php while ($district = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($district['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($district['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($district['name']) ?></h6>
                            <?php if (!empty($district['details_link'])): ?>
                                <a href="<?= htmlspecialchars($district['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No district found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modals.forEach(modal => {
        modal.addEventListener('shown.bs.modal', function() {
            const innerCarousels = this.querySelectorAll('.carousel');
