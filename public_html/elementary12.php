...
$query = "SELECT * FROM elementarys WHERE id = 12"; 
...
if (!empty($school['transportation_routes'])) {
    $routes_data = json_decode($school['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($school['nearby_places'])) {
    $nearby_places = json_decode($school['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $school['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $school['social_media'] ?? '')));

$hours = [];
if (!empty($school['hours'])) {
    $lines = explode("\n", $school['hours']);
...
<head>
    ...
    <title><?= htmlspecialchars($school['name'] ?? 'Elementary School Iloilo') ?> - Iloilo Education Guide</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($school['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($school['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($school['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($school['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($school['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($school['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📜 School History</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($school['history'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
                        ...
                        <?php if (!empty($school['programs'])): ?>
                            <div class="d-flex flex-wrap">
                                ...
                                foreach ($programs as $program):
                                    if (trim($program) != ''): ?>
                                        <span class="program-tag"><?= htmlspecialchars(trim($program)) ?></span>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                        <?php else: ?>
                            <p class="mb-0">Academic program information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($school['facilities'])): ?>
                            <h6>Facilities</h6>
                            <p class="mb-3"><?= nl2br(htmlspecialchars($school['facilities'] ?? '')) ?></p>
                        <?php endif; ?>
                        
                        <?php if (!empty($school['sports'])): ?>
                            <h6>Sports & Activities</h6>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($school['sports'] ?? '')) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <!-- Mobile Layout - School History & Academic Programs side by side -->
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($school['history'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($school['programs'])): ?>
                                <div class="d-flex flex-wrap">
                                    ...
                                    foreach ($programs as $program):
                                        if (trim($program) != ''): ?>
                                            <span class="program-tag small"><?= htmlspecialchars(trim($program)) ?></span>
                                        <?php endif;
                                    endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 small">Academic program information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <!-- School Features for mobile (full width) -->
        <div class="d-md-none">
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <?php if (!empty($school['facilities'])): ?>
                                <h6 class="small">Facilities</h6>
                                <p class="mb-3 small"><?= nl2br(htmlspecialchars($school['facilities'] ?? '')) ?></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($school['sports'])): ?>
                                <h6 class="small">Sports & Activities</h6>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($school['sports'] ?? '')) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($school['fee'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 School Fees</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($school['fee'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($school['contact'] ?? '') ?></p>
                            <?php if (!empty($school['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($school['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($school['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($school['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100 academic-hours">
                    <div class="card-header">
                            ...
                            <div class="hours-schedule">
                                <?php foreach ($hours as $day => $times): ?>
                                    <div class="hours-day mb-2">
                                        <h6 class="fw-bold mb-1"><?= htmlspecialchars($day) ?></h6>
                                        <p class="mb-1 small"><?= implode('<br>', array_map('htmlspecialchars', $times)) ?></p>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No school hours available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> School Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <?php if (!empty($school['fee'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">💵 School Fees</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($school['fee'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($school['contact'] ?? '') ?></p>
                                <?php if (!empty($school['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($school['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($school['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($school['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 School Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($school['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ School Type:</strong> <?= htmlspecialchars($school['type'] ?? 'Elementary School') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>👨‍🏫 Principal:</strong> <?= htmlspecialchars($school['principal'] ?? '') ?></p>
                                <p class="mb-2"><strong>🎯 Curriculum:</strong> <?= htmlspecialchars($school['curriculum'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-graduation-cap me-2"></i> Additional Information</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($school['admission'])): ?>
                            <h6>Admission Process</h6>
                            <p class="mb-3"><?= nl2br(htmlspecialchars($school['admission'] ?? '')) ?></p>
                        <?php endif; ?>
                        
                        <?php if (!empty($school['uniform'])): ?>
                            <h6>School Uniform</h6>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($school['uniform'] ?? '')) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 School Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($school['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ School Type:</strong> <?= htmlspecialchars($school['type'] ?? 'Elementary School') ?></p>
                                    <p class="mb-1 small"><strong>👨‍🏫 Principal:</strong> <?= htmlspecialchars($school['principal'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🎯 Curriculum:</strong> <?= htmlspecialchars($school['curriculum'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-graduation-cap me-2"></i> Additional Info</h5>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($school['admission'])): ?>
                                <h6 class="small">Admission Process</h6>
                                <p class="mb-3 small"><?= nl2br(htmlspecialchars($school['admission'] ?? '')) ?></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($school['uniform'])): ?>
                                <h6 class="small">School Uniform</h6>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($school['uniform'] ?? '')) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-8 mb-3">
                <?php if (!empty($school['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($school['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($school['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($school['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($school['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($school['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($school['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        ...
                        $transport_types = array(
                            ...
                            'school_bus' => array('icon' => 'fas fa-bus', 'title' => 'School Bus')
                        );
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($school['parent_portal'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($school['parent_portal']) ?>" class="btn btn-dark btn-sm" target="_blank">
                                        <i class="fas fa-user-circle me-1"></i> Parent Portal
                                    </a>
                                    <a href="" class="btn btn-outline-dark btn-sm" target="_blank">
                                        <i class="fas fa-calendar-alt me-1"></i> School Calendar
                                    </a>
                                </div>
                            <?php endif; ?>
                        </div>
...
function toggleTransport(type) {
    ...
    const allTypes = ['jeepney', 'bus', 'taxi', 'tricycle', 'school_bus'];
