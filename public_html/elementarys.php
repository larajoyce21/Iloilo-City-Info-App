...
$result = $conn->query("SELECT * FROM elementarys");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'elementarys') {
...
$query = "SELECT * FROM elementarys";
$elementarys_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $elementarys_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
<div class="album py-5" id="elementarys">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Elementary Schools in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($elementarys_exist): ?>
                <?php while ($elementary = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($elementary['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($elementary['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($elementary['name']) ?></h6>
                            <?php if (!empty($elementary['details_link'])): ?>
                                <a href="<?= htmlspecialchars($elementary['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No elementary  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
