...
$result = $conn->query("SELECT * FROM ferries");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'ferries') {
...
if ($stmt = $conn->prepare($query)) {
    ...
    $ferries_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>Shipping and Ferry Services</title>
...
<br>
<div class="album py-5" id="ferries">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Shipping and Ferry Services in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($ferries_exist): ?>
                <?php while ($ferry = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($ferry['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($ferry ['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($ferry['name']) ?></h6>
                            <?php if (!empty($ferry['details_link'])): ?>
                                <a href="<?= htmlspecialchars($ferry['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No ferry  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
