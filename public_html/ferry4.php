...
$query = "SELECT * FROM ferries WHERE id = 4";
...
if (!empty($ferry['transportation_routes'])) {
    $routes_data = json_decode($ferry['transportation_routes'], true);
...
$amenities = [];
if (!empty($ferry['amenities'])) {
    $amenities = json_decode($ferry['amenities'], true);
...
$nearby_places = [];
if (!empty($ferry['nearby'])) {
    $nearby_places = json_decode($ferry['nearby'], true);
...
$detail_images = [];
if (!empty($ferry['gallery_images'])) {
    $detail_images = explode(',', $ferry['gallery_images']);
}

$social_links = array_filter(array_map('trim', explode("\n", $ferry['social_media'] ?? '')));

$hours = [];
if (!empty($ferry['hours'])) {
    $lines = explode("\n", $ferry['hours']);
...
<head>
    ...
    <title><?= htmlspecialchars($ferry['name']) ?> - Iloilo Ferry Services</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($ferry['image'] ?? 'img/default-2go.jpg') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($ferry['image'] ?? 'img/default-2go.jpg') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($ferry['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($ferry['location'] ?? '') ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($ferry['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($ferry['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($ferry['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🚢 Ferry Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2"><strong>🚢 Type of Vessel:</strong> <?= htmlspecialchars($ferry['type']) ?></p>
                        <p class="mb-2"><strong>👤 Operator:</strong> <?= htmlspecialchars($ferry['operator'] ?? '') ?></p>
                        <p class="mb-2"><strong>🧑‍🤝‍🧑 Capacity:</strong> <?= htmlspecialchars($ferry['capacity'] ?? '') ?></p>
                        <p class="mb-0"><strong>🕒 Travel Time:</strong> <?= htmlspecialchars($ferry['travel_time'] ?? '') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2"><strong>📍 From:</strong> <?= htmlspecialchars($ferry['origin']) ?></p>
                        <p class="mb-2"><strong>📍 To:</strong> <?= htmlspecialchars($ferry['destination']) ?></p>
                        <p class="mb-0"><strong>🛑 Stopovers:</strong> <?= htmlspecialchars($ferry['stopovers']) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📅 Schedule & Fares</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-2"><strong>🕒 Departure:</strong> <?= htmlspecialchars($ferry['departure_time']) ?></p>
                        <p class="mb-2"><strong>🕒 Arrival:</strong> <?= htmlspecialchars($ferry['arrival_time']) ?></p>
                        <?php if (!empty($ferry['fare_rates'])): ?>
                            <p class="mb-1"><strong>💰 Fare Rates:</strong></p>
                            <ul class="small mb-0">
                                <?php foreach (explode("\n", $ferry['fare_rates']) as $fare): ?>
                                    <li><?= htmlspecialchars($fare) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php endif; ?>
                    </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-1 small"><strong>🚢 Type:</strong> <?= htmlspecialchars($ferry['type']) ?></p>
                            <p class="mb-1 small"><strong>👤 Operator:</strong> <?= htmlspecialchars($ferry['operator'] ?? '') ?></p>
                            <p class="mb-1 small"><strong>🧑‍🤝‍🧑 Capacity:</strong> <?= htmlspecialchars($ferry['capacity'] ?? '') ?></p>
                            <p class="mb-0 small"><strong>🕒 Travel Time:</strong> <?= htmlspecialchars($ferry['travel_time'] ?? '') ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">🧭 Route Info</h5>
                        </div>
                        <div class="card-body">
                            <p class="mb-1 small"><strong>📍 From:</strong> <?= htmlspecialchars($ferry['origin']) ?></p>
                            <p class="mb-1 small"><strong>📍 To:</strong> <?= htmlspecialchars($ferry['destination']) ?></p>
                            <p class="mb-0 small"><strong>🛑 Stopovers:</strong> <?= htmlspecialchars($ferry['stopovers']) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-1 small"><strong>🕒 Departure:</strong> <?= htmlspecialchars($ferry['departure_time']) ?></p>
                            <p class="mb-2 small"><strong>🕒 Arrival:</strong> <?= htmlspecialchars($ferry['arrival_time']) ?></p>
                            <?php if (!empty($ferry['fare_rates'])): ?>
                                <p class="mb-1 small"><strong>💰 Fare Rates:</strong></p>
                                <ul class="small mb-0">
                                    <?php foreach (explode("\n", $ferry['fare_rates']) as $fare): ?>
                                        <li><?= htmlspecialchars($fare) ?></li>
                                    <?php endforeach; ?>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($ferry['booking_info'])): ?>
                            <p class="mb-2"><?= nl2br(htmlspecialchars($ferry['booking_info'])) ?></p>
                        <?php else: ?>
                            <p class="mb-2">Booking information not available.</p>
                        <?php endif; ?>
                        
                        <?php if (!empty($ferry['requirements'])): ?>
                            <h6 class="mt-3 mb-1">📝 Requirements</h6>
                            <p class="mb-2 small"><?= nl2br(htmlspecialchars($ferry['requirements'])) ?></p>
                        <?php endif; ?>
                        
                        <?php if (!empty($ferry['policies'])): ?>
                            <h6 class="mt-3 mb-1">🦺 Safety Policies</h6>
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($ferry['policies'])) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($ferry['amenities'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($ferry['amenities'])) ?></p>
                        <?php else: ?>
                            <p class="mb-0">Amenities information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($ferry['booking_info'])): ?>
                                <p class="mb-1 small"><?= nl2br(htmlspecialchars($ferry['booking_info'])) ?></p>
                            <?php else: ?>
                                <p class="mb-1 small">Booking info not available.</p>
                            <?php endif; ?>
                            
                            <?php if (!empty($ferry['requirements'])): ?>
                                <h6 class="mt-2 mb-1 small">📝 Requirements</h6>
                                <p class="mb-1 small"><?= nl2br(htmlspecialchars($ferry['requirements'])) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($ferry['amenities'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($ferry['amenities'])) ?></p>
                            <?php else: ?>
                                <p class="mb-0 small">Amenities not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>⭐ Safety Rating:</strong> <?= htmlspecialchars($ferry['safety_rating'] ?? '') ?></p>
                                <p class="mb-0"><strong>🐾 Pet Policy:</strong> <?= htmlspecialchars($ferry['pet_policy'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📞 Contact:</strong> <?= htmlspecialchars($ferry['phone']) ?></p>
                                <?php if (!empty($ferry['email'])): ?>
                                    <p class="mb-0"><strong>📩 Email:</strong> <?= htmlspecialchars($ferry['email']) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-globe me-2"></i> Website & Links</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($ferry['website'])): ?>
                            <p class="mb-2"><strong>🌐 Website:</strong> <a href="<?= htmlspecialchars($ferry['website']) ?>" target="_blank" class="text-decoration-none"><?= htmlspecialchars($ferry['website']) ?></a></p>
                        <?php endif; ?>
                        
                        <div class="d-flex flex-wrap gap-1 mt-2">
                            <?php if (!empty($ferry['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($ferry['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($ferry['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($ferry['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>⭐ Safety:</strong> <?= htmlspecialchars($ferry['safety_rating'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🐾 Pets:</strong> <?= htmlspecialchars($ferry['pet_policy'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📞 Contact:</strong> <?= htmlspecialchars($ferry['phone']) ?></p>
                                    <?php if (!empty($ferry['email'])): ?>
                                        <p class="mb-0 small"><strong>📩 Email:</strong> <?= htmlspecialchars($ferry['email']) ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-globe me-2"></i> Website</h5>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($ferry['website'])): ?>
                                <p class="mb-2 small"><a href="<?= htmlspecialchars($ferry['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                            <?php else: ?>
                                <p class="mb-2 small">Website not available</p>
                            <?php endif; ?>
    ...
    <section class="content-section">
        ...
        <div class="row">
                <div class="col-lg-8 mb-3">
                    <?php if (!empty($store['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?= convertToEmbedURL($store['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($ferry['name'] ?? '') ?>:</p>
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($ferry['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($ferry['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
...
function selectRoute(element, routeName) {
    // Clear active state from all route items in the same list
