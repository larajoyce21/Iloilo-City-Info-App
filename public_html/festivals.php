...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'festivals') {
...
$festivals_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $festivals_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<br>
<div class="album py-5" id="festivals">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Festivals and Celebrations in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($festivals_exist): ?>
                <?php while ($festival = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($festival['name']) ?></h6>
                            <?php if (!empty($festival['details_link'])): ?>
                                <a href="<?= htmlspecialchars($festival['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No festival  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
