...
$query = "SELECT * FROM fires WHERE id = 11";
...
if (!empty($fire['transportation_routes'])) {
    $routes_data = json_decode($fire['transportation_routes'], true);
...
$amenities = [];
if (!empty($fire['amenities'])) {
    $amenities = json_decode($fire['amenities'], true);
...
$nearby_places = [];
if (!empty($fire['nearby_places'])) {
    $nearby_places = json_decode($fire['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $fire['image_detail'] ?? ''));
...
<head>
    ...
    <title><?= htmlspecialchars($fire['name']) ?> - Iloilo City Fire Station</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($fire['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($fire['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($fire['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($fire['location'] ?? '') ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($fire['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($fire['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($fire['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🚒 Fire Station Services</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($fire['services'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🧯 Fire Prevention</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($fire['prevention'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($fire['programs'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($fire['services'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($fire['prevention'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">🤝 Community Programs</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($fire['programs'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="content-section">
        <h2 class="section-title">ℹ️ Emergency Information</h2>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                <h6 class="fw-bold">🚨 Emergency Hotline</h6>
                                <p class="mb-2"><?= htmlspecialchars($fire['hotlines'] ?? '') ?></p>
                            </div>
                            
                            <p class="mb-2 ">📲 Phone: <?= htmlspecialchars($fire['contact'] ?? '') ?></p>
                            <?php if (!empty($fire['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($fire['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($fire['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($fire['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">🚨 24/7 Emergency Services</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold small">🚨 Hotline</h6>
                                    <p class="mb-2 small"><?= htmlspecialchars($fire['hotlines'] ?? '') ?></p>
                                </div>
                                
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($fire['contact'] ?? '') ?></p>
                                <?php if (!empty($fire['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($fire['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($fire['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($fire['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body small">
                            ...
                            <?php else: ?>
                                <div class="alert alert-info mb-0 py-2 small">🚨 24/7 Emergency</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($fire['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏘️ District:</strong> <?= htmlspecialchars($fire['district'] ?? '') ?></p>
                                <p class="mb-0"><strong>👨‍🚒 Personnel:</strong> <?= htmlspecialchars($fire['personnel'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🚒 Fire Trucks:</strong> <?= htmlspecialchars($fire['equipment'] ?? '') ?></p>
                                <p class="mb-0"><strong>🏡 Coverage Area:</strong> <?= htmlspecialchars($fire['coverage'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Est:</strong> <?= htmlspecialchars($fire['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏘️ District:</strong> <?= htmlspecialchars($fire['district'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>👨‍🚒 Personnel:</strong> <?= htmlspecialchars($fire['personnel'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🚒 Trucks:</strong> <?= htmlspecialchars($fire['equipment'] ?? '') ?></p>
                                    <p class="mb-0 small"><strong>🏡 Coverage:</strong> <?= htmlspecialchars($fire['coverage'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($fire['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($fire['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($fire['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($fire['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($fire['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($fire['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($fire['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($fire['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($fire['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
