...
$result = $conn->query("SELECT * FROM fires");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'fires') {
...
<br>
<div class="album py-5" id="fires">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Bureau of Fire Stations in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($fires_exist): ?>
                <?php while ($fire = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($fire['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($fire['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($fire['name']) ?></h6>
                            <?php if (!empty($fire['details_link'])): ?>
                                <a href="<?= htmlspecialchars($fire['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
