...
$query = "SELECT * FROM flowershops WHERE id = 2"; 
...
if (!empty($flowershop['transportation_routes'])) {
    $routes_data = json_decode($flowershop['transportation_routes'], true);
...
$amenities = [];
if (!empty($flowershop['amenities'])) {
    $amenities = json_decode($flowershop['amenities'], true);
...
$nearby_places = [];
if (!empty($flowershop['nearby_places'])) {
    $nearby_places = json_decode($flowershop['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $flowershop['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $flowershop['social_media'] ?? '')));

$hours = [];
if (!empty($flowershop['hours'])) {
    $lines = explode("\n", $flowershop['hours']);
...
<html lang="en">
  <head>
    ...
    <title><?= htmlspecialchars($flowershop['name'] ?? 'Flower Shop') ?> - Iloilo City</title>
    ...
    <style>
      :root {
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($flowershop['image'] ?? '') ?>');
       ...
       .slideshow-container {
        ...
        .badge-custom:hover {
            background-color: #004a8d; /* Darker blue on hover */
        ...
        .social-btn.website {
            background-color: #004a8d; /* Changed from secondary to blue */
            color: white;
        }
...
<br>
    <section class="hero-section" 
      ...
      url('<?= htmlspecialchars($flowershop['image'] ?? '') ?>') center/cover no-repeat;">
      <div class="container">
        <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($flowershop['name'] ?? '') ?></h1>
        <p class="lead"><?= htmlspecialchars($flowershop['location'] ?? '') ?></p>
      </div>
    </section>

    <main class="container mb-4">
      <section class="content-section">
        <h2 class="section-title">About <?= htmlspecialchars($flowershop['name'] ?? '') ?></h2>
        <div class="lead mb-4">
          <?= nl2br(htmlspecialchars($flowershop['description'] ?? '')) ?>
        </div>
          ...
          <div class="slideshow-container">
            <?php foreach ($detail_images as $index => $image): ?>
              <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($flowershop['name']) ?> Image <?= $index + 1 ?>">
      ...
      <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
                ...
                <?php if (!empty($branches)): ?>
                  <ul class="list-group list-group-flush">
                    <?php foreach ($branches as $branch): ?>
                      <li class="list-group-item">📍 <?= htmlspecialchars($branch) ?></li>
                    <?php endforeach; ?>
                  </ul>
                <?php else: ?>
                  <p class="mb-0">No branch information available.</p>
                <?php endif; ?>
              </div>
            </div>
          </div>
          ...
          <div class="col-md-4 mb-3">
            <div class="card info-card h-100">
              <div class="card-header">
                <h5 class="mb-0">💐 Our Flowers</h5>
              </div>
              <div class="card-body">
                <?php if (!empty($flowershop['flowers'])): ?>
                  <p class="mb-0"><?= nl2br(htmlspecialchars($flowershop['flowers'])) ?></p>
                <?php else: ?>
                  <p class="mb-0">No flower information available.</p>
                <?php endif; ?>
              </div>
            </div>
          </div>
          ...
          <div class="col-md-4 mb-3">
            <div class="card info-card h-100">
              <div class="card-header">
                <h5 class="mb-0">❤️ Our Promise</h5>
              </div>
              <div class="card-body">
                <?php if (!empty($flowershop['promise'])): ?>
                  <p class="mb-0"><?= nl2br(htmlspecialchars($flowershop['promise'])) ?></p>
                <?php else: ?>
                  <p class="mb-0">No promise information available.</p>
                <?php endif; ?>
              </div>
            </div>
          </div>
        ...
        <div class="d-md-none mobile-side-by-side">
          <div class="row">
              ...
              <div class="card info-card h-100">
                ...
                <div class="card-body">
                  <?php if (!empty($branches)): ?>
                    <ul class="list-group list-group-flush">
                      <?php foreach ($branches as $branch): ?>
                        <li class="list-group-item small">📍 <?= htmlspecialchars($branch) ?></li>
                      <?php endforeach; ?>
                    </ul>
                  <?php else: ?>
            ...
            <div class="col-md-6 mb-3">
              <div class="card info-card h-100">
                <div class="card-header">
                  <h6 class="mb-0">💐 Our Flowers</h6>
                </div>
                <div class="card-body">
                  <?php if (!empty($flowershop['flowers'])): ?>
                    <p class="mb-0 small"><?= nl2br(htmlspecialchars($flowershop['flowers'])) ?></p>
                  <?php else: ?>
                    <p class="mb-0 small">No flower information available.</p>
                  <?php endif; ?>
                </div>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-12 mb-3">
              <div class="card info-card">
                <div class="card-header">
                  <h6 class="mb-0">❤️ Our Promise</h6>
                </div>
                <div class="card-body">
                  <?php if (!empty($flowershop['promise'])): ?>
                    <p class="mb-0 small"><?= nl2br(htmlspecialchars($flowershop['promise'])) ?></p>
                  <?php else: ?>
                    <p class="mb-0 small">No promise information available.</p>
                  <?php endif; ?>
                </div>
              </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="card info-card h-100">
              ...
              <div class="card-body">
                <div class="row">
                  ...
                  <?php if (!empty($flowershop['price_range'])): ?>
                    <div class="col-md-6 mb-2">
                      <h6 class="fw-bold">💵 Price Range</h6>
                      <p class="mb-2 small"><?= htmlspecialchars($flowershop['price_range']) ?></p>
                    </div>
                  <?php endif; ?>
                  
                  <p class="mb-2">📲 Phone: <?= htmlspecialchars($flowershop['contact'] ?? '') ?></p>
                  <?php if (!empty($flowershop['email'])): ?>
                    <p class="mb-2">📧 Email: <?= htmlspecialchars($flowershop['email']) ?> </p>
                  <?php endif; ?>
                  <?php if (!empty($flowershop['website'])): ?>
                    <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($flowershop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                  <?php endif; ?>
                </div>
              </div>
            </div>
          ...
          <div class="col-lg-6 mb-3">
            <div class="card info-card h-100">
              ...
              <div class="card-body">
                <?php if (!empty($flowershop['services'])): ?>
                  <ul class="list-group list-group-flush">
                    <?php
                    $services = explode("\n", $flowershop['services']);
                    foreach ($services as $service) {
                      if (!empty(trim($service))) {
                        echo '<li class="list-group-item border-0 px-0 py-1 small">💐 ' . htmlspecialchars(trim($service)) . '</li>';
                      }
                    }
                    ?>
                  </ul>
                <?php else: ?>
                  <div class="alert alert-info mb-0 py-2 small">No services information available.</div>
                <?php endif; ?>
              </div>
            </div>
          </div>
        ...
        <div class="row d-none d-md-flex">
          <div class="col-lg-6 mb-3">
            <div class="card info-card">
              ...
              <div class="card-body">
                <div class="row">
                  <?= nl2br(htmlspecialchars($flowershop['hours'])) ?>
                </div>
              </div>
            </div>
          </div>
          <div class="col-lg-6 mb-3">
            <div class="card info-card">
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-md-6">
                    <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($flowershop['year_established'] ?? '') ?></p>
                    <p class="mb-2"><strong>🌟 Specialty:</strong> <?= htmlspecialchars($flowershop['specialty'] ?? '') ?></p>
                    <p class="mb-2"><strong>📍 Delivery Area:</strong> <?= htmlspecialchars($flowershop['delivery_area'] ?? '') ?></p>
                  </div>
                  <div class="col-md-6">
                    <p class="mb-2"><strong>💳 Payment Methods:</strong> <?= htmlspecialchars($flowershop['payment_methods'] ?? '') ?></p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
              <div class="card info-card">
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col-12">
                      <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($flowershop['year_established'] ?? '') ?></p>
                      <p class="mb-1 small"><strong>🌟 Specialty:</strong> <?= htmlspecialchars($flowershop['specialty'] ?? '') ?></p>
                      <p class="mb-1 small"><strong>📍 Delivery Area:</strong> <?= htmlspecialchars($flowershop['delivery_area'] ?? '') ?></p>
                      <p class="mb-1 small"><strong>💳 Payment Methods:</strong> <?= htmlspecialchars($flowershop['payment_methods'] ?? '') ?></p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section class="content-section">
        ...
        <div class="row">
          <div class="col-lg-8 mb-3">
            <?php if (!empty($flowershop['maps_embed'])): ?>
              <div class="map-container">
                <iframe src="<?= htmlspecialchars(convertToEmbedURL($flowershop['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
              </div>
            <?php else: ?>
            ...
            <div class="card info-card h-100">
              ...
              <div class="card-body">
                <?php if (!empty($nearby_places)): ?>
                  <p class="mb-2 small">Explore these places near <?= htmlspecialchars($flowershop['name'] ?? '') ?>:</p>
                ...
                <div class="d-flex flex-wrap gap-1">
                  <?php if (!empty($flowershop['maps_link'])): ?>
                    <a href="<?= htmlspecialchars($flowershop['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                      <i class="fas fa-map-marked-alt me-1"></i> Maps
                    </a>
                  <?php endif; ?>
                  
                  <?php if (!empty($flowershop['directions_link'])): ?>
                    <a href="<?= htmlspecialchars($flowershop['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
            ...
            <?php if (!empty($social_links)): ?>
              <div class="card info-card">
                ...
                <div class="card-body">
                  ...
                  <?php if (!empty($flowershop['mobile_app'])): ?>
                    <hr class="my-3">
                    <div class="d-flex flex-wrap gap-1">
                      <a href="<?= htmlspecialchars($flowershop['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
