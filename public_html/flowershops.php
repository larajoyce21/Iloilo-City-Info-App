...
$result = $conn->query("SELECT * FROM flowershops");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'flowershops') {
...
$query = "SELECT * FROM flowershops";
$flowershops_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $flowershops_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="flowershops">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Flower Shops in Iloilo City</h1> 
        <div class="card-container">
            <?php if ($flowershops_exist): ?>
                <?php while ($flowershop = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($flowershop['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($flowershop['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($flowershop['name']) ?></h6>
                            <?php if (!empty($flowershop['details_link'])): ?>
                                <a href="<?= htmlspecialchars($flowershop['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No festival landmarks found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
