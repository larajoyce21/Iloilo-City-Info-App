...
$result = $conn->query("SELECT * FROM funeral_services");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'funeral_services') {
...
$query = "SELECT * FROM funeral_services";
$funeral_services_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $funeral_services_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Funeral Home and Burial Services in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($funeral_services_exist): ?>
                <?php while ($funeral_service = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($funeral_service['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($funeral_service['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($funeral_service['name']) ?></h6>
                            <?php if (!empty($funeral_service['details_link'])): ?>
                                <a href="<?= htmlspecialchars($funeral_service['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No funeral service found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
