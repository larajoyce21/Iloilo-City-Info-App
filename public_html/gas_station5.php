...
$query = "SELECT * FROM gas_stations WHERE id = 5";
...
if (!empty($station['transportation_routes'])) {
    $routes_data = json_decode($station['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($station['nearby_places'])) {
    $nearby_places = json_decode($station['nearby_places'], true);
...
if (!empty($station['image_detail'])) {
    $detail_images = explode(',', $station['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $station['social_media'] ?? '')));
...
<head>
    ...
    <title><?php echo htmlspecialchars($station['name']); ?> - Gas Station</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($station['image'] ?? '') ?>');
...
<section class="hero-section" 
    style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($station['image'] ?? '') ?>') center/cover no-repeat;">
    <div class="container">
        <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($station['name'] ?? '') ?></h1>
        <p class="lead"><?= htmlspecialchars($station['location'] ?? '') ?></p>
    </div>
</section>

<main class="container mb-4">
    <section class="content-section">
        <h2 class="section-title">About <?= htmlspecialchars($station['name'] ?? '') ?></h2>
        <div class="lead mb-4">
            <?= nl2br(htmlspecialchars($station['description'] ?? '')) ?>
        </div>
            ...
            <div class="slideshow-container">
                <?php foreach ($detail_images as $index => $image): ?>
                    <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                        <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($station['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">⛽ Gas Station Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2">📲 Phone: <?= htmlspecialchars($station['contact'] ?? '') ?></p>
                        <?php if (!empty($station['email'])): ?>
                            <p class="mb-2">📧 Email: <?= htmlspecialchars($station['email']) ?> </p>
                        <?php endif; ?>
                        <?php if (!empty($station['website'])): ?>
                            <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($station['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?= nl2br(htmlspecialchars($station['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($station['contact'] ?? '') ?></p>
                            <?php if (!empty($station['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($station['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($station['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($station['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        <h2 class="section-title">⛽ Fuel & Services</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">⛽ Fuel Types Available</h5>
                    </div>
                    <div class="card-body">
                        <?php 
                        $fuel_types = !empty($station['fuel_types']) ? json_decode($station['fuel_types'], true) : [];
                        if (!empty($fuel_types)): ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($fuel_types as $fuel): ?>
                                    <span class="fuel-badge"><?php echo htmlspecialchars($fuel); ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <p class="mb-0">Fuel types not specified</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php 
                        $services = !empty($station['services']) ? json_decode($station['services'], true) : [];
                        if (!empty($services)): ?>
                            <div class="d-flex flex-wrap">
                        ...
                        <?php else: ?>
                            <p class="mb-0">Services not specified</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <p class="mb-0 small">Fuel types not specified</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <p class="mb-0 small">Services not specified</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($station['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($station['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($station['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($station['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($station['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($station['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($station['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($station['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($station['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
