...
$query = "SELECT * FROM gyms WHERE id = 10";
...
if (!empty($gym['transportation_routes'])) {
    $routes_data = json_decode($gym['transportation_routes'], true);
...
$trainers = [];
if (!empty($gym['trainers'])) {
    $trainers = json_decode($gym['trainers'], true);
...
if (!empty($gym['image_detail'])) {
    $detail_images = explode(',', $gym['image_detail']);
}
...
<head>
    ...
    <title><?php echo htmlspecialchars($gym['name']); ?> - Gym & Fitness Facility</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($gym['image'] ?? '') ?>');
        ...
        @media (max-width: 576px) {
            ...
            /* Stack amenities vertically on very small screens */
            .amenity-item {
                margin-bottom: 8px;
                padding: 6px;
            }
...
<section class="hero-section" 
    style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($gym['image'] ?? '') ?>') center/cover no-repeat;">
    <div class="container">
        <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($gym['name'] ?? '') ?></h1>
        <p class="lead"><?= htmlspecialchars($gym['location'] ?? '') ?></p>
    </div>
</section>

<main class="container mb-4">
    <section class="content-section">
        <h2 class="section-title">About <?= htmlspecialchars($gym['name'] ?? '') ?></h2>
        <div class="lead mb-4">
            <?= nl2br(htmlspecialchars($gym['description'] ?? '')) ?>
        </div>
            ...
            <div class="slideshow-container">
                <?php foreach ($detail_images as $index => $image): ?>
                    <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                        <img src="<?= htmlspecialchars(trim($image)) ?>" alt="<?= htmlspecialchars($gym['name']) ?> Image <?= $index + 1 ?>" 
                             onerror="this.src='data:image/svg+xml;base64,'">
    ...
    <section class="content-section">
        <h2 class="section-title">🏋️ Gym Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($gym['facility_type'])): ?>
                            <p class="mb-0 mt-2"><strong>Type:</strong> <?= htmlspecialchars($gym['facility_type']) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?= nl2br(htmlspecialchars($gym['hours'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2">📲 Phone: <?= htmlspecialchars($gym['contact'] ?? '') ?></p>
                        <?php if (!empty($gym['email'])): ?>
                            <p class="mb-2">📧 Email: <?= htmlspecialchars($gym['email']) ?> </p>
                        <?php endif; ?>
                        <?php if (!empty($gym['website'])): ?>
                            <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($gym['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($gym['facility_type'])): ?>
                                <p class="mb-0 mt-2 small"><strong>Type:</strong> <?= htmlspecialchars($gym['facility_type']) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($gym['contact'] ?? '') ?></p>
                            <?php if (!empty($gym['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($gym['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($gym['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($gym['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card info-card">
                        <div class="card-header">
                            <h5 class="mb-0">✨ Available Amenities</h5>
                        </div>
                        <div class="card-body">
                            <div class="row">
    ...
    <?php if (!empty($gym['membership_plans'])): 
        $membership_plans = json_decode($gym['membership_plans'], true);
                ...
                <?php foreach ($membership_plans as $index => $plan): ?>
                    <div class="col-md-6 col-lg-4 mb-3">
                        <div class="membership-card <?= $index == 1 ? 'featured' : '' ?>">
                            <h4 class="small"><?= htmlspecialchars($plan['name']) ?></h4>
                            <div class="price">₱<?= number_format($plan['price'], 2) ?></div>
                            <div class="period"><?= htmlspecialchars($plan['period']) ?></div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($gym['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($gym['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
            ...
            <div class="col-lg-4">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($gym['nearby_places'])): 
                            $nearby_places = json_decode($gym['nearby_places'], true);
                            if (is_array($nearby_places) && count($nearby_places) > 0): ?>
                                <p class="mb-2 small">Explore these places near <?= htmlspecialchars($gym['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($gym['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($gym['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($gym['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($gym['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($gym['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($gym['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
