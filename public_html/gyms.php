...
$result = $conn->query("SELECT * FROM gyms");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'gyms') {
...
<br>
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Gyms and Fitness Facilities in Iloilo City</h1>

        <div class="card-container">
            <?php if ($gyms_exist): ?>
                <?php while ($gym = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($gym['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($gym['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($gym['name']) ?></h6>
                            <?php if (!empty($gym['details_link'])): ?>
                                <a href="<?= htmlspecialchars($gym['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No gym found.</div>
            <?php endif; ?>
        </div>
