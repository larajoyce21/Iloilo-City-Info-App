...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'hardware') {
...
$hardware_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $hardware_exist = $result->num_rows > 0; 
    $stmt->close();
}
?>
...
<br>
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Hardwares in Iloilo City</h1>

        <div class="card-container">
            <?php if ($hardware_exist): ?>
                <?php while ($hardware = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($hardware['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($hardware['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($hardware['name']) ?></h6>
                            <?php if (!empty($hardware['details_link'])): ?>
                                <a href="<?= htmlspecialchars($hardware['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
