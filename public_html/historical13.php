...
$query = "SELECT * FROM historicals WHERE id = 13";
$result = mysqli_query($conn, $query);
$historical = mysqli_fetch_assoc($result);
...
if (!empty($historical['transportation_routes'])) {
    $routes_data = json_decode($historical['transportation_routes'], true);
...
if (empty($transport_routes) && !empty($historical['transportation_options'])) {
    $transport_options = explode("\n", $historical['transportation_options']);
    foreach ($transport_options as $option) {
        ...
        if (!empty($option)) {
            ...
            if (stripos($type, 'jeepney') !== false || stripos($type, 'bus') !== false) $transport_type = 'jeepney';
            ...
            elseif (stripos($type, 'walk') !== false) $transport_type = 'walking';
...
$nearby_places = [];
if (!empty($historical['nearby_places'])) {
    $nearby_places = json_decode($historical['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $historical['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $historical['social_media'] ?? '')));

$adminEditLink = '';
if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in']) {
    $adminEditLink = '<a href="add_historicals.php?id=1" class="btn btn-sm btn-outline-secondary">Edit Molo Mansion</a>';
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($historical['name'] ?? 'Molo Mansion') ?> - Iloilo Cultural Heritage</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($historical['image'] ?? '') ?>');
        ...
        .slideshow-controls button {
            background-color: rgba(14, 12, 12, 0.47);
            color: white;
            border: none;
            width: 30px;
            height: 30px;
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($historical['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($historical['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($historical['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($historical['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($historical['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($historical['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🎭 Cultural Background</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($historical['history'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($historical['history'])) ?></p>
                        <?php else: ?>
                            <p class="mb-0 text-muted">Cultural background information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏛️ Architectural Details</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($historical['architecture'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($historical['architecture'])) ?></p>
                        <?php else: ?>
                            <p class="mb-0 text-muted">Architectural details not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">⭐ Cultural Significance</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($historical['significance'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($historical['significance'])) ?></p>
                        <?php else: ?>
                            <p class="mb-0 text-muted">Cultural significance information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🎭 Cultural Background</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($historical['history'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($historical['history'])) ?></p>
                            <?php else: ?>
                                <p class="mb-0 small text-muted">Cultural background information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🏛️ Architectural Details</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($historical['architecture'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($historical['architecture'])) ?></p>
                            <?php else: ?>
                                <p class="mb-0 small text-muted">Architectural details not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">⭐ Cultural Significance</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($historical['significance'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($historical['significance'])) ?></p>
                            <?php else: ?>
                                <p class="mb-0 small text-muted">Cultural significance information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <?php if (!empty($historical['fees'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Admission</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($historical['fees'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($historical['contact'] ?? '') ?></p>
                            <?php if (!empty($historical['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($historical['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($historical['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($historical['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                             <p class="mb-2"><?= nl2br(htmlspecialchars($historical['hours'])) ?></p>
            ...
            <div class="col-lg-5 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🗓️ Built:</strong> <?= htmlspecialchars($historical['year_built'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏛️ Architectural Style:</strong> <?= htmlspecialchars($historical['architectural_style'] ?? '') ?></p>
                                <p class="mb-2"><strong>✅ Status:</strong> <?= htmlspecialchars($historical['status'] ?? '') ?></p>
                                <p class="mb-2"><strong>🧱 Materials Used:</strong> <?= htmlspecialchars($historical['materials'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Unique Feature:</strong> <?= htmlspecialchars($historical['unique_feature'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Visitor Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <?php if (!empty($historical['fees'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">💵 Admission</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($historical['fees'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($historical['contact'] ?? '') ?></p>
                            <?php if (!empty($historical['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($historical['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($historical['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($historical['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <p class="mb-2 small"><?= nl2br(htmlspecialchars($historical['hours'])) ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🗓️ Built:</strong> <?= htmlspecialchars($historical['year_built'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏛️ Architectural Style:</strong> <?= htmlspecialchars($historical['architectural_style'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>✅ Status:</strong> <?= htmlspecialchars($historical['status'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🧱 Materials Used:</strong> <?= htmlspecialchars($historical['materials'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Unique Feature:</strong> <?= htmlspecialchars($historical['unique_feature'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($historical['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($historical['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($historical['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($historical['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($historical['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($historical['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($historical['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($historical['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($historical['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
...
function toggleTransport(type) {
    ...
    const allTypes = ['jeepney', 'bus', 'taxi', 'tricycle', 'walking', 'other'];
