...
$result = $conn->query("SELECT * FROM home_stores");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'home_stores') {
...
$query = "SELECT * FROM home_stores";
$home_stores_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $home_stores_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="resorts">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Home Stores in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($home_stores_exist): ?>
                <?php while ($home_store = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($home_store['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($home_store['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($home_store['name']) ?></h6>
                            <?php if (!empty($home_store['details_link'])): ?>
                                <a href="<?= htmlspecialchars($home_store['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No home store found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
