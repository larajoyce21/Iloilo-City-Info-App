...
$query = "SELECT * FROM hospitals WHERE id = 13"; 
$result = $conn->query($query);
$hospital = $result->fetch_assoc();

$transport_routes = array();
if (!empty($hospital['transportation_routes'])) {
    $routes_data = json_decode($hospital['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($hospital['nearby_places'])) {
    $nearby_places = json_decode($hospital['nearby_places'], true);
...
$detail_images = [];
if (!empty($hospital['image_detail'])) {
    $detail_images = explode(',', $hospital['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $hospital['social_media'] ?? '')));

$hours = [];
if (!empty($hospital['hours'])) {
    $lines = explode("\n", $hospital['hours']);
...
<head>
    ...
    <title><?= htmlspecialchars($hospital['name'] ?? 'Hospital Name') ?> - Iloilo Hospitals</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($hospital['image'] ?? '') ?>');
...
<br>
    <section class="hero-section" 
      style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($hospital['image'] ?? '') ?>') center/cover no-repeat;">
      <div class="container">
        <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($hospital['name'] ?? '') ?></h1>
        <p class="lead"><?= htmlspecialchars($hospital['location'] ?? '') ?></p>
      </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($hospital['name'] ?? '') ?></h2>
            <div class="lead mb-4">
                <?= nl2br(htmlspecialchars($hospital['description'] ?? '')) ?>
            </div>
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($hospital['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            ...
            <div class="row d-none d-md-flex">
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    foreach ($services as $service) {
                                        if (!empty(trim($service))) {
                                            echo '<li class="list-group-item small">' . htmlspecialchars($service) . '</li>';
                                        }
                                    }
                                    ?>
                                </ul>
                ...
                <div class="col-md-4 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">👨‍⚕️👩‍⚕️ Available Doctors</h5>
                        </div>
                        <div class="card-body">
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    foreach ($doctors as $doctor) {
                                        if (!empty(trim($doctor))) {
                                            echo '<li class="list-group-item small">' . htmlspecialchars($doctor) . '</li>';
                                        }
                                    }
                                    ?>
                                </ul>
                            <?php else: ?>
                                <p class="text-muted small">No doctors information available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">🏥 Hospital Type</h5>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($hospital['type'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($hospital['type'])) ?></p>
                            <?php else: ?>
                                <p class="text-muted small">No hospital type information available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 mb-3">
                        <div class="card info-card">
                            <div class="card-header">
                                <h6 class="mb-0">🏥 Hospital Type</h6>
                            </div>
                            <div class="card-body">
                                <?php if (!empty($hospital['type'])): ?>
                                    <p class="mb-0 small"><?= nl2br(htmlspecialchars($hospital['type'])) ?></p>
        ...
        <section class="content-section">
            ...
            <div class="row d-none d-md-flex">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">🛏️ Beds</h6>
                                    <p class="mb-2"><?= htmlspecialchars($hospital['beds'] ?? '') ?></p>
                                </div>
                                
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">📞 Hotlines</h6>
                                    <p class="mb-2"><?= htmlspecialchars($hospital['hotlines'] ?? '') ?></p>
                                </div>                                
                                <p class="mb-2">📲 Phone: <?= htmlspecialchars($hospital['phone'] ?? '') ?></p>
                                <?php if (!empty($hospital['email'])): ?>
                                    <p class="mb-2">📧 Email: <?= htmlspecialchars($hospital['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($hospital['website'])): ?>
                                    <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($hospital['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">ℹ️ Room Information</h5>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($hospital['rooms'])): ?>
                                <p class="mb-0"><?= nl2br(htmlspecialchars($hospital['rooms'])) ?></p>
                            <?php else: ?>
                                <div class="alert alert-info mb-0 py-2 small">Room information not available.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="d-md-none mobile-side-by-side">
                <div class="row">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🛏️ Beds</h6>
                                        <p class="mb-2 small"><?= htmlspecialchars($hospital['beds'] ?? '') ?></p>
                                    </div>
                                    
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">📞 Hotlines</h6>
                                        <p class="mb-2 small"><?= htmlspecialchars($hospital['hotlines'] ?? '') ?></p>
                                    </div>
                                     <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($hospital['phone'] ?? '') ?></p>
                                    <?php if (!empty($hospital['email'])): ?>
                                        <p class="mb-2 small">📧 Email: <?= htmlspecialchars($hospital['email']) ?> </p>
                                    <?php endif; ?>
                                    <?php if (!empty($hospital['website'])): ?>
                                        <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($hospital['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            <div class="card-header">
                                <h6 class="mb-0">ℹ️ Room Information</h6>
                            </div>
                            <div class="card-body">
                                <?php if (!empty($hospital['rooms'])): ?>
                                    <p class="mb-0 small"><?= nl2br(htmlspecialchars($hospital['rooms'])) ?></p>
                                <?php else: ?>
                                    <div class="alert alert-info mb-0 py-2 small">Room information not available.</div>
                                <?php endif; ?>
                            </div>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-lg-8 mb-3">
                    <?php if (!empty($hospital['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?= htmlspecialchars(convertToEmbedURL($hospital['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($nearby_places)): ?>
                                <p class="mb-2 small">Explore these places near <?= htmlspecialchars($hospital['name'] ?? '') ?>:</p>
                            ...
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($hospital['maps_link'])): ?>
                                    <a href="<?= htmlspecialchars($hospital['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                        <i class="fas fa-map-marked-alt me-1"></i> Maps
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($hospital['directions_link'])): ?>
                                    <a href="<?= htmlspecialchars($hospital['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                    ...
                    <?php if (!empty($social_links)): ?>
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($hospital['mobile_app'])): ?>
                                    <hr class="my-3">
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($hospital['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
