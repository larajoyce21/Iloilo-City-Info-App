...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'hospitals') {
...
$hospitals_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $hospitals_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="hospitals">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Hospitals and Health Centers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($hospitals_exist): ?>
                <?php while ($hospital = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($hospital['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($hospital['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($hospital['name']) ?></h6>
                            <?php if (!empty($hospital['details_link'])): ?>
                                <a href="<?= htmlspecialchars($hospital['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No hospital  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
