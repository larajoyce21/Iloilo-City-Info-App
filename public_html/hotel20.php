...
$query = "SELECT * FROM hotels WHERE id = 20";
...
if (!empty($hotel['transportation_routes'])) {
    $routes_data = json_decode($hotel['transportation_routes'], true);
...
if (!empty($hotel['nearby_places'])) {
    $nearby_places = json_decode($hotel['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $hotel['image_detail'] ?? ''));
$amenities = array_filter(explode(',', $hotel['amenities'] ?? ''));
$social_links = !empty($hotel['social_media']) ? array_filter(explode("\n", $hotel['social_media'])) : [];

$hours = [];
if (!empty($hotel['hours'])) {
    $lines = explode("\n", $hotel['hours']);
...
<head>
    ...
    <meta name="description" content="<?= htmlspecialchars($hotel['name'] ?? 'Hotel') ?> in Iloilo City">
    <meta name="author" content="Iloilo City Tourism">
    <title><?= htmlspecialchars($hotel['name'] ?? 'Hotel') ?> - Iloilo City</title>
    ...
    <style>
      :root {
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($hotel['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($hotel['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($hotel['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($hotel['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($hotel['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($hotel['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🛏️ Room Types</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($hotel['room_types'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($hotel['room_types'])) ?></p>
                        <?php else: ?>
                        ...
                        <?php if (!empty($amenities)): ?>
                            <ul class="list-unstyled mb-0">
                                <?php foreach ($amenities as $amenity): ?>
                                    <li class="mb-1"><i class="fas fa-check text-success me-2"></i> <?= htmlspecialchars($amenity) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                            <p class="text-muted mb-0">Amenities information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🍽️ Dining Options</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($hotel['dining'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($hotel['dining'])) ?></p>
                        <?php else: ?>
                            <p class="text-muted mb-0">Dining information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🛏️ Room Types</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($hotel['room_types'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($hotel['room_types'])) ?></p>
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Room information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($amenities)): ?>
                                <ul class="list-unstyled mb-0">
                                    <?php foreach ($amenities as $amenity): ?>
                                        <li class="mb-1 small"><i class="fas fa-check text-success me-2"></i> <?= htmlspecialchars($amenity) ?></li>
                                    <?php endforeach; ?>
                                </ul>
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Amenities information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">🍽️ Dining Options</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($hotel['dining'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($hotel['dining'])) ?></p>
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Dining information not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Hotel Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">                                                        
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($hotel['contact'] ?? '') ?></p>
                            <?php if (!empty($hotel['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($hotel['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($hotel['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($hotel['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($hotel['policies'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($hotel['policies'])) ?></p>
                        <?php else: ?>
                            <p class="text-muted mb-0">Hotel policies not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Hotel Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">                                                              
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($hotel['contact'] ?? '') ?></p>
                                <?php if (!empty($hotel['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($hotel['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($hotel['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($hotel['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">📋 Hotel Policies</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($hotel['policies'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($hotel['policies'])) ?></p>
                            <?php else: ?>
                                <p class="text-muted mb-0 small">Hotel policies not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🏨 Floors:</strong> <?= htmlspecialchars($hotel['floors'] ?? '') ?></p>
                                <p class="mb-2"><strong>🛏️ Number of Rooms:</strong> <?= htmlspecialchars($hotel['room_count'] ?? '') ?></p>
                                <p class="mb-2"><strong>🗓️ Year Built:</strong> <?= htmlspecialchars($hotel['year_built'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🅿️ Parking:</strong> <?= htmlspecialchars($hotel['parking'] ?? '') ?></p>
                                <p class="mb-2"><strong>📶 WiFi:</strong> <?= htmlspecialchars($hotel['wifi'] ?? '') ?></p>
                                <p class="mb-2"><strong>🚐 Airport Shuttle:</strong> <?= htmlspecialchars($hotel['shuttle'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                      <?= nl2br(htmlspecialchars($hotel['hours'])) ?>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🏨 Floors:</strong> <?= htmlspecialchars($hotel['floors'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🛏️ Number of Rooms:</strong> <?= htmlspecialchars($hotel['room_count'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🗓️ Year Built:</strong> <?= htmlspecialchars($hotel['year_built'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🅿️ Parking:</strong> <?= htmlspecialchars($hotel['parking'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📶 WiFi:</strong> <?= htmlspecialchars($hotel['wifi'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🚐 Airport Shuttle:</strong> <?= htmlspecialchars($hotel['shuttle'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($hotel['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($hotel['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($hotel['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($hotel['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($hotel['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($hotel['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($hotel['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($hotel['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($hotel['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
