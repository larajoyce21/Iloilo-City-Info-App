...
$query = "SELECT * FROM jeeps WHERE id = 22"; 
...
$jeep = $result->fetch_assoc();

function parseJeepOperators($operators_data) {
    $operators = [];
    ...
    foreach ($lines as $line) {
        ...
        if (strpos($line, '|') !== false) {
            ...
            // Legacy format support (space separated)
            $parts = explode(' ', $line);
...
$jeep_operators = parseJeepOperators($jeep['jeep_operators']);

$detail_images = $jeep['image_detail'] ? explode(',', $jeep['image_detail']) : [];
$routes = $jeep['routes'] ? explode("\n", $jeep['routes']) : [];

$total_stops = count($routes);
$total_operators = count($jeep_operators);

if (empty($jeep['image'])) {
    $jeep['image'] = 'img/default-jeep.jpg';
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= $jeep['name'] ?> - Iloilo Transportation</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= $jeep['image'] ?>');
...
<div id="mySidebar" class="sidebar">
  ...
  <div class="mt-auto p-4 text-center">
    ...
    <section class="hero-section">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= $jeep['name'] ?></h1>
            <p class="lead"><?= $jeep['location'] ?></p>
        </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= $jeep['name'] ?></h2>
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        ...
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= trim($image) ?>" alt="Jeepney Route Detail <?= $index + 1 ?>" loading="lazy">
        ...
        <section class="content-section">
            ...
            <?php if ($jeep['peak_hours'] || $jeep['off_hours']): ?>
            <div class="row mb-4">
                <div class="col-12">
                    <div class="card info-card">
                        <div class="card-header">
                            <h5 class="mb-0">⏰ Jeepney Schedule Hours</h5>
                        </div>
                        <div class="card-body">
                            <div class="hours-container">
                                <?php if ($jeep['peak_hours']): ?>
                                    <div class="hours-badge peak-hours-badge">
                                        <i class="fas fa-users"></i>
                                        Peak Hours: <?= $jeep['peak_hours'] ?>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if ($jeep['off_hours']): ?>
                                    <div class="hours-badge off-hours-badge">
                                        <i class="fas fa-user-friends"></i>
                                        Off Hours: <?= $jeep['off_hours'] ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <div class="row d-none d-md-flex">
                            ...
                            <?php if (!empty($routes)): ?>
                                <ul class="route-list">
                                    <?php foreach ($routes as $route): ?>
                                        <?php if (trim($route)): ?>
                                        <li><i class="fas fa-map-marker-alt"></i> <?= trim($route) ?></li>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </ul>
                            <?php else: ?>
                ...
                <div class="col-lg-6 mb-4">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($jeep_operators)): ?>
                                <?php foreach ($jeep_operators as $company): ?>
                                    <div class="operator-card">
                                        <div class="operator-header">
                                            ...
                                            <div>
                                                <div class="operator-name"><?= $company['name'] ?></div>
                                                <div class="operator-address">
                                                    <i class="fas fa-map-marker-alt me-1"></i>
                                                    <?= substr($company['address'], 0, 50) . (strlen($company['address']) > 50 ? '...' : '') ?>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <?php foreach ($jeep_operators as $company): ?>
                                        <div class="operator-card">
                                            <div class="operator-header">
                                                ...
                                                <div>
                                                    <div class="operator-name"><?= substr($company['name'], 0, 25) . (strlen($company['name']) > 25 ? '...' : '') ?></div>
                                                    <div class="operator-address">
                                                        <i class="fas fa-map-marker-alt me-1"></i>
                                                        <?= substr($company['address'], 0, 30) . (strlen($company['address']) > 30 ? '...' : '') ?>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="operator-info">
                                                ...
                                                <span><?= $company['operating_hours'] ?></span>
                                            </div>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-lg-8 mb-4">
                    <?php if (!empty($jeep['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?= $jeep['maps_embed'] ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                    ...
                    <div class="card info-card quick-facts-card">
                        ...
                        <div class="card-body">
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($jeep['maps_link'])): ?>
                                    <a href="<?= $jeep['maps_link'] ?>" class="btn btn-outline-primary btn-lg" target="_blank">
