...
$result = $conn->query("SELECT * FROM laundry_services");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'laundry_services') {
...
$laundrys_exist = false; 

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $laundrys_exist = $result->num_rows > 0; 
    $stmt->close();
}
?>
...
<br>
    ...
    <br>
    <br>
    <h1 class="page-title text-light text-center">Laundry Services in Iloilo City</h1>

        <div class="card-container">
            <?php if ($laundrys_exist): ?>
                <?php while ($laundry = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($laundry['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($laundry['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($laundry['name']) ?></h6>
                            <?php if (!empty($laundry['details_link'])): ?>
                                <a href="<?= htmlspecialchars($laundry['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No laundry found.</div>
            <?php endif; ?>
        </div>
