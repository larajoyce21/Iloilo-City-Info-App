...
$query = "SELECT * FROM malls WHERE id = 38";
...
if (!empty($mall['transportation_routes'])) {
    $routes_data = json_decode($mall['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($mall['nearby_places'])) {
    $decoded = json_decode($mall['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $mall['image_detail'] ?? ''));
...
$stores = !empty($mall['stores'])
    ? array_filter(array_map('trim', explode("\n", $mall['stores'])))
    : [];

$dining_options = !empty($mall['dining_options'])
    ? array_filter(array_map('trim', explode("\n", $mall['dining_options'])))
    : [];

$entertainment = !empty($mall['entertainment'])
    ? array_filter(array_map('trim', explode("\n", $mall['entertainment'])))
    : [];
...
<head>
    ...
    <title><?= htmlspecialchars($mall['name']) ?> - Iloilo Shopping Malls</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($mall['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($mall['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($mall['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($mall['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($mall['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($mall['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($mall['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🛍️ Mall Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($mall['contact'] ?? '') ?></p>
                            <?php if (!empty($mall['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($mall['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($mall['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($mall['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($mall['hours'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Mall Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🗓️ Opened:</strong> <?= htmlspecialchars($mall['year_opened'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏢 Floors:</strong> <?= htmlspecialchars($mall['floors'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏬 Total Stores:</strong> <?= htmlspecialchars($mall['total_stores'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🅿️ Parking:</strong> <?= htmlspecialchars($mall['parking'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Unique Feature:</strong> <?= htmlspecialchars($mall['unique_feature'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($mall['contact'] ?? '') ?></p>
                            <?php if (!empty($mall['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($mall['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($mall['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($mall['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Mall Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🗓️ Opened:</strong> <?= htmlspecialchars($mall['year_opened'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏢 Floors:</strong> <?= htmlspecialchars($mall['floors'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏬 Total Stores:</strong> <?= htmlspecialchars($mall['total_stores'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🅿️ Parking:</strong> <?= htmlspecialchars($mall['parking'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($mall['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?php echo htmlspecialchars(convertToEmbedURL($mall['maps_embed'])); ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($mall['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($mall['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($mall['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($mall['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($mall['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($mall['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($mall['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
