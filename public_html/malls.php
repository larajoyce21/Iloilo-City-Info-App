...
$result = $conn->query("SELECT * FROM malls");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'malls') {
...
<div class="album py-5" id="malls">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Shopping Malls in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($malls_exist): ?>
                <?php while ($mall = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($mall['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($mall['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($mall['name']) ?></h6>
                            <?php if (!empty($mall['details_link'])): ?>
                                <a href="<?= htmlspecialchars($mall['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
