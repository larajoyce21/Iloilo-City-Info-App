...
$query = "SELECT * FROM markets WHERE id = 5";
...
if (!empty($market['transportation_routes'])) {
    $routes_data = json_decode($market['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($market['nearby_places'])) {
    $decoded = json_decode($market['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $market['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $market['social_media'] ?? '')));

$product_categories = !empty($market['products'])
    ? array_filter(array_map('trim', explode("\n", $market['products'])))
    : [];

$specialties = !empty($market['specialties'])
    ? array_filter(array_map('trim', explode("\n", $market['specialties'])))
    : [];

$vendor_types = !empty($market['vendor_types'])
    ? array_filter(array_map('trim', explode("\n", $market['vendor_types'])))
    : [];
...
<head>
    ...
    <title><?= htmlspecialchars($market['name']) ?> - Iloilo Public Markets</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($market['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($market['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($market['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($market['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($market['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($market['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($market['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🧺 Products Available</h5>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12">
                                <p class="mb-1 small"><strong>🗓️ Established:</strong> <?= htmlspecialchars($market['year_established'] ?? '') ?></p>
                                <p class="mb-1 small"><strong>🏪 Vendor Count:</strong> <?= htmlspecialchars($market['vendor_count'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Mobile Layout - Products & Specialties side by side -->
                        ...
                        <div class="card-body">
                            <?php if (!empty($product_categories)): ?>
                                <div class="products-list">
                                    <?php foreach ($product_categories as $product): ?>
                                        <div class="product-item">
                                            <i class="fas fa-shopping-basket text-success me-2 small"></i>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($market['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?php echo htmlspecialchars(convertToEmbedURL($market['maps_embed'])); ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($market['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($market['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($market['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($market['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($market['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($market['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($market['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
