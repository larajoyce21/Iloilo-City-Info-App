...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'markets') {
...
if ($stmt = $conn->prepare($query)) {
    ...
    $markets_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="markets">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Public Markets in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($markets_exist): ?>
                <?php while ($market = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($market['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($market['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($market['name']) ?></h6>
                            <?php if (!empty($market['details_link'])): ?>
                                <a href="<?= htmlspecialchars($market['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No market  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
