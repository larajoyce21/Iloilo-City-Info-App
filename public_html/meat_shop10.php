...
$query = "SELECT * FROM meat_shops WHERE id = 10";
...
if (!empty($meat_shop['transportation_routes'])) {
    $routes_data = json_decode($meat_shop['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($meat_shop['nearby_places'])) {
    $decoded = json_decode($meat_shop['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $meat_shop['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $meat_shop['social_media'] ?? '')));

$hours = [];
if (!empty($meat_shop['hours'])) {
    $lines = explode("\n", $meat_shop['hours']);
...
$meat_products = !empty($meat_shop['products'])
    ? array_filter(array_map('trim', explode("\n", $meat_shop['products'])))
    : [];

$meat_specialties = !empty($meat_shop['specialties'])
    ? array_filter(array_map('trim', explode("\n", $meat_shop['specialties'])))
    : [];

$services = !empty($meat_shop['services'])
    ? array_filter(array_map('trim', explode("\n", $meat_shop['services'])))
    : [];
...
<head>
    ...
    <title><?= htmlspecialchars($meat_shop['name']) ?> - Iloilo Meat Shops</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($meat_shop['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($meat_shop['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($meat_shop['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($meat_shop['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($meat_shop['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($meat_shop['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($meat_shop['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🥩 Meat Products & Services</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🥩 Meat Products</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($meat_products)): ?>
                            <div class="products-list">
                                <?php foreach ($meat_products as $product): ?>
                        ...
                        <?php else: ?>
                            <p class="mb-0">Product information coming soon.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($meat_specialties)): ?>
                            <div class="specialties-list">
                                <?php foreach ($meat_specialties as $specialty): ?>
                                    <div class="product-item">
                        ...
                        <?php else: ?>
                            <p class="mb-0">Specialty information coming soon.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <p class="mb-0">Service information coming soon.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($meat_products)): ?>
                                <div class="products-list">
                                    <?php foreach ($meat_products as $product): ?>
                                        <div class="product-item">
                                            <i class="fas fa-drumstick-bite text-danger me-2 small"></i>
                                            <span class="small"><?= htmlspecialchars($product) ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 small">Product information coming soon.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="specialties-list">
                                    <?php foreach ($meat_specialties as $specialty): ?>
                                        <div class="product-item">
                                            ...
                                            <span class="small"><?= htmlspecialchars($specialty) ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 small">Specialty information coming soon.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <?php if (!empty($services)): ?>
                                <div class="services-list">
                                    <?php foreach ($services as $service): ?>
                                        <div class="product-item">
                                            <i class="fas fa-concierge-bell text-primary me-2 small"></i>
                                            <span class="small"><?= htmlspecialchars($service) ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 small">Service information coming soon.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Shop Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($meat_shop['contact'] ?? '') ?></p>
                            <?php if (!empty($meat_shop['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($meat_shop['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($meat_shop['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($meat_shop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            
                            <?php if (!empty($meat_shop['delivery_hotline'])): ?>
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold">🚚 Delivery Hotline</h6>
                                    <p class="mb-2"><?= htmlspecialchars($meat_shop['delivery_hotline']) ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                      <?= nl2br(htmlspecialchars($meat_shop['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Shop Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($meat_shop['contact'] ?? '') ?></p>
                                <?php if (!empty($meat_shop['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($meat_shop['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($meat_shop['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($meat_shop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                                
                                <?php if (!empty($meat_shop['delivery_hotline'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🚚 Delivery Hotline</h6>
                                        <p class="mb-2 small"><?= htmlspecialchars($meat_shop['delivery_hotline']) ?></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🏆 Quality Standards</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($meat_shop['quality_standards'])): ?>
                            <p class="mb-0"><?= nl2br(htmlspecialchars($meat_shop['quality_standards'])) ?></p>
                        <?php else: ?>
                            <p class="text-muted small mb-0">Quality standards information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($meat_shop['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $meat_shop['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($meat_shop['quality_standards'])): ?>
                                <p class="mb-0 small"><?= nl2br(htmlspecialchars($meat_shop['quality_standards'])) ?></p>
                            <?php else: ?>
                                <p class="text-muted small mb-0">Quality standards information not available.</p>
                            <?php endif; ?>
                        </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($meat_shop['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?php echo htmlspecialchars(convertToEmbedURL($meat_shop['maps_embed'])); ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($meat_shop['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($meat_shop['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($meat_shop['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($meat_shop['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($meat_shop['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
            ...
            <div class="col-12">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        ...
                        <?php if (!empty($meat_shop['delivery_info'])): ?>
                            <div class="row mt-2">
                                <div class="col-12">
                                    <div class="transport-option">
                                        <div class="transport-icon">
                                        ...
                                        <div>
                                            <h6 class="mb-1 small fw-bold">Delivery Information</h6>
                                            <p class="mb-0 small text-muted"><?= htmlspecialchars($meat_shop['delivery_info']) ?></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($meat_shop['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($meat_shop['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
