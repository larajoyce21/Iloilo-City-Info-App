...
$result = $conn->query("SELECT * FROM meat_shops");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'meat_shops') {
...
$query = "SELECT * FROM meat_shops";
$meat_shops_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $meat_shops_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Meat Shops in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($meat_shops_exist): ?>
                <?php while ($meat_shop = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($meat_shop['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($meat_shop['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($meat_shop['name']) ?></h6>
                            <?php if (!empty($meat_shop['details_link'])): ?>
                                <a href="<?= htmlspecialchars($meat_shop['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No meat shop found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
