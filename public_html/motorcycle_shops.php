...
$result = $conn->query("SELECT * FROM motorcycle_shops");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'motorcycle_shops') {
...
$motorcycle_shops_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $motorcycle_shops_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="resorts">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Motorcycle Shops in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($motorcycle_shops_exist): ?>
                <?php while ($motorcycle_shop = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($motorcycle_shop['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($motorcycle_shop['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($motorcycle_shop['name']) ?></h6>
                            <?php if (!empty($motorcycle_shop['details_link'])): ?>
                                <a href="<?= htmlspecialchars($motorcycle_shop['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No motorcycle shop found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
