...
$query = "SELECT * FROM new_tourist_spots WHERE id = 3"; 
$result = $conn->query($query);
$spot = $result->fetch_assoc();

$transport_routes = array();
if (!empty($spot['transportation_routes'])) {
    $routes_data = json_decode($spot['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($spot['nearby_places'])) {
    $nearby_list = json_decode($spot['nearby_places'], true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        $nearby_list = array_filter(array_map('trim', explode(',', $spot['nearby_places'])));
    }
}

$detail_images = array_filter(explode(',', $spot['image_detail'] ?? ''));
$activities = !empty($spot['activities']) ? explode('|', $spot['activities']) : [];
$features = !empty($spot['features']) ? explode('|', $spot['features']) : [];
...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'new_tourist_spots') {
...
<head>
    ...
    <title><?= htmlspecialchars($spot['name']) ?> - Iloilo Tourist Spots</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($spot['image'] ?? '') ?>');
        ...
        /* Tourist Spot specific styles */
        .feature-list, .activity-list {
            list-style-type: none;
            padding-left: 0;
        }
        
        .feature-list li, .activity-list li {
            padding: 6px 0;
            border-bottom: 1px solid #eee;
            font-size: 0.85rem;
        }
        
        .feature-list li:last-child, .activity-list li:last-child {
            border-bottom: none;
        }
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($spot['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($spot['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($spot['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($spot['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($spot['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">📌 Features & Activities</h2>
        
        <!-- Desktop Layout - Features & Activities side by side -->
                        ...
                        <?php if (!empty($features)): ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($features as $feature): ?>
                                    <span class="badge badge-custom feature-badge mb-2"><?= htmlspecialchars($feature) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No features listed.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                        ...
                        <?php if (!empty($activities)): ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($activities as $activity): ?>
                                    <span class="badge badge-custom activity-badge mb-2"><?= htmlspecialchars($activity) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No activities listed.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <!-- Mobile Layout - Features & Activities side by side -->
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($features)): ?>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($features as $feature): ?>
                                        <span class="badge badge-custom feature-badge mb-2 small"><?= htmlspecialchars($feature) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-info py-2 small">No features listed.</div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($activities)): ?>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($activities as $activity): ?>
                                        <span class="badge badge-custom activity-badge mb-2 small"><?= htmlspecialchars($activity) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-info py-2 small">No activities listed.</div>
    ...
    <section class="content-section">
        ...
        <!-- Desktop Layout - Best Time & Travel Tips side by side -->
        <div class="row d-none d-md-flex">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($spot['best_time_to_visit'])): ?>
                            <p class="mb-2"><i class="fas fa-calendar-check me-2"></i> <strong>Best Time:</strong> <?= htmlspecialchars($spot['best_time_to_visit']) ?></p>
                        <?php endif; ?>
                        
                        <?php if (!empty($hours)): ?>
                            <h6 class="mt-3 fw-bold">Operating Hours:</h6>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($spot['travel_tips'])): ?>
                            <div class="alert alert-info py-2 small">
                                <?= nl2br(htmlspecialchars($spot['travel_tips'])) ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No travel tips available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <!-- Mobile Layout - Best Time & Travel Tips side by side -->
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">🕒 Best Time</h5>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($spot['best_time_to_visit'])): ?>
                                <p class="mb-2 small"><i class="fas fa-calendar-check me-2"></i> <strong>Best Time:</strong> <?= htmlspecialchars($spot['best_time_to_visit']) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2 small"><strong>📍 Location:</strong> <?= htmlspecialchars($spot['location'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2 small"><strong>🅿️ Parking:</strong> <?= !empty($spot['parking']) ? htmlspecialchars($spot['parking']) : 'Information not available' ?></p>
                                <p class="mb-2 small"><strong>🚻 Restrooms:</strong> <?= !empty($spot['restrooms']) ? htmlspecialchars($spot['restrooms']) : 'Information not available' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($spot['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($spot['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_list)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($spot['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($nearby_list as $place): ?>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($spot['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($spot['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($spot['website'])): ?>
                                <a href="<?= htmlspecialchars($spot['website']) ?>" class="btn btn-primary btn-sm" target="_blank">
