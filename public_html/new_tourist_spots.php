...
$result = $conn->query("SELECT * FROM new_tourist_spots");
$spots_exist = $result->num_rows > 0;
...
<div class="album py-5" id="spots">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">New Tourist Spots in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($spots_exist): ?>
                <?php while ($spot = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($spot['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($spot['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($spot['name']) ?></h6>
                            <?php if (!empty($spot['details_link'])): ?>
                                <a href="<?= htmlspecialchars($spot['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No bar found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
