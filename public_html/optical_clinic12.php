...
$query = "SELECT * FROM optical_clinics WHERE id = 12";
...
if (!empty($clinic['transportation_routes'])) {
    $routes_data = json_decode($clinic['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($clinic['nearby_places'])) {
    $nearby_places = json_decode($clinic['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $clinic['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $clinic['social_media'] ?? '')));

$hours = [];
if (!empty($clinic['hours'])) {
    $hours = explode("\n", $clinic['hours']);
}
...
if (!empty($clinic['specialties'])) {
    $specialties = array_filter(array_map('trim', explode(',', $clinic['specialties'])));
}

$insurance_providers = [];
if (!empty($clinic['insurance'])) {
    $insurance_providers = array_filter(array_map('trim', explode(',', $clinic['insurance'])));
}
...
<head>
    ...
    <title><?= htmlspecialchars($clinic['name']) ?> - Iloilo Optical Services</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($clinic['image'] ?? '') ?>');
        ...
        /* Optical Clinic specific styles */
        .services-list {
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($clinic['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($clinic['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($clinic['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($clinic['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($clinic['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($clinic['name']) ?> Image <?= $index + 1 ?>">
      ...
      <?php endif; ?>
        ...
        <section class="content-section">
        <h2 class="section-title">🔬 Services & Professionals</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🩺 Eye Care Services</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($services)): ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($services as $service): ?>
                                    <span class="badge badge-custom service-badge mb-2"><?= htmlspecialchars($service) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No services information available.</div>
                        <?php endif; ?>
                        
                        <?php if (!empty($clinic['equipment'])): ?>
                            <h6 class="mt-3 fw-bold">🔬 Equipment:</h6>
                            <p class="small"><?= htmlspecialchars($clinic['equipment']) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">👨‍⚕️ Medical Professionals</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($doctors)): ?>
                            <ul class="doctor-list">
                                <?php foreach ($doctors as $doctor): ?>
                                    <li><i class="fas fa-user-md"></i> <?= htmlspecialchars($doctor) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">No doctor information available.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($services)): ?>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($services as $service): ?>
                                        <span class="badge badge-custom service-badge mb-2 small"><?= htmlspecialchars($service) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-info py-2 small">No services available.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <div class="alert alert-info py-2 small">No doctors listed.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card info-card">
                    <div class="card-header">
                        <h5 class="mb-0">💳 Insurance & Payment</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <?php if (!empty($insurance_providers)): ?>
                                    <h6 class="fw-bold">🏥 Accepted Insurance:</h6>
                                    <div class="d-flex flex-wrap">
                                        <?php foreach ($insurance_providers as $provider): ?>
                                            <span class="badge badge-custom insurance-badge mb-2"><?= htmlspecialchars($provider) ?></span>
                                        <?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="col-md-6">
                                <?php if (!empty($clinic['payment_methods'])): ?>
                                    <h6 class="fw-bold">💵 Payment Methods:</h6>
                                    <p class="small"><?= htmlspecialchars($clinic['payment_methods']) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        <h2 class="section-title">👁️ Optical Clinic Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">ℹ️ About</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($specialties)): ?>
                            <h6 class="mt-3 fw-bold">🎯 Specialties:</h6>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($specialties as $specialty): ?>
                                    <span class="badge badge-custom specialty-badge mb-2"><?= htmlspecialchars($specialty) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($clinic['hours'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($clinic['appointment_contact'])): ?>
                            <p class="mb-0"><strong>📅 Appointments:</strong> <?= htmlspecialchars($clinic['appointment_contact']) ?></p>
                        <?php endif; ?>
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($clinic['contact'] ?? '') ?></p>
                            <?php if (!empty($clinic['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($clinic['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($clinic['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($clinic['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">ℹ️ About</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($specialties)): ?>
                                <h6 class="mt-3 fw-bold small">🎯 Specialties:</h6>
                                <div class="d-flex flex-wrap">
                                    <?php foreach ($specialties as $specialty): ?>
                                        <span class="badge badge-custom specialty-badge mb-2 small"><?= htmlspecialchars($specialty) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                        </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <?php if (!empty($clinic['appointment_contact'])): ?>
                                <p class="mb-0 small"><strong>📅 Appointments:</strong> <?= htmlspecialchars($clinic['appointment_contact']) ?></p>
                            <?php endif; ?>
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($clinic['contact'] ?? '') ?></p>
                            <?php if (!empty($clinic['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($clinic['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($clinic['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($clinic['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($clinic['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($clinic['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($clinic['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($nearby_places as $place): ?>
                                    <span class="badge badge-custom"><?= htmlspecialchars($place) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($clinic['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($clinic['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($clinic['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($clinic['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($clinic['online_booking'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($clinic['online_booking']) ?>" class="btn btn-dark btn-sm" target="_blank">
                                        <i class="fas fa-calendar-alt me-1"></i> Book Online
                                    </a>
