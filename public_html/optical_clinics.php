...
$result = $conn->query("SELECT * FROM optical_clinics");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'optical_clinics') {
...
$query = "SELECT * FROM optical_clinics";
$optical_clinics_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $optical_clinics_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="optical_clinics">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Optical Clinics in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($optical_clinics_exist): ?>
                <?php while ($optical_clinic = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($optical_clinic['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($optical_clinic['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($optical_clinic['name']) ?></h6>
                            <?php if (!empty($optical_clinic['details_link'])): ?>
                                <a href="<?= htmlspecialchars($optical_clinic['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No optical clinic found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
