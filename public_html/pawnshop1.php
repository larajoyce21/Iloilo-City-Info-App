...
$query = "SELECT * FROM pawnshops WHERE id = 1";
...
if (!empty($pawnshop['transportation_routes'])) {
    $routes_data = json_decode($pawnshop['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($pawnshop['nearby_places'])) {
    $decoded = json_decode($pawnshop['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $pawnshop['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $pawnshop['social_media'] ?? '')));

$hours = [];
if (!empty($pawnshop['hours'])) {
    $hours = explode("\n", $pawnshop['hours']);
}

$locations = array_filter(array_map('trim', explode('|', $pawnshop['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($pawnshop['name']) ?> - Iloilo Pawnshops</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($pawnshop['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($pawnshop['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($pawnshop['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($pawnshop['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($pawnshop['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($pawnshop['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($pawnshop['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Pawnshop Information</h2>
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($pawnshop['services'])): ?>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💎 Items Accepted</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($pawnshop['items_accepted'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">💎 Items Accepted</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($pawnshop['items_accepted'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($pawnshop['interest_rates'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">📈 Interest Rates</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($pawnshop['interest_rates'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($pawnshop['contact'] ?? '') ?></p>
                            <?php if (!empty($pawnshop['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($pawnshop['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pawnshop['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($pawnshop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($pawnshop['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="row">
                                <?php if (!empty($pawnshop['interest_rates'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">📈 Interest Rates</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($pawnshop['interest_rates'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($pawnshop['contact'] ?? '') ?></p>
                            <?php if (!empty($pawnshop['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($pawnshop['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pawnshop['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($pawnshop['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($pawnshop['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $pawnshop['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($pawnshop['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏢 Type:</strong> <?= htmlspecialchars($pawnshop['type'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🔒 Security:</strong> <?= htmlspecialchars($pawnshop['security_features'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($pawnshop['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏢 Type:</strong> <?= htmlspecialchars($pawnshop['type'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🔒 Security:</strong> <?= htmlspecialchars($pawnshop['security_features'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($pawnshop['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($pawnshop['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($pawnshop['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($pawnshop['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($pawnshop['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($pawnshop['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($pawnshop['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($pawnshop['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($pawnshop['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
