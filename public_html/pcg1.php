...
$query = "SELECT * FROM pcg_details WHERE id = 1";
$stmt = $conn->prepare($query);
$stmt->execute();
$result = $stmt->get_result();
$pcg = $result->fetch_assoc();

$transport_routes = array();
if (!empty($pcg['transportation_routes'])) {
    $routes_data = json_decode($pcg['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($pcg['nearby_places'])) {
    $decoded = json_decode($pcg['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $pcg['image_detail'] ?? ''));
...
$locations = array_filter(array_map('trim', explode('|', $pcg['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($pcg['name']) ?> - Philippine Coast Guard</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($pcg['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($pcg['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($pcg['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($pcg['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($pcg['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($pcg['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($pcg['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">PCG Station Information</h2>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🚨 Emergency Hotlines</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($pcg['hotlines'])): ?>
                            <div class="emergency-hotlines">
                                <?php
                                $hotlines = explode("\n", $pcg['hotlines']);
                                foreach ($hotlines as $hotline):
                                    if (trim($hotline) != ''): ?>
                                        <div class="hotline-item mb-2">
                                            <i class="fas fa-phone-alt text-danger me-2"></i>
                                            <span class="fw-bold"><?= htmlspecialchars(trim($hotline)) ?></span>
                                        </div>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                        <?php else: ?>
                            <p class="mb-0 text-muted">Emergency hotlines not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <p class="mb-0 text-muted">No services information available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">🚨 Emergency Hotlines</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($pcg['hotlines'])): ?>
                                <div class="emergency-hotlines">
                                    ...
                                    foreach ($hotlines as $hotline):
                                        if (trim($hotline) != ''): ?>
                                            <div class="hotline-item mb-2">
                                                <i class="fas fa-phone-alt text-danger me-2 small"></i>
                                                <span class="fw-bold small"><?= htmlspecialchars(trim($hotline)) ?></span>
                                            </div>
                                        <?php endif;
                                    endforeach; ?>
                                </div>
                            <?php else: ?>
                                <p class="mb-0 text-muted small">Emergency hotlines not available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <p class="mb-0 text-muted small">No services information available.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Contact Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($pcg['contact'] ?? '') ?></p>
                            <?php if (!empty($pcg['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($pcg['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pcg['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($pcg['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                      <?= nl2br(htmlspecialchars($pcg['office_hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Contact Details</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($pcg['contact'] ?? '') ?></p>
                            <?php if (!empty($pcg['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($pcg['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pcg['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($pcg['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($pcg['requirements'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $req_count = 0;
                                foreach (explode("\n", $pcg['requirements']) as $requirement):
                                    if (trim($requirement) != ''): 
                                        $req_count++;
                                        ?>
                                        <div class="list-group-item border-0 px-0 py-1">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <span class="small"><?= htmlspecialchars(trim($requirement)) ?></span>
                                            </div>
                                        </div>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                            <p class="text-muted small mt-2">Total: <?= $req_count ?> requirements</p>
                        <?php else: ?>
                            <p class="text-muted small mb-0">Requirements information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Station Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>⚓ Station Type:</strong> <?= htmlspecialchars($pcg['station_type'] ?? '') ?></p>
                                <p class="mb-2"><strong>🚢 Vessels:</strong> <?= htmlspecialchars($pcg['vessels'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🧑‍✈️ Personnel:</strong> <?= htmlspecialchars($pcg['personnel'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-clipboard-list me-2"></i> Requirements</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($pcg['requirements'])): ?>
                                <div class="list-group list-group-flush">
                                ...
                                <p class="text-muted small mt-1">Total: <?= $req_count ?> requirements</p>
                            <?php else: ?>
                                <p class="text-muted small mb-0">Requirements information not available.</p>
                            <?php endif; ?>
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Station Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>⚓ Station Type:</strong> <?= htmlspecialchars($pcg['station_type'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🚢 Vessels:</strong> <?= htmlspecialchars($pcg['vessels'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🧑‍✈️ Personnel:</strong> <?= htmlspecialchars($pcg['personnel'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🕒 Last Updated:</strong> <?= !empty($pcg['last_updated']) ? date('F j, Y', strtotime($pcg['last_updated'])) : '' ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($pcg['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($pcg['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($pcg['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($pcg['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($pcg['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($pcg['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($pcg['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        ...
                        $transport_types = array(
                            ...
                            'boat' => array('icon' => 'fas fa-ship', 'title' => 'Boat')
                        );
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($pcg['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($pcg['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
...
function toggleTransport(type) {
    ...
    const allTypes = ['jeepney', 'bus', 'taxi', 'tricycle', 'boat'];
