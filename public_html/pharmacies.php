...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'pharmacies') {
...
$pharmacies_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $pharmacies_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="pharmacies">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Pharmacies and Drugstores in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($pharmacies_exist): ?>
                <?php while ($pharmacy = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($pharmacy['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($pharmacy['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($pharmacy['name']) ?></h6>
                            <?php if (!empty($pharmacy['details_link'])): ?>
                                <a href="<?= htmlspecialchars($pharmacy['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
