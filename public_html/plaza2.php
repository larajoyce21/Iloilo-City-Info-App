...
$query = "SELECT * FROM plaza WHERE id = 2";
...
if (!empty($plaza['transportation_routes'])) {
    $routes_data = json_decode($plaza['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($plaza['nearby_places'])) {
    $decoded = json_decode($plaza['nearby_places'], true);
    $nearby_places = is_array($decoded) ? $decoded : [];
}

$detail_images = array_filter(explode(',', $plaza['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $plaza['social_media'] ?? '')));

$hours = [];
if (!empty($plaza['hours'])) {
    $lines = explode("\n", $plaza['hours']);
...
$locations = array_filter(array_map('trim', explode('|', $plaza['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($plaza['name']) ?> - Iloilo Plazas</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($plaza['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($plaza['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($plaza['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($plaza['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($plaza['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($plaza['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($plaza['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Plaza Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📜 Historical Background</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($plaza['history'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($plaza['significance'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">🎭 Features & Events</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-2">
                            <strong>🏛️ Main Feature:</strong> <?= htmlspecialchars($plaza['main_feature'] ?? '') ?>
                        </div>
                        <div class="mb-2">
                            <strong>📅 Events:</strong> <?= htmlspecialchars($plaza['events'] ?? '') ?>
                        </div>
                        <div>
                            <strong>♿ Accessibility:</strong> <?= htmlspecialchars($plaza['accessibility'] ?? '') ?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">📜 Historical Background</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($plaza['history'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($plaza['significance'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">🎭 Features & Events</h6>
                        </div>
                        <div class="card-body">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($plaza['contact'] ?? '') ?></p>
                            <?php if (!empty($plaza['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($plaza['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($plaza['website'])): ?>
                                <p class="mb-2 ">🌐 Website: <a href="<?= htmlspecialchars($plaza['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💡 Plaza Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🗓️ Established:</strong> <?= htmlspecialchars($plaza['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>🏛️ Main Feature:</strong> <?= htmlspecialchars($plaza['main_feature'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Events:</strong> <?= htmlspecialchars($plaza['events'] ?? '') ?></p>
                                <p class="mb-2"><strong>♿ Accessibility:</strong> <?= htmlspecialchars($plaza['accessibility'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($plaza['contact'] ?? '') ?></p>
                            <?php if (!empty($plaza['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($plaza['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($plaza['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($plaza['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">💡 Plaza Facts</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🗓️ Established:</strong> <?= htmlspecialchars($plaza['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏛️ Main Feature:</strong> <?= htmlspecialchars($plaza['main_feature'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>📅 Events:</strong> <?= htmlspecialchars($plaza['events'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>♿ Accessibility:</strong> <?= htmlspecialchars($plaza['accessibility'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($plaza['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars($plaza['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($plaza['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($plaza['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($plaza['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($plaza['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($plaza['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
