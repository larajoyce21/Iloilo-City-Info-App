...
$query = "SELECT * FROM pnps WHERE id = 13";
$stmt = $conn->prepare($query);
$stmt->execute();
$result = $stmt->get_result();
$pnp = $result->fetch_assoc();

$transport_routes = array();
if (!empty($pnp['transportation_routes'])) {
    $routes_data = json_decode($pnp['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($pnp['nearby_places'])) {
    $nearby_places = json_decode($pnp['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $pnp['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $pnp['social_media'] ?? '')));

$office_hours = [];
if (!empty($pnp['office_hours'])) {
    $office_hours = explode("\n", $pnp['office_hours']);
}

$locations = array_filter(array_map('trim', explode('|', $pnp['location'] ?? '')));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($pnp['name'] ?? 'PNP Station') ?> - Iloilo City</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($pnp['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($pnp['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($pnp['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($pnp['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($pnp['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($pnp['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($pnp['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">👮 PNP Station Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($pnp['services'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap">
                            <?php 
                            $hotlines = !empty($pnp['hotlines']) ? explode("\n", $pnp['hotlines']) : [];
                            foreach ($hotlines as $hotline):
                                if (!empty(trim($hotline))):
                            ?>
                                <span class="emergency-badge"><?= htmlspecialchars(trim($hotline)) ?></span>
                            <?php 
                                endif;
                            endforeach; 
                            ?>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">💵 Service Fees</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($pnp['fees'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($pnp['services'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">💵 Service Fees</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($pnp['fees'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($pnp['hotlines'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">🚨 Emergency Hotlines</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($pnp['hotlines'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($pnp['contact'] ?? '') ?></p>
                            <?php if (!empty($pnp['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($pnp['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pnp['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($pnp['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($pnp['office_hours'])) ?>
                            ...
                            <div class="row">
                                <?php if (!empty($pnp['hotlines'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🚨 Emergency Hotlines</h6>
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($pnp['hotlines'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($pnp['contact'] ?? '') ?></p>
                            <?php if (!empty($pnp['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($pnp['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($pnp['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($pnp['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12 mb-3">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🗓️ Established:</strong> <?= htmlspecialchars($pnp['established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Jurisdiction:</strong> <?= htmlspecialchars($pnp['jurisdiction'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🕒 Last Updated:</strong> <?= htmlspecialchars(date("F j, Y", strtotime($pnp['last_updated'] ?? ''))) ?></p>
                                <p class="mb-2"><strong>👮 Officer in Charge:</strong> <?= htmlspecialchars($pnp['officer_in_charge'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>🗓️ Established:</strong> <?= htmlspecialchars($pnp['established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Jurisdiction:</strong> <?= htmlspecialchars($pnp['jurisdiction'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🕒 Last Updated:</strong> <?= htmlspecialchars(date("F j, Y", strtotime($pnp['last_updated'] ?? ''))) ?></p>
                                    <p class="mb-1 small"><strong>👮 Officer in Charge:</strong> <?= htmlspecialchars($pnp['officer_in_charge'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($pnp['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars($pnp['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($pnp['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($pnp['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($pnp['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($pnp['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($pnp['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($pnp['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($pnp['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
