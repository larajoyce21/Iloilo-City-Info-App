...
if (isset($_GET['search'])) {
    ...
    if ($_SESSION['search_trigger']['table'] === 'resorts') {
...
if ($stmt = $conn->prepare($query)) {
    ...
    $resorts_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="resorts">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Beaches and Resorts in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($resorts_exist): ?>
                <?php while ($resort = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($resort['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($resort['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($resort['name']) ?></h6>
                            <?php if (!empty($resort['details_link'])): ?>
                                <a href="<?= htmlspecialchars($resort['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No resort found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
