...
$result = $conn->query("SELECT * FROM review_centers");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'review_centers') {
...
$query = "SELECT * FROM review_centers";
$review_centers_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $review_centers_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>REVIEW AND TRAINING CENTERS</title>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Review and Training Centers in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($review_centers_exist): ?>
                <?php while ($review_center = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($review_center['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($review_center['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($review_center['name']) ?></h6>
                            <?php if (!empty($review_center['details_link'])): ?>
                                <a href="<?= htmlspecialchars($review_center['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No review center found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
