...
$query = "SELECT * FROM samgyupsal_restaurants WHERE id = 5";
...
if (!empty($restaurant['transportation_routes'])) {
    $routes_data = json_decode($restaurant['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($restaurant['nearby_places'])) {
    $nearby_places = json_decode($restaurant['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $restaurant['image_detail'] ?? ''));
...
<head>
    ...
    <title><?= htmlspecialchars($restaurant['name'] ?? 'Samgyupsal Restaurant') ?> - Iloilo Samgyupsal Restaurants</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($restaurant['image'] ?? '') ?>');
        ...
        /* Transport Options - Similar to apartment1.php */
        .transport-option {
            margin-bottom: 10px;
        }
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($restaurant['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($restaurant['name'] ?? 'Samgyupsal Restaurant') ?></h1>
    <p class="lead"><?= htmlspecialchars($restaurant['location'] ?? 'Restaurant Location') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($restaurant['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($restaurant['name'] ?? '') ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🍽️ Menu & Pricing</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-utensils me-2"></i> Samgyupsal Sets</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($restaurant['menu_sets'])): ?>
                            <ul class="menu-list">
                                <?php
                                $menu_sets = explode("\n", $restaurant['menu_sets']);
                                foreach ($menu_sets as $set):
                                    if (trim($set) != ''): 
                                        $parts = explode('|', $set);
                                        $item_name = trim($parts[0]);
                                        $item_price = isset($parts[1]) ? trim($parts[1]) : '';
                                        ?>
                                        <li class="d-flex justify-content-between align-items-center">
                                            ...
                                            <?php if (!empty($item_price)): ?>
                                                <span class="price-tag"><?= htmlspecialchars($item_price) ?></span>
                                            <?php endif; ?>
                                        </li>
                                    <?php endif;
                                endforeach; ?>
                        ...
                        <?php else: ?>
                            <p class="text-muted small">Menu information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-concierge-bell me-2"></i> Additional Options</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <h6 class="fw-bold small">🥩 Meat Selection</h6>
                            <p class="small"><?= !empty($restaurant['meat_selection']) ? htmlspecialchars($restaurant['meat_selection']) : '' ?></p>
                        </div>
                        
                        <div class="mb-3">
                            <h6 class="fw-bold small">🥗 Side Dishes</h6>
                            <p class="small"><?= !empty($restaurant['side_dishes']) ? htmlspecialchars($restaurant['side_dishes']) : '' ?></p>
                        </div>
                        
                        <div>
                            <h6 class="fw-bold small">👥 Group Packages</h6>
                            <p class="small"><?= !empty($restaurant['group_packages']) ? htmlspecialchars($restaurant['group_packages']) : '' ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-utensils me-2"></i> Samgyupsal Sets</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($restaurant['menu_sets'])): ?>
                                <ul class="menu-list">
                                    ...
                                    foreach ($menu_sets as $set):
                                        if (trim($set) != ''): 
                                            ...
                                            <li class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <i class="fas fa-check-circle text-success"></i>
                                                    <span class="small"><?= htmlspecialchars($item_name) ?></span>
                                                </div>
                                                <?php if (!empty($item_price)): ?>
                                                    <span class="price-tag small"><?= htmlspecialchars($item_price) ?></span>
                                                <?php endif; ?>
                                            </li>
                                        <?php endif;
                                    endforeach; ?>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-concierge-bell me-2"></i> Additional Options</h6>
                        </div>
                        <div class="card-body">
                            <div class="mb-2">
                                <h6 class="fw-bold small">🥩 Meat Selection</h6>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-star me-2"></i> Restaurant Features</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                <h6 class="fw-bold small">🔥 Cooking Style</h6>
                                <p class="small"><?= !empty($restaurant['cooking_style']) ? htmlspecialchars($restaurant['cooking_style']) : '' ?></p>
                            </div>
                            
                            <div class="col-md-6 mb-2">
                                <h6 class="fw-bold small">♾️ Service Type</h6>
                                <p class="small"><?= !empty($restaurant['service_type']) ? htmlspecialchars($restaurant['service_type']) : '' ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Other Branches</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($restaurant['branches'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $branch_count = 0;
                                foreach (explode("\n", $restaurant['branches']) as $branch):
                                    if (trim($branch) != ''): 
                                        $branch_count++;
                                        ?>
                            ...
                            <p class="text-muted small mt-2">Total: <?= $branch_count ?> branches</p>
                        <?php else: ?>
                            <p class="text-muted small">Branch information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-star me-2"></i> Restaurant Features</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-code-branch me-2"></i> Other Branches</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($restaurant['branches'])): ?>
                                <div class="list-group list-group-flush">
                                ...
                                <p class="text-muted small mt-1">Total: <?= $branch_count ?> branches</p>
                            <?php else: ?>
                                <p class="text-muted small">Branch information not available.</p>
                            <?php endif; ?>
                        </div>
        ...
        <div class="row d-none d-md-flex">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                             ...
                             <?= nl2br(htmlspecialchars($restaurant['hours'])) ?>
            ...
            <div class="col-lg-5 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($restaurant['contact'] ?? '') ?></p>
                            <?php if (!empty($restaurant['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($restaurant['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($restaurant['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($restaurant['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4 mb-3">
                <div class="card info-card h-100 quick-facts-card">
                    <div class="card-header">
                        <h5 class="mb-0">⭐ Quick Facts</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-2">
                            <h6 class="fw-bold small">🏪 Restaurant Type</h6>
                            <p class="small"><?= !empty($restaurant['type']) ? htmlspecialchars($restaurant['type']) : '' ?></p>
                        </div>
                        
                        <div class="mb-2">
                            <h6 class="fw-bold small">📅 Established</h6>
                            <p class="small"><?= !empty($restaurant['established']) ? htmlspecialchars($restaurant['established']) : '' ?></p>
                        </div>
                            <div class="col-md-6 mb-2">                                
                                <h6 class="fw-bold">👥 Capacity</h6>
                                <p class="mb-2 small"><?= !empty($restaurant['capacity']) ? htmlspecialchars($restaurant['capacity']) : '' ?></p>
                            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($restaurant['contact'] ?? '') ?></p>
                            <?php if (!empty($restaurant['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($restaurant['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($restaurant['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($restaurant['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($restaurant['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($restaurant['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($restaurant['name'] ?? 'this restaurant') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($restaurant['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($restaurant['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($restaurant['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($restaurant['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($restaurant['food_delivery'])): ?>
                                <hr class="my-3">
                                <h5 class="mb-2 small fw-bold">🍽️ Food Delivery</h5>
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($restaurant['food_delivery']) ?>" class="btn btn-danger btn-sm" target="_blank">
                                        <i class="fas fa-utensils me-1"></i> Order Online
                                    </a>
                                    <a href="#" class="btn btn-outline-danger btn-sm" target="_blank">
                                        <i class="fas fa-info-circle me-1"></i> Delivery Info
                                    </a>
                                </div>
                            <?php endif; ?>
                            ...
                            <h5 class="mb-2 small fw-bold">❓ Need Help?</h5>
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($restaurant['contact'])): ?>
                                    <a href="tel:<?= htmlspecialchars(preg_replace('/[^0-9+]/', '', $restaurant['contact'])) ?>" class="btn btn-outline-primary btn-sm">
                                        <i class="fas fa-phone me-1"></i> Call Restaurant
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($restaurant['email'])): ?>
                                    <a href="mailto:<?= htmlspecialchars($restaurant['email']) ?>" class="btn btn-outline-secondary btn-sm">
