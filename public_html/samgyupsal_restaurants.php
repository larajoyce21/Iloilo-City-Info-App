...
$result = $conn->query("SELECT * FROM samgyupsal_restaurants");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'samgyupsal_restaurants') {
...
$query = "SELECT * FROM samgyupsal_restaurants";
$samgyupsal_restaurants_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $samgyupsal_restaurants_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<html lang="en">
  <head>
    ...
    <title>SAMGYUPSAL RESTAURANTS</title>
...
<div class="album py-5" id="salons">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Samgyupsal Restaurants in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($samgyupsal_restaurants_exist): ?>
                <?php while ($samgyupsal_restaurant = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($samgyupsal_restaurant['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($samgyupsal_restaurant['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($samgyupsal_restaurant['name']) ?></h6>
                            <?php if (!empty($samgyupsal_restaurant['details_link'])): ?>
                                <a href="<?= htmlspecialchars($samgyupsal_restaurant['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No samgyupsal restaurant  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
