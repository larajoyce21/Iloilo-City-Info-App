...
$query = "SELECT * FROM street_foods WHERE id = 1"; 
...
if (!empty($food['transportation_routes'])) {
    $routes_data = json_decode($food['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($food['nearby_places'])) {
    $nearby_places = json_decode($food['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $food['image_detail'] ?? ''));
...
if (!empty($food['hours'])) {
    ...
    foreach ($lines as $line) {
        if (strpos($line, ':') !== false) {
            ...
            $availability[trim($day)] = array_map('trim', explode(',', $times));
        }
    }
}
?>
...
<head>
    ...
    <title><?= htmlspecialchars($food['name'] ?? 'Famous Street Food') ?> - Iloilo Street Foods</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($food['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($food['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($food['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($food['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($food['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($food['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">📌 Food Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📜 Food Background</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($food['history'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($food['significance'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2 small"><strong>🌍 Origin:</strong> <?= htmlspecialchars($food['origin'] ?? 'Traditional Ilonggo street food') ?></p>
                                <p class="mb-2 small"><strong>⭐ Popularity:</strong> <?= htmlspecialchars($food['popularity'] ?? 'Local favorite') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2 small"><strong>🕒 Best Time to Eat:</strong> <?= htmlspecialchars($food['best_time'] ?? 'Afternoon or evening') ?></p>
                                <p class="mb-2 small"><strong>💰 Average Price:</strong> <?= htmlspecialchars($food['average_price'] ?? 'Affordable') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Mobile Layout - Food Background & Cultural Significance side by side -->
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($food['history'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($food['significance'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Quick Facts for mobile (full width) -->
        <div class="d-md-none">
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-6">
                                    <p class="mb-1 small"><strong>🌍 Origin:</strong> <?= htmlspecialchars($food['origin'] ?? 'Traditional Ilonggo street food') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Popularity:</strong> <?= htmlspecialchars($food['popularity'] ?? 'Local favorite') ?></p>
                                </div>
                                <div class="col-6">
                                    <p class="mb-1 small"><strong>🕒 Best Time:</strong> <?= htmlspecialchars($food['best_time'] ?? 'Afternoon or evening') ?></p>
                                    <p class="mb-1 small"><strong>💰 Price:</strong> <?= htmlspecialchars($food['average_price'] ?? 'Affordable') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <!-- Desktop Layout - Food Details & Availability Schedule side by side -->
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Food Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($food['price'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Price Range</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($food['price'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($food['contact']) || !empty($food['email'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">📞 Contact</h6>
                                    <p class="mb-2">
                                        <?php if (!empty($food['contact'])): ?>
                                            <?= htmlspecialchars($food['contact']) ?><br>
                                        <?php endif; ?>
                                        <?php if (!empty($food['email'])): ?>
                            ...
                            <?php if (!empty($food['website'])): ?>
                                <div class="col-12 mb-2">
                                    <h6 class="fw-bold">🌐 Website</h6>
                                    <p class="mb-2"><a href="<?= htmlspecialchars($food['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($availability)): ?>
                            <div class="availability-schedule">
                                ...
                                foreach ($availability as $day => $times): 
                                    $is_today = (strtolower($day) === strtolower($today));
                                ?>
                                    <div class="availability-day mb-2 <?= $is_today ? 'today' : '' ?>">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info mb-0 py-2 small">No schedule available.</div>
                        <?php endif; ?>
                        <p class="mt-2 text-muted small"><em>Availability may change during holidays and special occasions</em></p>
                    </div>
                </div>
            </div>
        </div>

        <!-- Mobile Layout - Food Details & Availability Schedule side by side -->
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                            ...
                            <div class="row">
                                <?php if (!empty($food['price'])): ?>
                                    <div class="col-12 mb-2">
                                        ...
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($food['price'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($food['website'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🌐 Website</h6>
                                        <p class="mb-2 small"><a href="<?= htmlspecialchars($food['website']) ?>" target="_blank" class="text-decoration-none">Visit Website</a></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <div class="alert alert-info mb-0 py-2 small">No schedule available.</div>
                            <?php endif; ?>
                            <p class="mt-1 text-muted small"><em>Availability may change during holidays</em></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($food['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($food['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($food['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($food['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($food['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($food['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($food['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="card info-card">
                    ...
                    <div class="card-body">
                        <?php 
                        // Define transport types and their icons (similar to apartment1.php)
                        ...
                        // Display each transport type (similar to apartment1.php)
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($food['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($food['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
...
// Transportation Dropdown Functions - Same as apartment1.php
function toggleTransport(type) {
    ...
    // Toggle route list visibility
    routeList.classList.toggle('active');
    
    // Close other open route lists
