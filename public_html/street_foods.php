...
$result = $conn->query("SELECT * FROM street_foods");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'street_foods') {
...
$query = "SELECT *FROM street_foods";
$street_foods_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $street_foods_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="street_foods">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Iloilo City Famous Street Foods</h1> 
        
        <div class="card-container">
            <?php if ($street_foods_exist): ?>
                <?php while ($street_food = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($street_food['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($street_food['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($street_food['name']) ?></h6>
                            <?php if (!empty($street_food['details_link'])): ?>
                                <a href="<?= htmlspecialchars($street_food['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No streetfood  found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
