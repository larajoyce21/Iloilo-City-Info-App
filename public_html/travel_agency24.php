...
$query = "SELECT * FROM travel_agencies WHERE id = 24";
...
$packages = [];
if (!empty($agency['packages'])) {
    $packages = json_decode($agency['packages'], true) ?: [];
}

$detail_images = array_filter(explode(',', $agency['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $agency['social_media'] ?? '')));

$nearby_places = [];
if (!empty($agency['nearby_places'])) {
    $nearby_places = json_decode($agency['nearby_places'], true) ?: [];
}

$payment_options = [];
if (!empty($agency['payment_options'])) {
    $payment_options = json_decode($agency['payment_options'], true) ?: [];
}

$services = [];
if (!empty($agency['services'])) {
    $services = json_decode($agency['services'], true) ?: [];
}

$requirements = [];
if (!empty($agency['requirements'])) {
    $requirements = json_decode($agency['requirements'], true) ?: [];
}

$safety_measures = [];
if (!empty($agency['safety_measures'])) {
    $safety_measures = json_decode($agency['safety_measures'], true) ?: [];
}
...
if (!empty($agency['hours'])) {
    $hours = json_decode($agency['hours'], true) ?: [];
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= $agency['name'] ?? 'Travel Agency' ?> - Iloilo Travel Services</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= $agency['image'] ?? '' ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= $agency['image'] ?? '' ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= $agency['name'] ?? 'Iloilo Travel Agency' ?></h1>
    <p class="lead"><?= $agency['location'] ?? 'Iloilo City, Philippines' ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= $agency['name'] ?? '' ?></h2>
      <div class="lead mb-4">
        <?= nl2br($agency['description'] ?? '') ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= $image ?>" alt="<?= $agency['name'] ?> Image <?= $index + 1 ?>">
                        ...
                        <?php if (!empty($services)): ?>
                            <ul class="list-group list-group-flush">
                                <?php foreach ($services as $service): ?>
                                    <li class="list-group-item border-0 px-0 py-1 small">✔️ <?= trim($service) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                                ...
                                <?php foreach ($requirements as $req): ?>
                                    <div class="col-md-6 mb-2">
                                        <div class="d-flex align-items-start">
                                            ...
                                            <span class="small"><?= $req ?></span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">Booking requirements not specified</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                    <div class="card-header">
                        <h6 class="mb-0">📅 Booking Requirements</h6>
                    </div>
                    <div class="card-body small">
                        <?php if (!empty($requirements)): ?>
                            <div class="row">
    ...
    <section class="content-section">
            ...
            <div class="row d-none d-md-flex">
                <?php foreach ($packages as $package): ?>
                    <div class="col-lg-4 mb-3">
                        <div class="card info-card h-100 package-card">
                            <div class="card-header">
                                <h5 class="mb-0"><?= $package['title'] ?></h5>
                            </div>
                            <div class="card-body">
                                <p class="small"><?= nl2br($package['description']) ?></p>
                                <p class="mb-1 small"><strong>💵 Price:</strong> <?= $package['price'] ?></p>
                                <p class="mb-0 small"><strong>🕒 Duration:</strong> <?= $package['duration'] ?></p>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            <div class="alert alert-info py-2 small">No packages available at this time.</div>
        <?php endif; ?>
    </section>

    <section class="content-section">
        <h2 class="section-title">ℹ️ Important Details</h2>
                    ...
                    <div class="card-body">
                        <?php if (!empty($hours)): ?>
                            <?php foreach ($hours as $day => $time): ?>
                                <p class="mb-1 small"><strong><?= $day ?>:</strong> <?= $time ?></p>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">Operating hours not specified</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                        ...
                        <?php if (!empty($payment_options)): ?>
                            <div class="d-flex flex-wrap gap-1">
                                <?php foreach ($payment_options as $option): ?>
                                    <span class="badge bg-success small"><?= $option ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">Payment options not specified</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($agency['cancellation_policy'])): ?>
                            <p class="small"><?= nl2br($agency['cancellation_policy']) ?></p>
                        <?php else: ?>
                            <div class="alert alert-info py-2 small">Cancellation policy not specified</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-2 small">📲 Phone: <?= $agency['contact'] ?? '' ?></p>
            ...
            <div class="col-lg-8 mb-3">
                <?php if (!empty($agency['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= convertToEmbedURL($agency['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= $agency['name'] ?? '' ?>:</p>
