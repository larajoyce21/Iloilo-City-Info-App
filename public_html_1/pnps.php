...
$result = $conn->query("SELECT * FROM pnps");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'pnps') {
...
$query = "SELECT * FROM pnps";
$pnps_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $pnps_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="pnps">
    <div class="container">
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Philippine National Police Stations in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($pnps_exist): ?>
                <?php while ($pnp = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($pnp['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($pnp['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($pnp['name']) ?></h6>
                            <?php if (!empty($pnp['details_link'])): ?>
                                <a href="<?= htmlspecialchars($pnp['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
