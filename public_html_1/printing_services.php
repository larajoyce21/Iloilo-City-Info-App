...
$result = $conn->query("SELECT * FROM printing_services");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'printing_services') {
...
$printing_services_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $printing_services_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="resorts">
    <div class="container">
        ...
        <br>
        <br>
        <h1 class="page-title text-light text-center">Printing Services in Iloilo City </h1> 
        
        <div class="card-container">
            <?php if ($printing_services_exist): ?>
                <?php while ($printing_service = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($printing_service['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($printing_service['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($printing_service['name']) ?></h6>
                            <?php if (!empty($printing_service['details_link'])): ?>
                                <a href="<?= htmlspecialchars($printing_service['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No printing service found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
