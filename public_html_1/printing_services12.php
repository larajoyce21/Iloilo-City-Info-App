...
$query = "SELECT * FROM printing_services WHERE id = 12";
...
if (!empty($printing['transportation_routes'])) {
    $routes_data = json_decode($printing['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($printing['nearby_places'])) {
    $nearby_places = json_decode($printing['nearby_places'], true);
...
if (!empty($printing['image_detail'])) {
    $detail_images = explode(',', $printing['image_detail']);
}

$social_links = array_filter(array_map('trim', explode("\n", $printing['social_media'] ?? '')));

$services_offered = [];
if (!empty($printing['services_offered'])) {
    $services_offered = json_decode($printing['services_offered'], true);
...
$hours = [];
if (!empty($printing['hours'])) {
    $hours = explode("\n", $printing['hours']);
}

$locations = array_filter(array_map('trim', explode('|', $printing['location'] ?? '')));
...
<head>
    ...
    <title><?= htmlspecialchars($printing['name']) ?> - Iloilo Printing Services</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($printing['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($printing['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($printing['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($printing['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($printing['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($printing['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($printing['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🖨️ Printing Service Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($printing['price_range'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Price Range</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($printing['price_range'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($printing['contact'] ?? '') ?></p>
                            <?php if (!empty($printing['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($printing['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($printing['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($printing['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                             <?= nl2br(htmlspecialchars($printing['hours'])) ?>
                            ...
                            <div class="row">
                                <?php if (!empty($printing['price_range'])): ?>
                                    <div class="col-12 mb-2">
                                        ...
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($printing['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($printing['contact'] ?? '') ?></p>
                                <?php if (!empty($printing['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($printing['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($printing['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($printing['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Services Offered</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($services_offered)): ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($services_offered as $service): ?>
                                    <span class="service-badge"><?= htmlspecialchars($service) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <p class="text-muted small mb-0">Service information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($printing['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($printing['specialty'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>💵 Price Range:</strong> <?= htmlspecialchars($printing['price_range'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-code-branch me-2"></i> Services</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($services_offered)): ?>
                                <div class="d-flex flex-wrap">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($printing['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($printing['specialty'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>💵 Price Range:</strong> <?= htmlspecialchars($printing['price_range'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($printing['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars($printing['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($printing['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($printing['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($printing['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($printing['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($printing['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($printing['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($printing['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
