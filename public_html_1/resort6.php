...
$query = "SELECT * FROM resorts WHERE id = 6";
...
if (!empty($resort['transportation_routes'])) {
    $routes_data = json_decode($resort['transportation_routes'], true);
...
$detail_images = array_filter(explode(',', $resort['image_detail']));
$social_links = array_filter(array_map('trim', explode("\n", $resort['social_media'] ?? '')));

$nearby_places = !empty($resort['nearby_places']) ? json_decode($resort['nearby_places'], true) : [];

$hours = [];
if (!empty($resort['hours'])) {
    $lines = explode("\n", $resort['hours']);
...
<head>
    ...
    <title><?= htmlspecialchars($resort['name']) ?> - Iloilo Resorts</title>
    ...
    <style>
       ...
       /* Navbar */
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($resort['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($resort['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($resort['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($resort['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($resort['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($resort['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">🏊 Resort Features & Amenities</h2>
        ...
        <div class="row d-none d-md-flex">
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($resort['dining_options'])): ?>
                            <?php $dining_options = explode(',', $resort['dining_options']); ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($dining_options as $option): ?>
                                    <span class="amenity-badge"><?= trim(htmlspecialchars($option)) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📶 General Amenities</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($resort['amenities'])): ?>
                            <?php $amenities = explode(',', $resort['amenities']); ?>
                            <div class="d-flex flex-wrap">
                                <?php foreach ($amenities as $amenity): ?>
                                    <span class="amenity-badge"><?= trim(htmlspecialchars($amenity)) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        <div class="card-header">
                            <h6 class="mb-0">📶 General Amenities</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($resort['amenities'])): ?>
                                <?php $amenities = explode(',', $resort['amenities']); ?>
    ...
    <section class="content-section">
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Pricing & Contact</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($resort['entrance_fee'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Entrance Fee</h6>
                                    <p class="mb-2 price-highlight"><?= htmlspecialchars($resort['entrance_fee']) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <?php if (!empty($resort['room_rates'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">🏨 Room Rates</h6>
                                    <p class="mb-2"><?= htmlspecialchars($resort['room_rates']) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($resort['contact'] ?? '') ?></p>
                            <?php if (!empty($resort['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($resort['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($resort['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($resort['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                      <?= nl2br(htmlspecialchars($resort['hours'])) ?>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i> Pricing & Contact</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <?php if (!empty($resort['entrance_fee'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">💵 Entrance Fee</h6>
                                        <p class="mb-2 small price-highlight"><?= htmlspecialchars($resort['entrance_fee']) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if (!empty($resort['room_rates'])): ?>
                                    <div class="col-12 mb-2">
                                        <h6 class="fw-bold small">🏨 Room Rates</h6>
                                        <p class="mb-2 small"><?= htmlspecialchars($resort['room_rates']) ?></p>
                                    </div>
                                <?php endif; ?>
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($resort['contact'] ?? '') ?></p>
                            <?php if (!empty($resort['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($resort['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($resort['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($resort['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($resort['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars($resort['maps_embed']) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($resort['name'] ?? '') ?>:</p>
                            <div class="d-flex flex-wrap">
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($resort['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($resort['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($resort['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($resort['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($resort['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($resort['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
