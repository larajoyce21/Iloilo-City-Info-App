...
$query = "SELECT * FROM salons WHERE id = 4";
...
if (!empty($salon['transportation_routes'])) {
    $routes_data = json_decode($salon['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($salon['nearby_places'])) {
    $nearby_places = json_decode($salon['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $salon['image_detail'] ?? ''));
...
<head>
    ...
    <title><?= htmlspecialchars($salon['name'] ?? 'Salon Name') ?> - Iloilo Beauty Salons</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($salon['image'] ?? '') ?>');
...
<br>
    <section class="hero-section" 
        style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($salon['image'] ?? '') ?>') center/cover no-repeat;">
        <div class="container">
            <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($salon['name'] ?? '') ?></h1>
            <p class="lead"><?= htmlspecialchars($salon['location'] ?? '') ?></p>
        </div>
    </section>

    <main class="container mb-4">
        <section class="content-section">
            <h2 class="section-title">About <?= htmlspecialchars($salon['name'] ?? '') ?></h2>
            <div class="lead mb-4">
                ...
                <div class="slideshow-container">
                    <?php foreach ($detail_images as $index => $image): ?>
                        <div class="slide <?= $index === 0 ? 'active' : '' ?>">
                            <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($salon['name']) ?> Image <?= $index + 1 ?>">
        ...
        <section class="content-section">
            <h2 class="section-title">💇‍♀️ Salon Information</h2>
            ...
            <div class="row d-none d-md-flex">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <?php else: ?>
                                <p class="mb-0">No services listed.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h5 class="mb-0">ℹ️ Visitor Details</h5>
                        </div>
                        <div class="card-body">
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($salon['contact'] ?? '') ?></p>
                            <?php if (!empty($salon['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($salon['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($salon['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($salon['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="d-md-none mobile-side-by-side">
                <div class="row">
                            ...
                            <div class="card-body">
                                <?php if (!empty($services)): ?>
                                    <div class="service-list">
                                        <?php foreach ($services as $service): ?>
                                            <div class="service-item">
                                                <h6 class="small"><?= htmlspecialchars($service) ?></h6>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                <?php else: ?>
                                    <p class="mb-0 small">No services listed.</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-3">
                        <div class="card info-card h-100">
                            <div class="card-header">
                                <h6 class="mb-0">ℹ️ Visitor Details</h6>
                            </div>
                            <div class="card-body">
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($salon['contact'] ?? '') ?></p>
                                <?php if (!empty($salon['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($salon['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($salon['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($salon['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row d-none d-md-flex">
                ...
                <div class="col-lg-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($salon['hours'])): ?>
                                <?= nl2br(htmlspecialchars($salon['hours'])) ?>
                            <?php else: ?>
        ...
        <section class="content-section">
            ...
            <div class="row">
                <div class="col-lg-8 mb-3">
                    <?php if (!empty($salon['maps_embed'])): ?>
                        <div class="map-container">
                            <iframe src="<?= htmlspecialchars(convertToEmbedURL($salon['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                        </div>
                    <?php else: ?>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <?php if (!empty($nearby_places)): ?>
                                <p class="mb-2 small">Explore these places near <?= htmlspecialchars($salon['name'] ?? '') ?>:</p>
                            ...
                            <div class="d-flex flex-wrap gap-1">
                                <?php if (!empty($salon['maps_link'])): ?>
                                    <a href="<?= htmlspecialchars($salon['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                        <i class="fas fa-map-marked-alt me-1"></i> Maps
                                    </a>
                                <?php endif; ?>
                                
                                <?php if (!empty($salon['directions_link'])): ?>
                                    <a href="<?= htmlspecialchars($salon['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                    ...
                    <?php if (!empty($social_links)): ?>
                        <div class="card info-card">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($salon['mobile_app'])): ?>
                                    <hr class="my-3">
                                    <div class="d-flex flex-wrap gap-1">
                                        <a href="<?= htmlspecialchars($salon['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
