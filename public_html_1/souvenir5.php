...
$query = "SELECT * FROM souvenirs WHERE id = 5";
...
if (!empty($souvenir['transportation_routes'])) {
    $routes_data = json_decode($souvenir['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($souvenir['nearby_places'])) {
    $nearby_places = json_decode($souvenir['nearby_places'], true);
    if (!is_array($nearby_places)) {
        $nearby_places = [];
    }
}

$detail_images = array_filter(explode(',', $souvenir['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $souvenir['social_media'] ?? '')));

$hours = [];
if (!empty($souvenir['hours'])) {
    $lines = explode("\n", $souvenir['hours']);
...
$locations = array_filter(array_map('trim', explode('|', $souvenir['location'] ?? '')));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($souvenir['name']) ?> - Iloilo Souvenirs</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($souvenir['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($souvenir['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($souvenir['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($souvenir['location'] ?? '') ?></p>
  </div>
</section>

  <main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($souvenir['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        <?= nl2br(htmlspecialchars($souvenir['description'] ?? '')) ?>
      </div>
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($souvenir['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Souvenir Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($souvenir['significance'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">📦 Materials & Craftsmanship</h5>
                    </div>
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($souvenir['materials'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($souvenir['significance'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0">📦 Materials & Craftsmanship</h6>
                        </div>
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($souvenir['materials'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
                        ...
                        <div class="row">
                            <?php if (!empty($souvenir['price_range'])): ?>
                                <div class="col-md-6 mb-2">
                                    ...
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($souvenir['price_range'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($souvenir['contact'] ?? '') ?></p>
                            <?php if (!empty($souvenir['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($souvenir['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($souvenir['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($souvenir['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($souvenir['hours'])): ?>
                            <?= nl2br(htmlspecialchars($souvenir['hours'])) ?>
                        <?php else: ?>
                            <p class="mb-0">Operating hours not specified.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                            ...
                            <div class="row">
                                <?php if (!empty($souvenir['price_range'])): ?>
                                    <div class="col-12 mb-2">
                                        ...
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($souvenir['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                                <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($souvenir['contact'] ?? '') ?></p>
                                <?php if (!empty($souvenir['email'])): ?>
                                    <p class="mb-2 small">📧 Email: <?= htmlspecialchars($souvenir['email']) ?> </p>
                                <?php endif; ?>
                                <?php if (!empty($souvenir['website'])): ?>
                                    <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($souvenir['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body small">
                            <?php if (!empty($souvenir['hours'])): ?>
                                <?= nl2br(htmlspecialchars($souvenir['hours'])) ?>
                            <?php else: ?>
                                <p class="mb-0 small">Operating hours not specified.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($souvenir['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($souvenir['specialty'] ?? '') ?></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-2"><strong>🏪 Store:</strong> <?= htmlspecialchars($souvenir['store_name'] ?? '') ?></p>
                                <p class="mb-2"><strong>🛒 Market:</strong> <?= htmlspecialchars($souvenir['market_name'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($souvenir['store_address'])): ?>
                            <div class="list-group list-group-flush">
                                <div class="list-group-item border-0 px-0 py-1">
                                    <div class="d-flex align-items-start">
                                        <i class="fas fa-map-marker-alt text-danger mt-1 me-2"></i>
                                        <span class="small"><?= htmlspecialchars(trim($souvenir['store_address'])) ?></span>
                                    </div>
                                </div>
                            </div>
                        <?php else: ?>
                            <p class="text-muted small mb-0">Store location information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($souvenir['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($souvenir['specialty'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🏪 Store:</strong> <?= htmlspecialchars($souvenir['store_name'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>🛒 Market:</strong> <?= htmlspecialchars($souvenir['market_name'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-store me-2"></i> Store Location</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($souvenir['store_address'])): ?>
                                <div class="list-group list-group-flush">
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($souvenir['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($souvenir['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($souvenir['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($souvenir['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($souvenir['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($souvenir['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($souvenir['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($souvenir['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($souvenir['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
