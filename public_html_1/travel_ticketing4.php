...
$query = "SELECT * FROM travel_ticketings WHERE id = 4";
...
if (!empty($travel['transportation_routes'])) {
    $routes_data = json_decode($travel['transportation_routes'], true);
...
$nearby_places = [];
if (!empty($travel['nearby_places'])) {
    $nearby_places = json_decode($travel['nearby_places'], true) ?: [];
}

$detail_images = array_filter(explode(',', $travel['image_detail'] ?? ''));
$social_links = array_filter(array_map('trim', explode("\n", $travel['social_media'] ?? '')));

$hours = [];
if (!empty($travel['hours'])) {
    $lines = explode("\n", $travel['hours']);
...
if (!empty($destinations)) {
    ...
    $cleaned_destinations = array();
    
    foreach ($destination_lines as $line) {
        $trimmed = trim($line);
        if (!empty($trimmed)) {
            $cleaned_destinations[] = $trimmed;
        }
    }
...
<head>
    ...
    <title><?= htmlspecialchars($travel['name'] ?? 'Travel Agency Iloilo') ?> - Iloilo Travel Services</title>
    ...
    <style>
        ...
        /* Hero Section */
        .hero-section {
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('<?= htmlspecialchars($travel['image'] ?? '') ?>');
...
<section class="hero-section" 
  style="background:linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url('<?= htmlspecialchars($travel['image'] ?? '') ?>') center/cover no-repeat;">
  <div class="container">
    <h1 class="display-4 fw-bold mb-3"><?= htmlspecialchars($travel['name'] ?? '') ?></h1>
    <p class="lead"><?= htmlspecialchars($travel['location'] ?? '') ?></p>
  </div>
</section>

<main class="container mb-4">
    <section class="content-section">
      <h2 class="section-title">About <?= htmlspecialchars($travel['name'] ?? '') ?></h2>
      <div class="lead mb-4">
        ...
        <div class="slideshow-container">
          <?php foreach ($detail_images as $index => $image): ?>
            <div class="slide <?= $index === 0 ? 'active' : '' ?>">
              <img src="<?= htmlspecialchars($image) ?>" alt="<?= htmlspecialchars($travel['name']) ?> Image <?= $index + 1 ?>">
    ...
    <section class="content-section">
        <h2 class="section-title">Travel Agency Information</h2>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-code-branch me-2"></i> Destinations</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($travel['destinations'])): ?>
                            <div class="list-group list-group-flush">
                                <?php 
                                $destination_count = 0;
                                foreach (explode("\n", $travel['destinations']) as $destination):
                                    if (trim($destination) != ''): 
                                        $destination_count++;
                                        ?>
                                        <div class="list-group-item border-0 px-0 py-1">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <span class="small"><?= htmlspecialchars(trim($destination)) ?></span>
                                            </div>
                                        </div>
                                    <?php endif;
                                endforeach; ?>
                            </div>
                            <p class="text-muted small mt-2">Total: <?= $destination_count ?> destinations</p>
                        <?php else: ?>
                            <p class="text-muted small mb-0">Destination information not available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($travel['services'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <p class="mb-0"><?= nl2br(htmlspecialchars($travel['offers'] ?? '')) ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-md-none mobile-side-by-side">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-code-branch me-2"></i> Destinations</h6>
                        </div>
                        <div class="card-body">
                            <?php if (!empty($travel['destinations'])): ?>
                                <div class="list-group list-group-flush">
                                ...
                                <p class="text-muted small mt-1">Total: <?= $destination_count ?> destinations</p>
                            <?php else: ?>
                                <p class="text-muted small mb-0">Destination information not available.</p>
                            <?php endif; ?>
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($travel['services'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 mb-3">
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            <p class="mb-0 small"><?= nl2br(htmlspecialchars($travel['offers'] ?? '')) ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row d-none d-md-flex">
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php if (!empty($travel['price_range'])): ?>
                                <div class="col-md-6 mb-2">
                                    <h6 class="fw-bold">💵 Price Range</h6>
                                    <p class="mb-2"><?= nl2br(htmlspecialchars($travel['price_range'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <p class="mb-2">📲 Phone: <?= htmlspecialchars($travel['contact'] ?? '') ?></p>
                            <?php if (!empty($travel['email'])): ?>
                                <p class="mb-2">📧 Email: <?= htmlspecialchars($travel['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($travel['website'])): ?>
                                <p class="mb-2">🌐 Website: <a href="<?= htmlspecialchars($travel['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-6 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                      <?= nl2br(htmlspecialchars($travel['hours'])) ?>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="row">
                                <?php if (!empty($travel['price_range'])): ?>
                                    <div class="col-12 mb-2">
                                        ...
                                        <p class="mb-2 small"><?= nl2br(htmlspecialchars($travel['price_range'])) ?></p>
                                    </div>
                                <?php endif; ?>
                                
                            <p class="mb-2 small">📲 Phone: <?= htmlspecialchars($travel['contact'] ?? '') ?></p>
                            <?php if (!empty($travel['email'])): ?>
                                <p class="mb-2 small">📧 Email: <?= htmlspecialchars($travel['email']) ?> </p>
                            <?php endif; ?>
                            <?php if (!empty($travel['website'])): ?>
                                <p class="mb-2 small">🌐 Website: <a href="<?= htmlspecialchars($travel['website']) ?>" target="_blank" class="text-decoration-none"> Visit Website</a></p>
                            <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-10 mb-3">
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-2"><strong>📅 Established:</strong> <?= htmlspecialchars($travel['year_established'] ?? '') ?></p>
                                <p class="mb-2"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($travel['specialty'] ?? '') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-7 mb-3">
                    <div class="card info-card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-1 small"><strong>📅 Established:</strong> <?= htmlspecialchars($travel['year_established'] ?? '') ?></p>
                                    <p class="mb-1 small"><strong>⭐ Specialty:</strong> <?= htmlspecialchars($travel['specialty'] ?? '') ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section class="content-section">
        ...
        <div class="row">
            <div class="col-lg-8 mb-3">
                <?php if (!empty($travel['maps_embed'])): ?>
                    <div class="map-container">
                        <iframe src="<?= htmlspecialchars(convertToEmbedURL($travel['maps_embed'])) ?>" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy"></iframe>
                    </div>
                <?php else: ?>
                ...
                <div class="card info-card h-100">
                    ...
                    <div class="card-body">
                        <?php if (!empty($nearby_places)): ?>
                            <p class="mb-2 small">Explore these places near <?= htmlspecialchars($travel['name'] ?? '') ?>:</p>
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            <?php if (!empty($travel['maps_link'])): ?>
                                <a href="<?= htmlspecialchars($travel['maps_link']) ?>" class="btn btn-outline-primary btn-sm" target="_blank">
                                    <i class="fas fa-map-marked-alt me-1"></i> Maps
                                </a>
                            <?php endif; ?>
                            
                            <?php if (!empty($travel['directions_link'])): ?>
                                <a href="<?= htmlspecialchars($travel['directions_link']) ?>" class="btn btn-primary btn-sm" target="_blank">
                ...
                <?php if (!empty($social_links)): ?>
                    <div class="card info-card">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($travel['mobile_app'])): ?>
                                <hr class="my-3">
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="<?= htmlspecialchars($travel['mobile_app']) ?>" class="btn btn-dark btn-sm" target="_blank">
