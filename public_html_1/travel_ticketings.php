...
$result = $conn->query("SELECT * FROM travel_ticketings");

$searchNameToTrigger = '';
if (isset($_GET['search'])) {
    $searchNameToTrigger = $_GET['search'];
} elseif (isset($_SESSION['search_trigger'])) {
    if ($_SESSION['search_trigger']['table'] === 'travel_ticketings') {
...
$query = "SELECT * FROM travel_ticketings";
$travel_ticketings_exist = false;

if ($stmt = $conn->prepare($query)) {
    $stmt->execute();
    $result = $stmt->get_result();
    $travel_ticketings_exist = $result->num_rows > 0;
    $stmt->close();
}
?>
...
<div class="album py-5" id="bakeries">
    <div class="container">
        <br>
        <br>
        <br>
        <br>
        <h1 class="page-title text-light text-center">Travel Ticketings in Iloilo City</h1> 
        
        <div class="card-container">
            <?php if ($travel_ticketings_exist): ?>
                <?php while ($travel_ticketing = $result->fetch_assoc()): ?>
                    <div class="card shadow-sm">
                        <img src="<?= htmlspecialchars($travel_ticketing['image']) ?>" class="card-img-top" alt="<?= htmlspecialchars($travel_ticketing['name']) ?>">
                        <div class="card-body text-center">
                            <h6 class="card-title"><?= htmlspecialchars($travel_ticketing['name']) ?></h6>
                            <?php if (!empty($travel_ticketing['details_link'])): ?>
                                <a href="<?= htmlspecialchars($travel_ticketing['details_link']) ?>" class="card-button">
                                    View Details
                                </a>
                            <?php else: ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="alert alert-info">No travel ticketing found.</div>
            <?php endif; ?>
        </div>
    </div>
</div>
